...
//
// Every gallery block that carries a Solid snippet, in ONE tree, compiled by
...
await runHostProbeApp<Ui>({
    applicationId: 'eu.jumplink.AdwaitaGallerySolid',
    // `createRoot`, not bare JSX: a computation created without an owner is never
    // disposed, Solid says so on stderr, and the harness counts stderr.
    build: (app) => {
        const root = createRoot(() => widgetOf((<Gallery />) as HostNode));
        return { root, owner: root, app };
    },
    assert: (ui, check) => assertAll(ui.root, check),
    teardown: (ui) => ui.owner.run_dispose(),
    present: (ui) => present(ui, 'Solid'),
});
