...
export class SpiderEnemy extends EnemyActor {
    ...
    onPreUpdate(_engine: ex.Engine, _elapsed: number): void {
        ...
        const bottomLeft = this.raycast(
            new ex.Ray(
                ex.vec(this.collider.bounds.left + 1, Math.round(this.collider.bounds.bottom) - 1),
                ex.Vector.Down,
            ),
            1,
        );
        ...
        const bottomRight = this.raycast(
            new ex.Ray(
                ex.vec(this.collider.bounds.right - 1, Math.round(this.collider.bounds.bottom) - 1),
                ex.Vector.Down,
            ),
            1,
        );
