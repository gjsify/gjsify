...
export class CircularSawHazard extends PhysicsActor {
    ...
    onPreUpdate(_engine: ex.Engine, elapsed: number) {
        ...
        const [groundHit] = this.raycast(new ex.Ray(ex.vec(edge, bottom), ex.Vector.Down), 1, {
            collisionGroup: CollisionGroup.Ground,
        });

        // check if we are hitting a wall
        const [wallHit] = this.raycast(
            new ex.Ray(ex.vec(edge, bottom), this.direction === 1 ? ex.Vector.Right : ex.Vector.Left),
            1,
            {
                collisionGroup: CollisionGroup.Ground,
            },
