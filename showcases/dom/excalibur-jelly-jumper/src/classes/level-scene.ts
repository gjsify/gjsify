...
import type { FactoryProps, TiledObject } from '@excaliburjs/plugin-tiled';
...
export default class LevelScene extends ex.Scene {
    ...
    entityFactory: Record<string, (props: FactoryProps) => ex.Entity | undefined> = {
        /* Player */
        Player: (props) =>
            new Player({
                x: props.object?.x ?? 0,
            ...
            return new MovingPlatform(
                ...
                (actions) => {
                    ...
                    actions.repeatForever((ctx) =>
                        ctx.moveTo(ex.vec(pathObject.x!, pathObject.y!), speed).moveTo(ex.vec(x, y), speed),
                    );
                },
            );
        },
        ...
        SpiderEnemy: makeSpawner((args, props) => {
            const typeProp = props.object?.properties.get('type') as 'green' | 'gray' | undefined;
        ...
        BirdEnemy: makeSpawner((args, props) => {
            const typeProp = props.object?.properties.get('type') as 'purple' | 'orange';
    ...
    constructor(args: { tilemap: TiledResource; background: ex.ImageSource; song?: ex.Sound }) {
        super();
