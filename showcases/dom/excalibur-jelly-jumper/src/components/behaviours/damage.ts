...
export class DamageComponent extends ex.Component {
    ...
    constructor({ amount, cancelContactOnDamage }: { amount: number; cancelContactOnDamage?: boolean }) {
        super();
    ...
    onPreCollisionResolve(event: ex.PreCollisionEvent) {
        ...
        const isBeingStomped = other instanceof PhysicsActor && stompable?.isBeingStomped(other);
        ...
        if (damageable) {
            ...
            if (!dead && !isBeingStomped) {
                damageable.damage(this.amount, other.center.x < this.owner.center.x ? 'left' : 'right');
            }
        }
    }
}
