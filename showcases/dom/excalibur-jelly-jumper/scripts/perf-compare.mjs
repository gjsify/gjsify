...
if (!gjs.length) {
    console.error(`No [PERF] lines found in ${gjsFile}`);
    process.exit(1);
}
if (!browser.length) {
    console.error(`No [PERF] lines found in ${browserFile}`);
    process.exit(1);
}
...
if (drawDelta > 3)
    console.log(`  • Draw is ${drawDelta.toFixed(1)}ms slower on GJS → consider pixelRatio:2 or WebGL bridge overhead`);
if (updateDelta > 3)
    console.log(
        `  • Update is ${updateDelta.toFixed(1)}ms slower on GJS → physics/logic cost; profile actor update() hotpaths`,
    );
if (unexplained > 2)
    console.log(
        `  • ${unexplained.toFixed(1)}ms/frame unaccounted overhead on GJS (not update, not draw) → WebGL bridge + GTK compositor + SpiderMonkey vs V8`,
    );
if (droppedAvg > 5 && updateDelta > 3)
    console.log(
        `  • ${droppedAvg.toFixed(1)} dropped frames/batch on GJS → if fixedUpdateFps is already ≤30, the physics step itself is too expensive`,
    );
if (droppedAvg > 5 && updateDelta <= 3)
    console.log(
        `  • ${droppedAvg.toFixed(1)} dropped frames/batch on GJS → caused by per-frame runtime overhead, not physics; core package optimization needed`,
    );
if (drawDelta <= 3 && updateDelta <= 3 && unexplained <= 2)
    console.log('  • No significant bottleneck detected from logs alone');
console.log();
