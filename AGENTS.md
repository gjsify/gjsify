...
3. **Common tests vs. platform-specific tests:**
   ...
   - **Platform-specific tests** (`*.gjs.spec.ts` or guarded with `on('Gjs')`/`on('Node.js')`): May import `@girs/*` and use platform-specific APIs. Use these only for testing platform-specific behavior that cannot be verified cross-platform (e.g. WebSocket via Soup server, WebGL via GTK GL context, Gio stream integration). Keep them minimal — the focus should always be on common tests.
   - Packages that are inherently GJS-only (e.g. `web-globals`, `webgl`) may have only GJS tests. Their `package.json` `test` script should omit `test:node`.
