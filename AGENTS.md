...
Node.js/Web/DOM API + Framework for GJS (GNOME JS). npm-workspaces monorepo, v0.11.0, ESM-only, GNOME libs. Bootstraps via the committed `packages/infra/cli/dist/cli.gjs.mjs` GJS bundle — `gjsify install --immutable` is the supported install path (no yarn / no Node-only npm CLI required at runtime, see Phase D.7d). Four equal pillars: **Node.js** `packages/node/` (42 + 1 meta) | **Web** `packages/web/` (21 + 1 meta) | **DOM** `packages/dom/` (2) | **Framework** `packages/framework/` (6 bridge pkgs). `packages/infra/` + `packages/gjs/` = supporting infra. **NativeScript Bridge** `packages/nativescript-bridge/` (4 pkgs — `@gjsify/native-fs-bridge`, `@gjsify/native-platform`, `@gjsify/adwaita-nativescript`, `@gjsify/devtools-nativescript`) = native mobile API wrappers + design-identity widgets + an in-app devtools agent for the NativeScript runtime axis (Welle 5+).
...
| adwaita (`@gjsify/adwaita-nativescript`) | Android + iOS | **Design-identity axis 4** — **14 native Adwaita Tier-1 widgets** (real NativeScript components, not a webview), parallel to `@gjsify/adwaita-web`. Rows: `AdwPreferencesPage` (`ScrollView`+`StackLayout`, `addGroup`), `AdwPreferencesGroup` (`StackLayout`), `AdwActionRow` (`GridLayout` cols `*, auto`), `AdwSwitchRow` (`Switch` suffix, `active`↔`checked` + `notify::active`), `AdwEntryRow`/`AdwPasswordEntryRow` (title-above-`TextField` stacked in col 0, `text` + `notify::text`, `secure`), `AdwComboRow` (inline value+chevron suffix, tap→native `action()` chooser, `selectedIndex`/`selectedValue` + `notify::selected`), `AdwSpinRow` (`−`/`+` `Button` stepper, clamped `value`/`min`/`max`/`step` + `notify::value`), `AdwExpanderRow` (2nd grid-row disclosure, `addRow`/`expanded` + `notify::expanded`). Chrome: `AdwButton` (extends `Button`, `variant`→`.suggested-action`/`.destructive-action`/`.flat`/`.pill`), `AdwBanner` (`GridLayout` strip, neutral info bg, `title`/`buttonLabel`/`revealed` + `buttonClicked`), `AdwAvatar` (circular `GridLayout`, `text`→initials, `size`), `AdwWindowTitle` (centered title+subtitle `StackLayout` for `ActionBar`), `AdwClamp` (max-width center, `maximumSize` def 600). All Material buttons set `androidElevation:0` (Adwaita = flat). Boxed-list rows: tight 10×16 padding + 1px top-border hairline separator. `registerAdwaitaElements()` wires the global `registerElement` for XML use; static `theme/adwaita.css` (NS CSS subset, light + `.ns-dark` for every widget); Adwaita Sans fonts via `@gjsify/adwaita-fonts` TTFs (dual iOS PostScript / Android file-name `font-family` + `App_Resources` copy step). **CSS-subset compromises** (no `transform`/animation/gradient/shadow/focus-ring): expander reveal = instant `visibility` toggle (no slide), avatar = single flat accent fill (no per-name gradient), buttons use flat fills (no hover/focus states). 41 tests. |
