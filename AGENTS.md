...
|L2 integration points: **`gjsGiNodePlugin` SHIPPED** (`packages/infra/rolldown-plugin-gjsify/src/plugins/gjs-gi-node.ts`) — for `--app node` it runs before `gjsImportsEmptyPlugin` and resolveId-rewrites `gi://Ns?version=X` → a virtual module `export default requireGi('Ns','X')` from `@gjsify/node-gi/gi` (the L1 backend), kept EXTERNAL (native addon; added to `exactExternal` in `app/node.ts`); `@girs/*` still falls through to empty. e2e `tests/e2e/node-gi-build`. STILL PENDING: fill `ALIASES_GJS_FOR_NODE` (`resolve-npm/lib/index.mjs`, currently `{}`) for bare built-ins; node byte-1 banner (template = `process-stub.ts`) to seed `globalThis.imports` + GJS globals (`print`/`printerr`/`log`/`logError`/`ARGV`/`pkg`) BEFORE module init; `getGlobalsMapForTarget('node')` + the acorn scanner (data-driven via `Object.keys(GJS_GLOBALS_MAP)`) inject on detection. Use the resolveId-hook `{external|id}` form (NOT a function `external` — the native-rolldown JSON boundary drops functions)
