...
| storybook (`@gjsify/storybook-nativescript`) | Android + iOS | **NativeScript renderer for the `@gjsify/stories` contract** — the NS counterpart of `@gjsify/storybook` (GTK) + `@gjsify/adwaita-storybook` (browser), rendering the SAME stories (shared renderer-agnostic `*.meta.ts`) in a native Adwaita component browser built from REAL `@gjsify/adwaita-nativescript` widgets (NOT a webview), so the three targets screenshot-compare 1:1. `StoryView` (extends NS `StackLayout`) is the NS twin of browser `StoryElement` / GTK `StoryWidget` — IDENTICAL authoring surface (`(meta, story='Default', customRoot?)` ctor; `meta`/`story`/`args` getters, `args` setter→`updateArgs`+notify; `onArgsChanged`/`setArg`; overridable `initialize`/`updateArgs`/`teardown`; `addContent`) so a `*.ns.ts` story is a near-1:1 port of its `*.web.ts` twin. Default chrome = `AdwClamp`+`AdwPreferencesGroup` header (title+description) above a centered dashed-frame `.story-stage`. `StoryRegistry` (one-per-run, no singleton; category grouping). `createControlRow` maps each `StoryControl` kind to a live two-way-bound Adwaita row: TEXT→`AdwEntryRow`, BOOLEAN→`AdwSwitchRow`, NUMBER/RANGE→`AdwSpinRow`, SELECT→`AdwComboRow`, COLOR→`AdwEntryRow` hex-fallback (NS has no native color/slider row). `StorybookNativeApp`/`runStorybook(options)` lay out a two-column `GridLayout`: a boxed-list story SIDEBAR grouped by category (tap→select), a PREVIEW column with an Adwaita preview header + the selected `StoryView`, and a collapsible CONTROLS section (the GTK `OverlaySplitView` sidebar folded under the preview for the phone form factor — toggled via a header button). Exposes the same MCP/devtools control surface as the browser app (`listStories`/`openStoryByTitle`/`getCurrentStory`/`setActiveArg`/`activeStory`). Re-exports the `@gjsify/stories` contract (one import per story). Static `theme/storybook.css` (NS CSS subset, `.sb-*`+`.story-stage` chrome, light + `.ns-dark`). `@nativescript/core` optional peer (ambient `src/ns-core.d.ts` superset of the adwaita slice). Pure named exports. Tests: registry grouping + control-row arg binding (mock `StoryView`) + `argsFromControls`, GJS + Node off-device. |
