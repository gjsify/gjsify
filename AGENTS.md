...
|L2 integration points (the reverse machinery is pre-wired but EMPTY): replace `gjsImportsEmptyPlugin` for `--app node` with a `gjsGiNodePlugin` that resolveId-rewrites `gi://Ns?version=X` → a runtime `require` into the L1 backend (`packages/infra/rolldown-plugin-gjsify/src/app/node.ts` stubs it to empty TODAY); fill `ALIASES_GJS_FOR_NODE` (`resolve-npm/lib/index.mjs`, currently `{}`) for built-ins; node byte-1 banner (template = `process-stub.ts`) seeds `globalThis.imports` + GJS globals (`print`/`printerr`/`log`/`logError`/`ARGV`/`pkg`) BEFORE module init; `getGlobalsMapForTarget('node')` + the acorn scanner (data-driven via `Object.keys(GJS_GLOBALS_MAP)`) inject on detection. Use the resolveId-hook `{external|id}` form (NOT a function `external` — the native-rolldown JSON boundary drops functions)
|mainloop: inherit node-gtk's model (libuv backend-fd as a GLib `GSource`, `uv_run(NOWAIT)` in dispatch) so `Gio.Application.run()`/`GLib.MainLoop.run()` BLOCKS like GJS yet keeps Node async alive — call `startLoop()` before the blocking run. The single most reusable node-gtk artifact
