...
async function main(): Promise<void> {
    ...
    console.log(
        `${DIM}Runtime: ${runtimeName} | Workers: ${WORKER_COUNT} | Payload: ${(PAYLOAD_BYTES / 1024 / 1024).toFixed(0)} MiB${RESET}`,
    );
    console.log('');
    ...
    workers.forEach((w, i) =>
        w.once('message', (msg: { slot: number; ms: number }) => {
    ...
    for (let i = 0; i < WORKER_COUNT; i++) {
        ...
        const d = sb.readBytes(DIGEST_OFF + i * 32, 32);
        console.log(
            `  ${CYAN}Worker ${i}${RESET}: ${sliceMiB.toFixed(2)} MiB in ${ms.toString().padStart(4, ' ')} ms ` +
