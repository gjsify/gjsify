...
async function main(): Promise<void> {
    ...
    pool.on('result', (result: TaskResult) => {
        results.push(result);
        console.log(
            `  ${GREEN}Done:${RESET} ${result.taskId.slice(0, 8)}... → ${result.result} (${result.duration}ms)`,
        );
    });
    ...
    const tasks: Task[] = [
        ...
        { id: randomUUID(), type: 'fibonacci', data: 35 },
        {
            id: randomUUID(),
            type: 'sort',
            data: Array.from({ length: 100000 }, () => Math.floor(Math.random() * 1000000)),
        },
    ...
    for (const task of tasks) {
        console.log(
            `  ${DIM}Submit:${RESET} ${task.id.slice(0, 8)}... [${task.type}(${typeof task.data === 'object' ? 'array' : task.data})]`,
        );
        pool.submit(task);
    }
    ...
    console.log(
        `${DIM}Pool size: ${poolSize} | Total task time: ${results.reduce((sum, r) => sum + r.duration, 0)}ms${RESET}`,
    );
}
