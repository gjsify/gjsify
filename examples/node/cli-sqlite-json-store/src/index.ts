...
class SqliteTodoStore {
    ...
    findById(id: number): Todo | undefined {
        const row = this.db.prepare('SELECT * FROM todos WHERE id = :id').get({ id }) as Row | undefined;
        return row ? this.#toTodo(row) : undefined;
    }

    findAll(filter?: Partial<{ priority: string; done: boolean }>): Todo[] {
        const rows = this.db.prepare('SELECT * FROM todos ORDER BY id').all() as Row[];
    ...
    update(id: number, data: Partial<Pick<Todo, 'title' | 'priority' | 'done'>>): Todo | null {
        ...
        this.db.prepare('UPDATE todos SET title = :title, priority = :priority, done = :done WHERE id = :id').run({
            id,
    ...
    delete(id: number): boolean {
        const result = this.db.prepare('DELETE FROM todos WHERE id = :id').run({ id });
        return (result.changes as number) > 0;
    }

    count(): number {
        const row = this.db.prepare('SELECT COUNT(*) as count FROM todos').get() as Row | undefined;
