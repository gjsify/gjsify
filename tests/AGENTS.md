...
```
tape→unit: `t.equal`→`toBe` | `t.deepEqual`→`toStrictEqual` | `t.ok/notOk`→`toBeTruthy/Falsy` | `t.error(err)`→`expect(err).toBeFalsy()` | `t.throws(fn)`→`toThrow()` | `t.plan/t.end` omitted | callback cleanup → promisified. **Never weaken**; failure → root-cause fix. Exception: pre-known out-of-scope gap → wrap in `on('Node.js', …)` + document in header + `status/open-todos/`. Suites + pass counts: `status/integration-coverage.md` (webtorrent, socket.io, streamx, chokidar, autobahn, claude-agent-sdk, devtools-cdp, …).
