...
export default async () => {
    await describe('acorn-walk simple/ancestor/full', async () => {
        await it('simple: counts node visits per type', async () => {
            const ast = parse('function f(a, b) { return a + b; } f(1, 2); const x = 3;', PARSE_OPTS);
            const counts: Record<string, number> = {};
            const idNames: string[] = [];
            simple(ast, {
                Identifier(node) {
                    counts.Identifier = (counts.Identifier || 0) + 1;
                    idNames.push(node.name);
                },
                Literal(_node) {
                    counts.Literal = (counts.Literal || 0) + 1;
                },
                FunctionDeclaration(_node) {
                    counts.FunctionDeclaration = (counts.FunctionDeclaration || 0) + 1;
                },
            });
        ...
        await it('simple: state is threaded through visitor calls', async () => {
            ...
            simple(
                ast,
                {
                    Literal(node, st: { sum: number }) {
                        st.sum += node.value as number;
                    },
                },
                undefined,
                state,
        ...
        await it('ancestor: provides ancestor chain', async () => {
            ...
            ancestor(ast, {
                FunctionDeclaration(node, _st, ancestors: AnyNode[]) {
                    if (node.id?.name === 'inner') {
                        innerAncestors = ancestors.map((a: AnyNode) => a.type);
                    }
                },
            });
        ...
        await it('findNodeAt locates a node at a given offset', async () => {
            ...
            const found = findNodeAt(ast, src.indexOf('xx'), src.indexOf('xx') + 2);
            expect(found).toBeDefined();
            expect(found!.node.type).toBe('Identifier');
            expect((found!.node as Extract<AnyNode, { type: 'Identifier' }>).name).toBe('xx');
        });
        ...
        await it('findNodeAround finds the innermost matching node', async () => {
            ...
            const found = findNodeAround(ast, offset, 'BinaryExpression');
            expect(found).toBeDefined();
            expect(found!.node.type).toBe('BinaryExpression');
            expect((found!.node as Extract<AnyNode, { type: 'BinaryExpression' }>).operator).toBe('+');
        });
    });
};
