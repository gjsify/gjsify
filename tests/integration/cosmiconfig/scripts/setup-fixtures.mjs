...
await writeFile(join(dest, 'projectA', '.foorc.json'), JSON.stringify({ source: 'json-rc', value: 1 }, null, 2) + '\n');
...
await writeFile(join(dest, 'projectB', 'package.json'), JSON.stringify({ name: 'projectB', type: 'module' }) + '\n');
await writeFile(join(dest, 'projectB', 'foo.config.js'), "export default { source: 'js-config', value: 2 };\n");
...
await writeFile(join(dest, 'projectD', 'foo.config.mjs'), "export default { source: 'mjs-config', value: 4 };\n");

// projectE — CJS .cjs with module.exports
await mkdir(join(dest, 'projectE'), { recursive: true });
await writeFile(join(dest, 'projectE', '.foorc.cjs'), "module.exports = { source: 'cjs-rc', value: 5 };\n");

// projectF — YAML rc
await mkdir(join(dest, 'projectF'), { recursive: true });
await writeFile(join(dest, 'projectF', '.foorc.yaml'), 'source: yaml-rc\nvalue: 6\n');
...
await writeFile(join(dest, 'projectG', 'package.json'), JSON.stringify({ name: 'projectG' }) + '\n');
await writeFile(join(dest, 'projectG', '.foorc.json'), JSON.stringify({ source: 'walk-up', value: 7 }) + '\n');
...
await writeFile(join(dest, 'projectI', '.foorc.json'), JSON.stringify({ source: 'transform-input', value: 9 }) + '\n');

// projectJ — cache hit/miss target
await mkdir(join(dest, 'projectJ'), { recursive: true });
await writeFile(join(dest, 'projectJ', '.foorc.json'), JSON.stringify({ source: 'cache-target', value: 10 }) + '\n');
