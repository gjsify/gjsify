...
export default async () => {
    await describe('MCP resource()', async () => {
        ...
        await it('should read resource contents', async () => {
            ...
            mcpServer.registerResource('test', 'test://localhost/resource', {}, async () => ({
                contents: [{ uri: 'test://localhost/resource', text: 'Hello from resource' }],
            }));
        ...
        await it('should send resource list changed notification', async () => {
            ...
            mcpServer.registerResource('initial', 'test://initial', {}, async () => ({
                contents: [{ uri: 'test://initial', text: 'Initial' }],
            }));
        ...
        await it('should prevent duplicate resource registration', async () => {
            ...
            try {
                mcpServer.registerResource('test', 'test://resource', {}, async () => ({
                    contents: [{ uri: 'test://resource', text: 'Duplicate' }],
                }));
            } catch (e) {
                threw = true;
