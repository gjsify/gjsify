...
async function setupServer() {
    ...
    const mcpServer = new McpServer({ name: 'test-http-server', version: '1.0.0' });
    ...
    mcpServer.registerResource('test', 'test://localhost/info', {}, async () => ({
    ...
    await describe('MCP Streamable HTTP Transport', async () => {
        ...
        await it('should sustain inspector-like mixed workload across sessions', async () => {
            ...
            const buildMcpServer = () => {
                ...
                m.registerResource('info', 'info://server/meta', {}, async () => ({
                    contents: [{ uri: 'info://server/meta', text: 'inspector-server' }],
                }));
                mcpServers.add(m);
                return m;
            };
