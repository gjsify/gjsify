    ...
    await describe('webtorrent/wire-transfer: bittorrent-protocol Wire over streamx.pipeline', async () => {
        ...
        await it(
            ...
            async () => {
                ...
                const { writeCompleted, timedOut } = await new Promise<{ writeCompleted: boolean; timedOut: boolean }>(
                    (resolve) => {
        ...
        await it('net.Socket client-to-server data flow', async () => {
            const { serverGotData, timedOut } = await new Promise<{ serverGotData: boolean; timedOut: boolean }>(
                (resolve) => {
        ...
        await it(
            'transfers all pieces over loopback without stalling',
            async () => {
                const infoHash = makeInfoHash();
                const seederPeerId = makePeerId(0x01);
                const downloaderPeerId = makePeerId(0x02);
                ...
                const { piecesReceived, timedOut } = await new Promise<{ piecesReceived: number; timedOut: boolean }>(
                    (resolve) => {
                        ...
                        server = createServer((conn) => {
                            ...
                            seederWire.on(
                                ...
                                (
                                    ...
                                    _offset: number,
                                    _length: number,
                                    respond: (err: Error | null, data: Buffer) => void,
                                ) => {
