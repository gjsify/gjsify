...
describe('gjsify install — nested-node_modules version conflict (Phase D.7b)', { timeout: 90_000 }, () => {
    ...
    it('installs both readable majors — one hoisted, the other nested', async () => {
        ...
        const rootReadableVersion = JSON.parse(
            readFileSync(join(projectDir, 'node_modules', 'readable', 'package.json'), 'utf-8'),
        ).version;
        ...
        if (rootReadableVersion.startsWith('2.')) {
            ...
            assert.ok(
                !existsSync(nestedUnderBridge),
                'bridge-pkg should NOT have nested readable when its version is at root',
            );
            ...
            assert.ok(
                !existsSync(nestedUnderModern),
                'modern-pkg should NOT have nested readable when its version is at root',
            );
        } else {
