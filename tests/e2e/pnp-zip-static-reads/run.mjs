...
function packFixture(tarballsDir) {
    ...
    writeFileSync(join(fixtureDir, 'data.json'), JSON.stringify({ secret: 'inlined-from-pnp-zip' }) + '\n');
...
describe('PnP zip-resident static reads E2E', { timeout: 10 * 60 * 1000 }, () => {
    ...
    before(() => {
        ...
        if (existsSync(cacheDir)) {
            ...
            if (!/@fixture-pnp-zip-reads.*\.zip/.test(cached)) {
                throw new Error(`expected @fixture-pnp-zip-reads .zip in ${cacheDir}, got: ${cached}`);
            }
        }
    });
    ...
    function buildAndAssert(appTarget, outFile) {
        ...
        execFileSync(
            'yarn',
            ['gjsify', 'build', 'src/app.ts', '--app', appTarget, '--outfile', bundlePath, '--no-minify'],
            {
                cwd: projectDir,
                stdio: 'pipe',
                timeout: 2 * 60 * 1000,
        ...
        assert.ok(
            bundle.includes('inlined-from-pnp-zip'),
        ...
        assert.ok(
            bundle.includes('"@fixture/pnp-zip-reads"') || bundle.includes(`"name":"@fixture/pnp-zip-reads"`),
