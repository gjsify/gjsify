...
describe('CLI flatpak subcommand group E2E', { timeout: 10 * 60 * 1000 }, () => {
    ...
    it('flatpak init writes a GNOME-Platform manifest with build-options', () => {
        ...
        assert.equal(manifest['sdk-extensions'], undefined, 'pure gjsify project should not need any SDK extensions');
        assert.equal(manifest['build-options'], undefined, 'no build-options without extensions / explicit appendPath');
    ...
    it('flatpak init --cli-only strips GUI finish-args but keeps GNOME runtime', () => {
        ...
        assert.equal(
            manifest.runtime,
            'org.gnome.Platform',
            '--cli-only must keep GNOME runtime — GJS bundles need GLib/GIO at runtime',
        );
    ...
    it('flatpak init --kind cli emits console-application MetaInfo + skip-icons-check flathub.json + no .desktop', () => {
        ...
        assert.equal(
            existsSync(join(cliProjectDir, 'data/org.example.CliTool.desktop.in')),
            false,
    ...
    it('flatpak init --kind app with rich AppStream config emits all advanced sections', () => {
                ...
                gjsify: {
                    flatpak: {
                        ...
                        description: [
                            ...
                            {
                                ul: [{ item: 'First feature', translatorHint: 'Feature 1' }, 'Second feature'],
        ...
        const metainfo = readFileSync(join(richProjectDir, 'data/org.example.RichApp.metainfo.xml.in'), 'utf-8');

        // Translator hints land on the right tags
        assert.match(
            metainfo,
            /<!-- TRANSLATORS: App tagline shown in app stores -->\s*<summary>Rich content demo<\/summary>/,
        );
        ...
        assert.match(
            metainfo,
            /<supports>[\s\S]*<control>keyboard<\/control>[\s\S]*<control>pointing<\/control>[\s\S]*<control>touch<\/control>[\s\S]*<internet>offline-only<\/internet>[\s\S]*<\/supports>/,
        );
        ...
        assert.match(
            metainfo,
            /<release version="1\.0\.0" date="2026-05-18">[\s\S]*<description>[\s\S]*<p>Initial rich release\.<\/p>[\s\S]*<ul>[\s\S]*<li>Feature A<\/li>[\s\S]*<\/ul>[\s\S]*<\/description>[\s\S]*<\/release>/,
        );
...
function writeShim(binDir, name, traceFile, createFile = false, exitCode = 0) {
    ...
    const lines = ['#!/bin/sh', `echo "$@" >> ${shellQuote(trace)}`];
    if (createFile) {
        lines.push(
