...
describe('--app browser gi://-free bundle E2E', { timeout: 10 * 60 * 1000 }, () => {
    ...
    it('builds a Web-global entry without gi:// or @girs/ leakage', () => {
        ...
        execFileSync('npx', ['gjsify', 'build', 'src/app.ts', '--app', 'browser', '--outfile', 'dist/app.mjs'], {
            cwd: projectDir,
            stdio: 'pipe',
            timeout: 90 * 1000,
        });
    ...
    it('entry using node:assert alias produces no gi:// or @girs/ leakage', () => {
        writeFileSync(
            join(projectDir, 'src', 'with-assert.ts'),
            [
        ...
        execFileSync(
            'npx',
            ['gjsify', 'build', 'src/with-assert.ts', '--app', 'browser', '--outfile', 'dist/with-assert.mjs'],
            {
                cwd: projectDir,
                stdio: 'pipe',
                timeout: 90 * 1000,
