...
describe('CLI gresource E2E', { timeout: 10 * 60 * 1000 }, () => {
    ...
    before(() => {
        ...
        writeFileSync(join(projectDir, 'data', 'app.gresource.xml'), GRESOURCE_XML);
        writeFileSync(join(projectDir, 'data', 'hello.txt'), HELLO_TXT);
    ...
    it('compiles a .gresource.xml into a .gresource blob next to the descriptor (default target)', (t) => {
        ...
        assert.ok(existsSync(out), 'data/app.gresource is missing after default compilation');
        assert.ok(statSync(out).size > 0, 'data/app.gresource is empty — compilation produced no output');
    });

    it('honours --target to write the .gresource to an explicit path', (t) => {
        if (!projectDir) return t.skip();
        const out = join(projectDir, 'out', 'custom.gresource');
        execFileSync('npx', ['gjsify', 'gresource', 'data/app.gresource.xml', '--target', out], {
            cwd: projectDir,
            stdio: 'pipe',
            timeout: 60 * 1000,
        });
        assert.ok(existsSync(out), `${out} is missing — --target was not honoured`);
        assert.ok(statSync(out).size > 0, `${out} is empty — --target compilation produced no output`);
    });
    ...
    it('honours --sourcedir to locate resource files in a separate directory', (t) => {
        ...
        execFileSync(
            'npx',
            ['gjsify', 'gresource', 'xml-only/app.gresource.xml', '--sourcedir', srcDir, '--target', out],
            {
                cwd: projectDir,
                stdio: 'pipe',
                timeout: 60 * 1000,
        ...
        assert.ok(existsSync(out), `${out} is missing — --sourcedir compilation failed`);
        assert.ok(statSync(out).size > 0, `${out} is empty — --sourcedir compilation produced no output`);
    });
});
