...
/**
 * A real MUSL-LINKED prebuild — `libc.musl-x86_64.so.1` in DT_NEEDED, no glibc
 * soname under any name. The fourth state, and the one the rule shipped without:
 * `muslVerdictOfNeeded` read any libc soname it did not recognise as the glibc
 * loader as "glibc-linked, musl-loadability undetermined", so the first four
 * `-musl` packages to land were told the only defensible value for their `libc`
 * field was `["glibc"]` — the opposite of what the generator writes from their
 * token, and `main`'s `commit-prebuilds` went red on it.
 *
 * The suite's only fixture for a `-musl` directory was LIBC_AGNOSTIC (no libc
 * soname at all), which is why nothing here caught it: an agnostic library in a
 * `-musl` directory is legitimately unconstrained, and the case that is not was
 * untestable until a musl artifact existed in the tree to copy from.
 */
const MUSL_LINKED = {
    dir: realPrebuild('node', 'sab-native', 'linux-x64-musl'),
    files: ['libgjsifysabnative.so', 'GjsifySabNative-1.0.typelib', 'GjsifySabNative-1.0.gir'],
};
...
describe('prebuild-libc — the token must agree with what is in the directory', () => {
    ...
    it('accepts a `-musl` directory holding a MUSL-linked library, which is the point of it', () => {
        // What the token exists for. Before the `musl` verdict existed this failed with
        // "declares libc: [\"musl\"] … the only defensible package-level value here is
        // [\"glibc\"]", about a library whose DT_NEEDED is `libc.musl-x86_64.so.1` and
        // whose loadability on musl had been proven by a real gjs run inside alpine:3.24
        // in the job that built it.
        assert.deepEqual(
            libcFailures(
                libcPkg({
                    declared: ['linux-x64-musl'],
                    stage: { 'linux-x64-musl': MUSL_LINKED },
                    libc: ['musl'],
                }),
            ),
            [],
    ...
    it('REQUIRES `libc: ["musl"]` from a musl-linked package', () => {
        // The one tier besides `incompatible` where the ELF settles it in both
        // directions: no `libc.musl-*` file exists on a glibc host under any name, so
        // without the filter npm installs it where nothing can load — and a musl host
        // resolves the `-musl` token FIRST, so the directory shadows the default build
        // that might have loaded.
        const problems = libcFailures(
            libcPkg({ declared: ['linux-x64-musl'], stage: { 'linux-x64-musl': MUSL_LINKED } }),
        );
        assert.ok(
            problems.some(
                (p) => /every committed Linux target is musl-linked/.test(p) && /Add `"libc": \["musl"\]`/.test(p),
            ),
            problems.join('\n'),
        );
    });

    it('REJECTS `libc: ["glibc"]` on a musl-linked package', () => {
        const problems = libcFailures(
            libcPkg({ declared: ['linux-x64-musl'], stage: { 'linux-x64-musl': MUSL_LINKED }, libc: ['glibc'] }),
        );
        assert.ok(
            problems.some((p) => /Declare exactly `\["musl"\]`/.test(p)),
            problems.join('\n'),
        );
    });

    it('REJECTS a package mixing a musl-linked target with a glibc one', () => {
        // npm's `libc` is ONE package-level filter, so neither value is right: `["musl"]`
        // refuses the glibc half on glibc hosts and `["glibc"]` refuses the musl half on
        // musl hosts. Since ADR 0017 the suffix is part of the package NAME, so this can
        // only be a directory staged into the wrong package.
        const problems = libcFailures(
            libcPkg({
                declared: ['linux-x64', 'linux-x64-musl'],
                stage: { 'linux-x64': GLIBC_LINKED, 'linux-x64-musl': MUSL_LINKED },
                libc: ['musl'],
                glibcRequires: { 'linux-x64': '2.2.5' },
            }),
        );
        assert.ok(
            problems.some((p) => /mixes musl-linked target\(s\)/.test(p)),
            problems.join('\n'),
        );
    });

    it('reports NO glibc floor for a musl artifact, and no note asking for one', () => {
        // The measurement that held `main` red. `libgjsify_lightningcss.so`'s aarch64
        // musl build carries one `.gnu.version_r` group — `File: libgcc_s.so.1`, holding
        // `GLIBC_2.0` among the `GCC_*` names, because GCC's unwinder still labels
        // `__register_frame_info` with the version glibc gave it before the symbol moved.
        // Attributed to glibc, that became `gjsify.glibcRequires: {"linux-arm64-musl":
        // "2.0"}` in a GENERATED manifest, which made the exemption clearer refuse to
        // clear an exemption whose artifact had landed.
        const { failures, notes, stats } = auditPrebuildLibc([
            libcPkg({
                declared: ['linux-arm64-musl'],
                stage: {
                    'linux-arm64-musl': {
                        dir: realPrebuild('infra', 'lightningcss-native', 'linux-arm64-musl'),
                        files: [
        ...
        assert.deepEqual(failures, []);
        assert.equal(stats.floorsReported, 0);
        assert.equal(stats.muslTargets, 1);
        assert.deepEqual(notes, []);
    });

    it('names the DECLARED value in the undetermined note, not a hardcoded one', () => {
        // The note read `declares libc: ["glibc"]` from a literal while the failure beside
        // it interpolated the real value, so one audit run said two contradictory things
        // about the same package — measured on the four `-musl` packages, which declare
        // `["musl"]`.
        ...
        assert.ok(
            notes.some((n) => /declares `libc: \["glibc","musl"\]` while the ELF alone cannot prove it/.test(n)),
            notes.join('\n'),
        );
    });
