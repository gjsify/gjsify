...
describe('CLI-only E2E (no user polyfill deps)', { timeout: 10 * 60 * 1000 }, () => {
    ...
    it('builds a script importing node:events (alias resolves via CLI deps)', () => {
        writeFileSync(
            join(projectDir, 'src', 'with-events.ts'),
    ...
    it('gjsify system-check --json skips gwebgl when project does not use @gjsify/webgl', () => {
        ...
        assert.strictEqual(gwebgl, undefined, 'gwebgl should be skipped when project does not depend on @gjsify/webgl');
    });
    ...
    it('gjsify system-check --json resolves gwebgl from project deps (primary path)', () => {
        ...
        assert.strictEqual(gwebgl.found, true, 'gwebgl should be found from project node_modules (primary path)');
    });
    ...
    it("gjsify showcase <name> pins the dlx spec to the CLI's own version", () => {
        const cliPkgJson = execFileSync('node', ['-p', 'JSON.stringify(require("@gjsify/cli/package.json"))'], {
            cwd: projectDir,
            encoding: 'utf-8',
        });
        ...
        assert.ok(combined.includes(expected), `Expected "${expected}" in showcase output:\n${combined}`);
    });

    // -- PR #18: gjsify build --shebang -------------------------------------
    it('gjsify build --shebang prepends GJS shebang and sets +x', () => {
        writeFileSync(join(projectDir, 'src', 'shebang-app.ts'), "console.log('hello from shebanged app');\n");
        execFileSync(
            'npx',
            ['gjsify', 'build', 'src/shebang-app.ts', '--app', 'gjs', '--shebang', '--outfile', 'dist/shebang-app.mjs'],
            {
                cwd: projectDir,
                stdio: 'pipe',
                timeout: 60 * 1000,
    ...
    it('gjsify build --shebang is idempotent on repeated builds', () => {
        ...
        writeFileSync(join(projectDir, 'src', 'shebang-idem.ts'), "console.log('idempotent');\n");
        const args = [
            'gjsify',
            'build',
            'src/shebang-idem.ts',
