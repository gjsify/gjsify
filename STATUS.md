...
> **Current state (v0.4.0-pre, 2026-05-08):** `cssAsStringPlugin` now resolves `@import` and lowers nesting/modern syntax via lightningcss `bundleAsync` — the `--app gjs` orchestrator passes `targets: { firefox: 60 << 16 }` so the output stays in the GTK4 CSS subset. New e2e suite `css-bundling/` (1 test). Removes the manual lightningcss preprocess step app-gnome had carried over from its old `vite.config.js`.
>
...
- **v0.3.16** — `@gjsify/module` PnP-aware `createRequire`. Parses `.pnp.cjs`'s `RAW_RUNTIME_STATE` directly (no Yarn-manifest execution), so `require.resolve("<pkg>/package.json")` works in PnP-built bundles for the first time. Removed the last reason ts-for-gir had to embed its EJS templates at build time.
...
| Build tools | 9 (infra/) |
| E2E suites | 16 (create-app, cli-only, cli-only-pnp, standalone-plugin, native-install, cli-config, inline-static-reads, library-multi-build, terminal-native, gsettings, define-from-pkg, text-loader, css-bundling, plugins-by-name, shebang-string, flatpak) |
...
Once v0.3.6 is on npm and ts-for-gir PR #378 bumps to it, the granular `@gjsify/*` devDeps + `nodeLinker: node-modules` + `packageExtensions` workarounds in `ts-for-gir/.yarnrc.yml` and `ts-for-gir/packages/cli/package.json` can finally be deleted — that was always the goal of Phase B. (Status as of v0.3.16: this is now done — see [gjsify/ts-for-gir#378](https://github.com/gjsify/ts-for-gir/pull/378).)
...
### Completed (Phase B.2 — gjsify v0.3.16 — `@gjsify/module` PnP-aware `createRequire`)
