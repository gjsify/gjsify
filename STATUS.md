...
| cli.spec.ts (Node bundle, run from Node) | ✅ (8) | (skip) | Spawns `node dist/cli.node.mjs <args>`: `--version` returns the `--define`-injected `4.0.0-rc.8`; `--help` lists every command from yargs's command tree; yargs `.strict()` rejects unknown commands; `list --help` + `list -g <dir>` via `glob` + colorette; Phase 6: `json --help` + `doc --help` (TypeDoc commands now fully bundled on GJS, stubs removed); Phase 9: `list --configName` loads an ESM rc through cosmiconfig's dynamic-import loader (regression coverage for [gjsify/ts-for-gir#385](https://github.com/gjsify/ts-for-gir/pull/385)) |
| cli.spec.ts (GJS bundle, run from Node) | ✅ (7, 1 skip) | ✅ (7, 1 skip) | Same 7 active assertions, spawned as `gjs -m dist/cli.gjs.mjs <args>` with the prebuild dirs on `LD_LIBRARY_PATH`/`GI_TYPELIB_PATH`. Validates the TypeDoc-unbundled GJS bundle end-to-end on SpiderMonkey 128. The Phase 9 `list --configName` test is gated on [gjsify/ts-for-gir#385](https://github.com/gjsify/ts-for-gir/pull/385) — cosmiconfig hands the ESM `.js` loader an absolute path, which GJS rejects per spec; remove `it.skip` once a fixed `@ts-for-gir/cli` rc lands. |
| cli.spec.ts (GJS bundle, run from GJS) | (skip) | ✅ (7, 1 skip) | Same 7 active assertions, spawned as `gjs -m dist/cli.gjs.mjs <args>` from a GJS parent. Phase 5: `ensureMainLoop()` in `@gjsify/child_process.spawn()`. Phase 6: TypeDoc commands (`json --help`, `doc --help`) verified. Phase 9 `list --configName` skipped on the GJS bundle until [gjsify/ts-for-gir#385](https://github.com/gjsify/ts-for-gir/pull/385) is consumed. |
...
### Medium priority — un-skip `cli.spec.ts` `list --configName` GJS-bundle assertion

Surfaced by user-reported reproduction inside the ts-for-gir submodule: the `@gjsify/cli install ts-for-gir` GJS bundle could not load any rc file — `ts-for-gir list --configName='.ts-for-gir.packages-all.rc.js'` printed `Module not found: <abs-path>` and exited. Root cause is in `@ts-for-gir/cli`: cosmiconfig hands its ESM `.js` loader an absolute filesystem path; Node tolerates `import('/abs/path.js')` as a non-spec extension while GJS/SpiderMonkey rejects it per ESM spec. Upstream fix: [gjsify/ts-for-gir#385](https://github.com/gjsify/ts-for-gir/pull/385) wraps the path with `pathToFileURL`. The Phase 9 regression test `list --configName loads an ESM rc via cosmiconfig (file:// dynamic import)` in `tests/integration/ts-for-gir/src/cli.spec.ts` exercises this end-to-end on both the Node and GJS bundles. The GJS-bundle path is currently gated via `it.skip` because `@ts-for-gir/cli@4.0.0-rc.9` (the version pinned in this workspace) still has the bug. **Action when upstream rc.13 (or the next published rc with the fix) lands:** bump `@ts-for-gir/cli` in `tests/integration/ts-for-gir/package.json`, remove the `bundle.label === 'GJS bundle' ? it.skip : it` gate, and update the Phase 9 row in this file to drop the skip note.

The same gap also surfaced an unrelated symptom worth verifying after un-skip: `start.ts`'s `process.exit(1)` after the `loadConfigFile` catch path returns exit code `0` from the GJS bundle even though `process.stderr.write(...)` wrote the error. Stdout-content assertions in the test catch the regression regardless, but if the exit-code mismatch persists once the upstream fix lands, file a follow-up against `@gjsify/process` / GLib-MainLoop teardown.
