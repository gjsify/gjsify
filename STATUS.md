...
> Last updated: 2026-04-30 (ts-for-gir Phase 4b cleanup — `@gjsify/process.exit()` is now async-safe on GJS: the syscall is scheduled via `GLib.idle_add(PRIORITY_DEFAULT)` from a fresh main-loop iteration with the parked loop drained inside the idle handler, and `ensureMainLoop()` is invoked so the source actually dispatches when no other component had registered a setMainLoopHook. This lets ts-for-gir's `cli.entry.ts` (and any consumer calling `process.exit()` from a microtask continuation) drop their inline `GLib.MainLoop` bootstrap + `idle_add`/`system.exit` shutdown helper. Earlier today: Phase 4b landed — non-interactive `@ts-for-gir/cli` runs on GJS through `dist/cli.gjs.mjs` built via a stub-alias map for `typedoc`, `prettier`, `@inquirer/prompts`, `inquirer`, `@ts-for-gir/generator-html-doc`, `@ts-for-gir/generator-json` (heavy deps that can't be bundled for SpiderMonkey 128 today). cli.spec.ts spawns BOTH bundles from Node (10 tests). Total ts-for-gir suite: Node 229/229, GJS 169/169. `@gjsify/process` unit tests: 145/145.)
