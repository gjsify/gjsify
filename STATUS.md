...
Current baseline excludes cases `9.*` (performance — ~30 min/run), `12.*` and `13.*` (permessage-deflate — separate validation axis). Also:

- `6.4.x` NON-STRICT fragmented-text-with-invalid-UTF-8 cases close with `1007` but not "fast enough" by Autobahn's yardstick — Soup buffers the whole message before UTF-8 validation, so fast-fail needs changes in libsoup.
...
Plan: (1) enable `12.*`/`13.*` once `permessage-deflate` is implemented (via libsoup extension or userspace DEFLATE); (2) investigate libsoup patch or userspace fast-fail path for `6.4.x`; (3) wire autobahn scripts into a nightly CI job once Podman-in-CI is unblocked.
