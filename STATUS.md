...
| **GJS Boxed-Source GC race for chunked responses to non-GJS HTTP clients.** A single `fetch()` from a Node.js process to a `@gjsify/http` server that responds with chunked `text/event-stream` causes a SIGSEGV ~10–13 s later (`gjs exited with code null`, no JS traceback). Backtrace: `BoxedBase::finalize → g_source_unref` with `assertion 'old_ref > 0' failed`. Mechanism: a libsoup-internal `GLib.Source` wrapper (likely from chunked write-side IO bookkeeping) is exposed to JS without the wrapper holding its own ref. After Soup finishes the response GLib frees the source (refcount 1→0); the JS wrapper survives until GJS's deferred-GC heuristic fires it (`g_timeout_add_seconds(G_PRIORITY_LOW, 10, trigger_gc_if_needed)` in `refs/gjs/gjs/context.cpp:873-906`); the finalizer's `g_source_unref` then decrements 0→-1 and crashes. Reproducer is *one* `node -e 'await fetch(url, {…SSE…})'` against a server that streams. Same sequence over an in-process MCP `client.callTool` does **not** crash, so the CI test suite passes — the trigger requires a non-GJS process talking to us. | @gjsify/http (Server chunked responses), @gjsify/example-node-net-mcp-server, anything serving SSE/chunked to external HTTP clients (MCP inspector subprocess, browser EventSource, curl piping, etc.) | None ships. **Mitigations attempted and rejected:** (a) eager `imports.system.gc()` after each `'finished'` signal — fixes the deferred-GC crash for one-shot requests but corrupts shared per-connection state when a long-poll GET sibling is still paused on the same keep-alive TCP, crashing differently and earlier (call 2 instead of call 4–5). (b) Same gated on `_inFlightCount === 0` — paused long-polls keep the count above zero indefinitely, so GC never runs. (c) Force-`Connection: close` on every response — does not change the crash window. (d) HUP/ERR watchdog via `Gio.Socket.condition_check` — POLLHUP only fires when both sides have closed, not on the typical client-side half-close. (e) Periodic `setTimeout` GC at long intervals — race-prone and easy to interfere with in-flight Soup IO. | **GJS GIR-bindings audit** — track down which boxed type this is (the Source wrapper is silently created somewhere in the libsoup → GJS marshalling path) and either fix the transfer-mode annotation or block the wrapper creation. Likely culprit is a chunked-write-related `GLib.Source` returned with mismatched ownership semantics in a Soup public API. **OR libsoup patch** (same as the long-poll patch above) — moving Soup's chunked-write internals away from JS-visible Sources would also fix this. |
