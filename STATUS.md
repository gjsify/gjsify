...
> Last updated: 2026-05-01 (ts-for-gir bumped to `4.0.0-rc.8`. The new release lifted prettier post-processing into the templates/string-builders, so `tests/integration/ts-for-gir/src/stubs/prettier.ts` was removed and the `prettier` alias dropped from `--alias`/`--external` in the CLI build pipeline. `@ts-for-gir/cli@4.0.0-rc.8` ships the previous gjsify cleanup unmodified — startup is plain `await yargs(...).parseAsync()` + `process.exit(N)` against the async-safe `@gjsify/process` polyfill from PR #58. `@modelcontextprotocol/sdk@1.29.0`'s zod-compat shim breaks under `zod@4.4.1` (its `AnySchema = z3.ZodTypeAny | z4.$ZodType` doesn't structurally accept the new `ZodString` / `ZodObject` shapes), pinned back to `zod@4.3.6` until upstream catches up. Total ts-for-gir suite: Node 229/229, GJS 169/169. `@gjsify/process` unit tests: 145/145.)
