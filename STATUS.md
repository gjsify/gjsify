...
| Build tools | 9 (infra/) |
| E2E suites | 14 (create-app, cli-only, cli-only-pnp, standalone-plugin, native-install, cli-config, inline-static-reads, library-multi-build, terminal-native, gsettings, define-from-pkg, text-loader, plugins-by-name, shebang-string) |
...
| cli.spec.ts (GJS bundle, run from Node) | ✅ (8) | ✅ (8) | Same 8 assertions, spawned as `gjs -m dist/cli.gjs.mjs <args>` with the prebuild dirs on `LD_LIBRARY_PATH`/`GI_TYPELIB_PATH`. Validates the TypeDoc-unbundled GJS bundle end-to-end on SpiderMonkey 128. Phase 9 `list --configName` exercises cosmiconfig's ESM `.js` loader through the GJS-spec `file://` dynamic-import path. |
| cli.spec.ts (GJS bundle, run from GJS) | (skip) | ✅ (8) | Same 8 assertions, spawned as `gjs -m dist/cli.gjs.mjs <args>` from a GJS parent. Phase 5: `ensureMainLoop()` in `@gjsify/child_process.spawn()`. Phase 6: TypeDoc commands (`json --help`, `doc --help`) verified. Phase 9: `list --configName` rc loading via cosmiconfig + `pathToFileURL` (gjsify/ts-for-gir#385). |
...
Phase B.3 (PR1) shipped the bundler-side primitives that the flatpak helpers will lean on (`gsettings` subcommand, env- and package.json-driven defines, text loaders, plugin-by-name resolution, env-templated shebang). PR2 adds four `gjsify flatpak <sub>` subcommands consolidating the GJS-app + CLI-tool flatpak workflow that's currently spread across hand-written manifests + shell scripts:

- `flatpak init` — generates `<app-id>.json` from `package.json` plus a new `gjsify.flatpak` config namespace (`appId`, `runtime: "gnome" | "freedesktop"`, `runtimeVersion`, `sdkExtensions`, `command`, `finishArgs`, `permissions`, `extraModules`). Flag `--cli-only` strips GUI-shaped finish-args (`--socket=wayland`, `--device=dri`) but keeps `org.gnome.Platform` as runtime — GJS bundles need GLib/GIO/GObject at runtime, which Freedesktop doesn't ship (see memory `project_flatpak_runtime_choice.md`).
- `flatpak build` — wraps `flatpak-builder` with sensible defaults (`--force-clean`, `--sandbox`, `--delete-build-dirs`); pass-through for power-user flags. Drops in for `easy6502/build-flatpak.sh`. Flag `--tarball` for parity with the existing build-flatpak.sh tarball step.
...
- `flatpak ci` — scaffolds `.github/workflows/flatpak.yml` matching the upstream `flatpak/flatpak-github-actions/flatpak-builder@v6` shape (`ghcr.io/flathub-infra/flatpak-github-actions:gnome-50` container, runtime-image derived from manifest's runtime-version). Idempotent without `--force`.
...
In-flight PR1 of the `gjsify-as-Flatpak-toolchain` line of work (see "Open TODOs → Flatpak helper subcommands"). Adds the bundler-side primitives that `@learn6502/app-gnome` (Vite → `gjsify build` migration) and `@ts-for-gir/cli` (drop the `scripts/build-gjs.mjs` wrapper) both need. No flatpak-specific code yet — that lands in PR2.
