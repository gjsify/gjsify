...
> Last updated: 2026-04-30 (ts-for-gir Phase 4b landed — non-interactive `@ts-for-gir/cli` now also runs on GJS: `dist/cli.gjs.mjs` is built via a stub-alias map for `typedoc`, `prettier`, `@inquirer/prompts`, `inquirer`, `@ts-for-gir/generator-html-doc`, `@ts-for-gir/generator-json` (heavy deps that can't be bundled for SpiderMonkey 128 today). The same 5 cli.spec.ts assertions now exercise BOTH bundles (Node spawns gjs as a subprocess to validate the GJS bundle). The cli.entry.ts uses `imports.system.exit` from a `GLib.idle_add` callback to terminate cleanly without deadlocking on the parked GLib MainLoop, and yargs's `.exitProcess(false)` keeps yargs's own `process.exit` from blocking inside `parseAsync`. Total ts-for-gir suite: Node 229/229, GJS 169/169. Earlier today: Phase 4a — Node-only CLI bundle + 3 root-cause fixes (`util.styleText`/`stripVTControlCharacters`, per-file `__filename`/`__dirname` injection, `--define`/`--external`/`--alias` flags).)
...
| cli.spec.ts (Node bundle, run from Node) | ✅ (5) | (skip) | Spawns `node dist/cli.node.mjs <args>`: `--version` returns the `--define`-injected `4.0.0-rc.6`; `--help` lists every command from yargs's command tree; yargs `.strict()` rejects unknown commands; `list --help` prints the per-command flags from cosmiconfig + the option builder; `list -g <dir>` walks our local fixtures via `glob` and renders them through colorette |
| cli.spec.ts (GJS bundle, run from Node) | ✅ (5) | (skip) | Same 5 assertions, spawned as `gjs -m dist/cli.gjs.mjs <args>` with the prebuild dirs on `LD_LIBRARY_PATH`/`GI_TYPELIB_PATH`. Validates the stub-aliased GJS bundle end-to-end on SpiderMonkey 128. Skipped on the GJS test runtime because `@gjsify/child_process` (Gio.Subprocess) currently hangs when the parent is also `gjs` and waits on the child's `close`/`exit` events — both bundles are exercised from Node so re-running them from GJS would be a no-op |

Fixtures (`tests/integration/ts-for-gir/girs/`) are gjsify's own Vala-generated GIRs — committed alongside the suite (no prebuild copy step), real-world parser surface, no upstream-fixture coupling. Phase 3 additionally writes a minimal synthetic GIR to `tmpdir()` at test-module load time so `DependencyManager` can resolve it via `glob` on the real filesystem. Phase 4a bundles the upstream CLI's `start.ts` workflow into `dist/cli.node.mjs` via a small `src/cli.entry.ts` shim that mirrors the upstream wiring (the published package's `exports` map only exposes `.`, so we re-construct the yargs setup against the named exports). The Node bundle is built with `--define '__TS_FOR_GIR_VERSION__="4.0.0-rc.6"' --external typedoc,prettier,@inquirer/prompts,inquirer` — all four are runtime-only deps that Node resolves from `node_modules`. Phase 4b adds `dist/cli.gjs.mjs`: same `cli.entry.ts` plus a per-test `src/stubs/` directory wired in via `--alias` (`typedoc`, `prettier`, `@inquirer/prompts`, `inquirer`, `@ts-for-gir/generator-html-doc`, `@ts-for-gir/generator-json` → small in-suite stub modules; commands that never execute these still work, others throw a clear stub error). The GJS bundle bootstraps an idempotent `GLib.MainLoop` so async `fs/promises` handlers complete, sets yargs's `.exitProcess(false)` so yargs's own `process.exit` does not deadlock the loop, and routes its own shutdown through `GLib.idle_add` + `imports.system.exit` so the syscall fires from a fresh main-loop iteration.
...
- ✅ **Phase 4b:** Non-interactive CLI on GJS — same `cli.entry.ts`, plus per-test stubs for the bundle-hostile deps (typedoc, prettier, @inquirer/*, generator-{html-doc,json}). The GJS bundle bootstraps a GLib MainLoop and uses yargs `.exitProcess(false)` + `GLib.idle_add` + `imports.system.exit` to terminate cleanly. cli.spec.ts now spawns BOTH bundles from Node (10 tests total).

Remaining phases:

- **Phase 5 — `@gjsify/child_process` GJS-from-GJS subprocess deadlock.** When the parent process is `gjs` and uses `@gjsify/child_process.spawn` to launch another `gjs`, neither `close` nor `exit` fires on the parent side; the parent hangs forever. The Phase 4b cli.spec.ts works around this by always spawning from Node (which validates the GJS bundle end-to-end). Real fix: investigate `Gio.Subprocess.communicate_async`'s interaction with the parent's main loop when the child is itself a GJS process.
- **Phase 6 — `gjsify run` runtime npm-package resolution.** Without it, `--external` cannot be used on GJS bundles (esbuild leaves bare `import 'pkg'` literally; GJS has no node_modules resolver). A small Node-compat resolver hooked into GJS's module loader would let us drop the per-test stub maps and use real upstream `typedoc`/`prettier` at runtime.
- **Phase 7 — CLI tarball end-to-end** — `npm install @ts-for-gir/cli` and exec the published `bin/ts-for-gir` directly. Validates the published distribution shape rather than our bundle. Already works on Node today (the tarball ships a pre-bundled binary); on GJS, blocked on Phase 6.
...
- **External-deps regex assertion port** (`refs/ts-for-gir/tests/external-deps/assert.mjs`) — meaningful only after Phase 7 lands.
