...
> Last updated: 2026-04-29 (`@gjsify/fs` adds `watchFile`/`unwatchFile` (StatWatcher polling), `statfsSync`/`statfs`/`promises.statfs` (Gio filesystem info), 13 new tests, 610 total; previously: `promises.watch()` async iterator, `globSync`/`glob`/`promises.glob`, `cp`/`Dir`/`opendir`.)
