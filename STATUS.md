...
| `soup_server_message_pause()` destroys the input-polling `GSource` (`refs/libsoup/libsoup/server/http1/soup-server-message-io-http1.c:32, 80–84, 1005–1010`), so libsoup never observes a peer-side TCP close while a `SoupServerMessage` is paused. The `'disconnected'` signal therefore does not fire for long-poll / SSE clients that hang up mid-stream. Compounded by GJS not exposing a non-destructive `recv(MSG_PEEK)` (`Gio.Socket.receive_message` is non-introspectable for `(out caller-allocates)` buffers) and `condition_check(HUP\|ERR)` only firing when both sides are torn down on Linux, so JS code cannot detect peer-close on its own. | @gjsify/http (Server long-poll/SSE responses), @gjsify/example-node-net-mcp-server (the canonical reproducer — MCP inspector CLI exits without DELETE, leaving its GET notifications stream paused on the server side; ~5 of those crashes the GJS process) | `SoupMessageLifecycle` opts GET requests out of the JS-side GC guard so SpiderMonkey GC can at least eventually collect the JS wrapper. The real long-poll leak is unaddressed; documented as a known limitation. The `mcp-inspector-cli` integration test caps its sequential-call loop at 3 to stay below the crash threshold. | **libsoup patch** — either (a) expose `soup_server_message_set_idle_timeout(msg, ms)` so the application can opt a paused message into "drop after N ms of silence", or (b) keep a lightweight HUP/RDHUP poll on the underlying socket even while paused, so `'disconnected'` fires for long-poll clients. **OR a GJS bindings patch** to make `g_socket_receive_message` introspectable (`(out caller-allocates)` for `gint8[]` buffers), which would let us implement the watchdog ourselves with a non-destructive 1-byte `MSG_PEEK` probe. |
| **GJS Boxed-Source / MainContext GC race for chunked responses to non-GJS HTTP clients.** A single `fetch()` from a Node.js process to a `@gjsify/http` server that responds with chunked `text/event-stream` causes a SIGSEGV ~10–13 s later (`gjs exited with code null`, no JS traceback by default). Two related assertions surface depending on which side hits zero first: `g_source_unref_internal: assertion 'old_ref > 0' failed` (a libsoup IO source wrapper) **or** `g_main_context_unref: assertion 'g_atomic_int_get (&context->ref_count) > 0' failed` (the thread-default `GMainContext` shared between libsoup's `SoupMessageIOHTTP1.async_context` (`refs/libsoup/libsoup/server/http1/soup-server-message-io-http1.c:70,87`) and GJS's own `MainLoop::spin` (`refs/gjs/gjs/mainloop.cpp:28-29`)). Both libsoup C-side ref/unref pairs are clean; the imbalance comes from the GJS binding layer, where some Boxed wrapper is keeping a reference past the underlying object's lifetime. The GJS deferred-GC heuristic (`g_timeout_add_seconds(10, trigger_gc_if_needed)` in `refs/gjs/gjs/context.cpp:873-906`) then runs the JS finalizer, which decrements an already-zero refcount and crashes. Reproducer is *one* `node -e 'await fetch(url, {…SSE…})'` against a server that streams. Same sequence over an in-process MCP `client.callTool` does **not** crash, so the CI test suite passes — the trigger requires a non-GJS HTTP client talking to us. | @gjsify/http (Server chunked responses), @gjsify/example-node-net-mcp-server, anything serving SSE/chunked to external HTTP clients (MCP inspector subprocess, browser EventSource, curl piping, etc.) | **Diagnostic helper:** `installCriticalLogWriter()` in `@gjsify/utils/log-writer.ts` (called from `ensureMainLoop`) prints a one-time advisory at startup recommending `G_DEBUG=fatal-criticals` so the user gets a SIGABRT with backtrace + coredump instead of a silent SIGSEGV. (A JS log writer cannot capture the assertion itself: GJS blocks `GLogWriterFunc()` from running during a SpiderMonkey GC sweep — `Gjs-CRITICAL: Attempting to run a JS callback during garbage collection`.) **Mitigations attempted and rejected:** (a) eager `imports.system.gc()` after each `'finished'` signal — fixes the deferred-GC crash for one-shot requests but corrupts shared per-connection state when a long-poll GET sibling is still paused on the same keep-alive TCP, crashing differently and earlier. (b) Same gated on `_inFlightCount === 0` — paused long-polls keep the count above zero indefinitely, so GC never runs. (c) Force-`Connection: close` on every response — does not change the crash window. (d) HUP/ERR watchdog via `Gio.Socket.condition_check` — Linux POLLHUP only fires on bilateral close, not the typical client-side half-close. (e) `Gio.Socket.receive_message(MSG_PEEK)` non-destructive probe — not introspectable from JS, the `(out caller-allocates)` annotation for `gint8[]` buffers is not bound. (f) Custom `g_log_set_writer_func` — blocked by GJS at GC re-entry. (g) `g_log_writer_default_set_use_stderr(true)` — refuses with `g_thread_n_created()==0 failed` once GJS has spun up its worker threads. | **GJS GIR-bindings audit** — track down which boxed type the leaking wrapper belongs to (likely chunked-write-related, sharing the thread-default `GMainContext` with `SoupMessageIOHTTP1.async_context`) and fix the transfer-mode annotation in either Soup's `.gir` or a metadata override, so the wrapper either holds its own ref or doesn't get exposed at all. **OR libsoup patch** moving chunked-write internals away from JS-visible Sources / sharing patterns. |
