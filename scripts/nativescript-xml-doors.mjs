...
/**
 * The in-repo, TYPELIB-READ enum values — `generated/enum-values.mts`.
 *
 * The second independent side arm 7 has and arm 5 does not: `props.ts` carries the SPELLING
 * and the ORDER of every nick and no numbers, so a table deriving constants from positions
 * can only be held against something that read the library. This file is committed,
 * generated by `scripts/generate-enum-values.mjs` from the installed typelib, and reachable
 * from a `checkout` + `setup-node` job with no `@girs` install — which is the whole job a
 * committed artifact exists to do. Reading it from here is what `gtk-align.ts`'s header names
 * as the thing that would retire ITS "no in-repo oracle can check this" caveat.
 */
export const GTK_HOST_ENUM_VALUES = 'packages/framework/gtk-host/src/generated/enum-values.mts';

/**
 * `<enum GType>.<nick>` -> value, for one enum, read out of `enum-values.mts`.
 *
 * Its own reader rather than `readRecordLiteral`: that one's key pattern is `[\w-]+`, and
 * every key here carries a `.`. Returns an empty Map when the enum is absent, which the
 * caller must treat as a failure — an enum nothing declares would otherwise hold a table
 * against nothing.
 */
export function readEnumValues(text, gtype) {
    const out = new Map();
    const pattern = new RegExp(`^\\s*'${gtype}\\.([\\w-]+)':\\s*(-?\\d+),`, 'gm');
    for (const [, nick, value] of text.matchAll(pattern)) out.set(nick, Number(value));
    return out;
}
