...
export const E2E_UNLISTED_SUITES = {
    ...
    'react-native-devtools': [
        ...
        'wrong thing. IT RUNS INSTEAD in main.yml’s `e2e` job (shard 1), under `xvfb-run` with',
        'the env the `examples` job puts around the showcase smoke launch — and its own',
        'per-vector `dbus-run-session` rather than one around the step — with',
        '`GJSIFY_E2E_REQUIRE=react-native-devtools` — so a container missing any one of its',
        'preconditions FAILS naming the one it lacks instead of going quiet (#1550). The `e2e`',
        'job is the right host rather than `examples` because of the TRIGGER: `examples` is gated',
        'on `@gjsify/example-*` being in the closure, so a change confined to',
        '`packages/framework/react-native/**` would skip it — the suite would be silent for',
        'exactly the PRs it exists to catch. A SCRIPT_COUPLINGS entry in `affected-classify.ts`',
        'turns the e2e tier on for that path, the same shape `.githooks/**` already needed.',
        'What this buys is real: `runApplication` holds THREE lines no unit test can reach, each',
        'observable only by running the loop — `registerBuiltinWidgets()`, the option passthrough',
        'and `provideWindowChrome()`. The last is measured: deleting it plus its now-dead import',
        'leaves oxfmt, oxlint, tsc and all `@gjsify/react-native` assertions green — #1540’s ten',
        'window-chrome vectors included, because router.spec.ts composes the shell itself — while',
        'the shipping application draws two header bars. Measured GREEN on a desktop session:',
        '3 vectors, 3 passing, 0 skipped, with the switch on.',
    ].join(' '),
    'terminal-native': [
