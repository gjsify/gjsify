...
export const E2E_UNLISTED_SUITES = {
    ...
    'react-native-devtools': [
        ...
        'a settled exemption, and it costs real coverage: `registerBuiltinWidgets()` inside',
        '`runApplication` has no other vector, because no unit test can observe a call that is only',
        'reachable by running the loop. Tracked in status/open-todos.md beside `devtools-export`,',
        'which needs the same kind of environment for a different and still-open reason. Measured',
        'GREEN on a desktop session: 2 vectors, 2 passing.',
    ].join(' '),
    'terminal-native': [
