...
const KNOWN_GAPS = {
    ...
    'gtk-button': {
        ...
        why: "Three of the five gaps this entry used to carry are gone, and the reason they were here is the reason they closed: the class extended the REAL NativeScript `Button`, which hosts no child view, so `label` was NS's inherited `text` — \"a divergence no gate sees because `text` is inherited rather than declared here\" — and `icon-name` had no slot to reach at all. It is a tappable `GridLayout` with one cell now, so `label`, `iconName` and `useUnderline` are declared properties and `child` is the slot the other two replace (gtk-button.ts). What is left is the GTK chrome NativeScript has no counterpart for: `has-frame` draws the button's frame or not, and this port's frame is the `.adw-button` fill a style class replaces; `can-shrink` is the size-negotiation property no `GridLayout` has a minimum/natural protocol behind, the same answer `adw-split-button` and `adw-inline-view-switcher` give. `<gtk-button>` declares four names on the web surface.",
    },
    'gtk-drop-down': {
    ...
    'gtk-image': {
        gaps: ['file', 'resource', 'useFallback'],
        why: 'The port renders ONE source: an Adwaita symbolic SVG resolved by name through `renderSymbolicIcon` (gtk-image.ts:14-17), so `file`, `resource` and the icon-theme `use-fallback` name loading paths it does not have. `pixel-size` was the fourth entry here, and it is the one this ledger is worth re-reading for: the property was not MISSING, it was present under the wrong name. The widget answered to `iconSize` as "the icon size in DIPs" where `Gtk.Image:icon-size` is a `GtkIconSize` ENUM, so the port carried GTK\'s `pixel-size` under GTK\'s `icon-size` name — a false friend no name comparison can see, counted as agreement by this gate and by `check-vocabulary-alignment` alike. #1584 split the two apart: `pixelSize` is the number and `iconSize` is the three-member enum, so the gap closed by being NAMED rather than by being added. `<gtk-image>` declares five on the web surface, `icon-size` among them.',
    },
    'gtk-menu-button': {
