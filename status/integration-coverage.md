...
`Scope` that must close on the way out. **Node: 64/64 green. GJS: 85/85 green, 0 skips.** No `@gjsify/*` change was
required — Effect 4 loads and runs unmodified under `gjsify build --app gjs`, which was the open
question the suite was written to answer. The GJS leg runs more cases than the Node one because the
FileSystem conformance suite runs TWICE there, over `node:fs` and over `@gjsify/effect-platform`'s
Gio layer (ADR 0050); the Node entry deliberately does not import the Gio leg, which would put GIO
in an `--app node` bundle.
...
**Test defects found by the NODE leg**, which is the leg that says the test is wrong rather than the
implementation, all recorded in the specs: `Stream.toPull` is scoped and needed an explicit
`Effect.scoped` the upstream harness supplies per case, and `ConfigProvider.fromEnv` reads `_` as
its PATH SEPARATOR, so the root record lists first segments (`GJSIFY`) and not variable names
(`GJSIFY_EFFECT_CONFIG_PROBE`).

**Three defects the GIO leg found in `@gjsify/effect-platform`** on its first run, each fixed in the
layer rather than declared as a gap: `g_file_replace*` is atomic-replace and so cannot back POSIX
`w`/`w+` (a mid-stream read-back saw an empty file); a `GFileOutputStream` buffers where a
descriptor does not, so every write now flushes; and `writeFile`, built out of `open` + `writeAll`,
reported the inner method's name where the contract asks for its own.

**The application-side counterpart is `showcases/gtk/effect-adw-services`**, which asks what this
suite cannot: does an interrupted fiber actually cancel a `Gio.Cancellable`, does a GObject lifetime
close an Effect `Scope`, and does a boolean-returning GTK signal still get its answer on time.

**Cost, measured rather than assumed.** Importing Effect and running one effect adds 25 KB to a GJS
bundle and 2 ms to cold start (12 runs, 45 ms → 47 ms). Tree-shaking over Effect 4 works.
...
5 test files ported from socket.io v4 upstream. **Node: 70/70 green. GJS: 70/70 green, 0 skips** (112 assertions per runtime; the "112/112" this section used to carry counted assertions, not tests). Full transport coverage: polling, polling→WebSocket upgrade, and WebSocket-only. handshake (4 — CORS, allowRequest), socket-middleware (2), socket-timeout (4), socket (32 — emit/acks, onAny/offAny/prependAny, volatile, compression, disconnect, reserved-event guards), namespaces (28 — multi-namespace, `except()`, dynamic namespaces). Root-cause fixes surfaced (landed): `@gjsify/fetch` POST body never sent (raw-body attach via `set_request_body_from_bytes`); `IncomingMessage` wrongly emitted `'close'` after body end (breaking engine.io long-poll — `_autoClose` hook, close only via destroy per Node semantics); `EventEmitter.prototype` methods made enumerable (socket.io builds its namespace proxy from `Object.keys(EventEmitter.prototype)`); `req.socket` set on WebSocket upgrades; `--globals auto,WebSocket` for the alias-shaped `globalThisShim.WebSocket` access the detector cannot follow.

Two further causes, measured on Linux (`ghcr.io/gjsify/ci-fedora`) and macOS 27 arm64 before they were fixed:

- **The four `(ws)` volatile cases** (all platforms): `@gjsify/http` emitted `'upgrade'` with `req.socket === null`, so engine.io's WebSocket-only handshake threw on `req.connection.remoteAddress` inside an async function and the client never connected (`Expected: 1, Actual: 0`).
- **Five connect timeouts from ~30 tests in** (macOS; which five varied per run): `@gjsify/fetch` never closed response body streams, and libsoup 3 returns a connection to the pool only on close — reading to EOF is not enough. Every polling request pinned its connection until GC finalized the stream; after 64 (the shared session's `max-conns`) the next request queued forever. The Linux CI run did not show it — its GC evidently finalized the streams before the pool filled; macOS did not.
...
**HELD OUT of CI** (measured per-suite in `ghcr.io/gjsify/ci-fedora`): GJS — the basic verb cases failed with `me is not a function`, and the reading taken from it, that a mangled identifier at a call site indicts the bundle rather than the suite, was never established. Re-measured on a workstation 2026-09-03 against a freshly built `dist/test.gjs.mjs`: that string does not occur anywhere in the run. The verb cases fail with `fetch failed` and per-case timeouts, and the WebSocket cases with `setting getter-only property "protocol"`. In the bundle `me` is undici's own mangled binding for its CJS `assert` import, not the `keepNames` helper — `__name` is declared at byte 203944 and called once at byte 212982, in the correct order — so nothing connects this suite to the `keepNames` helper-order entry in `open-todos`. A workstation run does not disprove an image measurement; it does mean neither symptom should be treated as current until the suite is re-measured in the image.
