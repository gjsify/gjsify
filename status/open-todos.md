...
Residual, and the honest price of the fix: an object that SURVIVES `run_dispose` keeps a toggle ref this binding can no longer remove — the pointer is gone and glib offers no way to learn afterwards that the object lived — so it leaks, one object per `run_dispose`d-but-still-referenced wrapper. The old code leaked exactly the same ref; it just took a use-after-free with it. Discriminating on `ref_count` at notify time (2 under `run_dispose`, 1 under finalize — measured) was deliberately NOT done: it reads an implementation detail of `g_object_unref`'s last-ref ordering, and a rule that clever is one refactor away from being simplified back into the bug.
