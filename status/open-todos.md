...
- **musl.** Every CI image is Fedora/glibc. The measurement found the glibc-declared prebuild loads and runs on a pure-musl host anyway (Bun 1.3.14, 38/38 cross-runtime files — mechanism in AGENTS.md § cross-runtime), so this is not a missing TARGET, it is a missing GATE: nothing would notice if a future addon change started referencing a glibc-only symbol, because musl binds eagerly and the failure would be a clean load error nobody runs into. Cheapest shape is an Alpine container leg running the existing `test:bun` (Deno cannot participate — it publishes no musl build at all).
