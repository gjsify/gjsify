...
- **musl.** Every CI image is Fedora/glibc. The measurement found the glibc-declared prebuild loads and runs on a pure-musl host anyway (Bun 1.3.14, 38/38 cross-runtime files — mechanism in AGENTS.md § cross-runtime), so this is not a missing TARGET, it is a missing GATE. And the gate has to RUN the code, not just load it: measured on that host, an absent STRONG undefined symbol does fail `dlopen` at load, but an absent **WEAK** one loads fine, runs unrelated code in the same library, and only SIGSEGVs at the first call that needs it. That is not hypothetical — `@gjsify/sab-native`'s pre-#955 arm64 library imported `fcntl64@GLIBC_2.28`, a name musl does not export at all, and its first two probes (`SharedBuffer.create` + typed accessors + atomics; `FdChannel` SCM_RIGHTS) passed with exit 0 while the third (`from_fd`, the only caller of the CLOEXEC-dup helper that compiled to `fcntl64`) died at the call site with exit 139. **A load-only smoke gate would have passed that library twice.** So: an Alpine container leg running the existing `test:bun` (which exercises 38 files, not a load), and worth pairing with a cheap static check — `readelf -W --dyn-syms` over each committed prebuild, flagging any WEAK undefined symbol that is not one of the three NULL-guarded gcc hooks (`_ITM_{,de}registerTMCloneTable`, `__gmon_start__`), since that class is exactly what a runtime gate can still miss if no test happens to call the affected path. Deno cannot participate in a musl leg at all — it publishes no musl build.
