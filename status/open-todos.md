...
- **`PANGOCAIRO_BACKEND=fc`**, which selects a fontconfig-backed Pango on darwin and would make
  ...
  that the bundle was already making that choice by compiled-in ordering. What is NOT measured
  is the result on a real Mac: no macOS leg has run against it yet. Until one has, this stands.
...
**RE-MEASURED 2026-09-14, and the answer got sharper rather than changing.** ADR 0038
§ Amendment 3 has `@gjsify/node-gi`'s loader ask for the backend that WOULD read this
configuration (`PANGOCAIRO_BACKEND=fc`), and win32 refuses: CI run 34873488108, with the
variable set, still built a `PangoCairoWin32FontMap` — the same run resolved `Adwaita Sans`
under the DirectWrite spelling `Adwaita Sans Text`, which only that map produces — and Tamil
still counted 5 unknown glyphs per 5 characters. gvsbuild's pango has no FreeType/fontconfig
cairo backend to select. So `etc/fonts` is still read by nobody, #1668 is NOT fixed on Windows,
and the repair is upstream: a gvsbuild pango built with that backend.
`test/font-script-coverage.test.mjs` ASSERTS this gap rather than skipping it, so the day
gvsbuild ships one the suite says so and this entry goes with it.
