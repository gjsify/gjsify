...
### `@gjsify/iframe` cannot load on macOS, and no build leg would close it

The entry above ends "the pattern is proven — these two were simply never extended". This one is the case where that sentence does NOT apply, which is why it is filed apart from it: there is no upstream library to build a darwin target OF.

`gi://WebKit` version 6.0 has no macOS provider. Homebrew's `webkitgtk` (2.52.5) carries an unconditional `depends_on :linux` whose comment states the reason outright — *"Use JavaScriptCore.Framework on macOS"* — ships `arm64_linux`/`x86_64_linux` bottles only, and pulls `systemd`, `wayland`, `mesa`, `libdrm` and `libxkbcommon`; there is no systemd for macOS. It would not even satisfy the requirement if it installed: it builds `-DUSE_GTK4=OFF` and its own test links `webkit2gtk-4.1`, i.e. the GTK3 port, while this package needs the GTK4 one. MacPorts offers no alternative and no tap does either. The WebKit **GTK port** targets X11/Wayland; macOS gets the **Mac port** (`WebKit.framework`), which ships no GObject-Introspection typelib.

Deliberately NOT filed as work: nothing here is ours to fix, and no `gjsify.os` claim is added either — the package makes no OS decision in shipping source, so the `os-axis` rule does not demand one and declaring on suspicion would contradict ADR 0018's derivation rule. What a port would look like, and why the engine half is cheap while the widget half is a compositor, is [ADR 0022](../docs/adr/0022-webkit-on-darwin.md) — Proposed, with both measurements reproducible under `docs/poc/`. The entry exists so the typelib error is not rediscovered as a packaging bug.
