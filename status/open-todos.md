...
### `@gjsify/node-gi` — bun and deno cannot construct a GTK object on macOS

Measured across a cross-platform run of every command the homepage prints, on macOS 15.7.9 x86_64 (Homebrew GTK, node 24.18.1 / bun 1.3.14 / deno 2.9.x). Every GTK showcase dies at the first widget, and the two runtimes fail differently, so this is two defects sharing one column:

- **bun** — `node-gi: GtkWidget is not registered in this process's GObject type registry`, then `TypeError: Adw.Application has no property 'application-id'`. That first line is the signature of a failed bare-leaf `g_module_open('libgtk-4.1.dylib')`: the typelib resolves, no `gtk_*_get_type` is ever registered, and the constructor rejects its own property one layer up. **Plausibly already fixed and NOT yet re-measured**: the composed `DYLD_FALLBACK_LIBRARY_PATH` used to drop `/usr/local/lib` (see `composeDyldFallback` in `system-gi.js` / `system-gi.ts`), which is exactly where that dylib lives on Intel macOS, and `runRuntimeBundle` launches bun through the same `buildNativeEnv()`. Re-run `gjsify showcase three-geometry-teapot --runtime bun` on a Mac before assuming anything else is wrong. NB `maybeReexecForGtkRuntime` deliberately skips bun/deno, so the CLI-composed launch env is the ONLY loader repair those two get.
- **deno** — two copies of `libgio-2.0` in one process, Homebrew's and the one inside the gjsify prebuild (`objc: Class GNotificationCenterDelegate is implemented in both …`), then `GLib-GObject-CRITICAL: invalid class cast from (NULL) pointer to 'GObject'`. Independent of the loader-path question above and not addressed by it. Only the Canvas2D showcase survives on deno; everything reaching WebGL does not. The fix shape belongs with the `gtkSource()` policy (ADR 0023): the bundle and the host stack must not both be loaded, and nothing states today which one wins for a non-Node runtime.

### `@gjsify/node-gi` — deno on Windows resolves no `Gwebgl` typelib

Same run, win32-x64, driven as an interactive scheduled task (over SSH every GTK showcase dies in session 0 with a misleading `0xC0000005` — see `website/src/content/docs/platform-support.md`). Under deno every Three.js and Excalibur showcase fails with `Failed to require Gwebgl 0.1: Typelib file for namespace 'Gwebgl' (any version) not found`, while `canvas2d-fireworks`, which needs no such typelib, survives — and node and bun render the same showcases on the same box, one of them screenshot-verified at `OpenGL 4.6`. So `GI_TYPELIB_PATH` reaches node and bun and not deno.

UNRESOLVED, and deliberately not guessed at from Linux: the same error also appeared in that box's `gjs` column, which resolved through an npm-global gjsify **0.33.0**, so the measurement cannot separate "deno drops the env" from "an older CLI composed it wrong". `buildNativeEnv()` writes `GI_TYPELIB_PATH` with the `;` separator on win32 and `runRuntimeBundle` hands the same env to all three runtimes, so if this reproduces on a current CLI the difference is inside deno's launch (`--node-modules-dir=manual`, `RUNTIMES.deno.launch`), not in the composition. Needs one re-run on 0.37.0+ in an interactive Windows session before anyone changes code.
