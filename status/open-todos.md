...
- **`dpkg` is on no CI runner this project uses**, so the `.deb` is verified by GNU `ar` + GNU `tar` (independent readers of the container and of both inner tars) and by recomputing every `md5sums` digest — but never by a real `dpkg -i`. The `.rpm` half has no such gap: `rpm` is on every Fedora image and `rpm -i --test` runs there. Closing it needs a Debian container leg or `dpkg` in the image, and is worth doing before anyone publishes a `.deb`.
