...
### `@gjsify/webrtc-native` has no darwin target, and `@gjsify/webrtc`'s spec says so by failing

`@gjsify/webrtc` dies at `Requiring GjsifyWebrtc … Typelib file for namespace 'GjsifyWebrtc' not found` on macOS, because no `webrtc-native-darwin-*` package exists. An honest failure of a promise nobody made — the declaration is correct and the artifact genuinely does not exist. Recorded so a macOS run's red is READ correctly: it is a missing target, not a broken port. Closing it means adding the darwin leg to `build-prebuilds-macos` and the targets to the manifest, at which point the same spec becomes the check. (`@gjsify/sab-native`, the other package this entry used to name, gained `darwin-{arm64,x64}` with the ADR 0013 macOS port.)
