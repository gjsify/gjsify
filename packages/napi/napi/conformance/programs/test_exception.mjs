...
export default async function run(h) {
    ...
    h.emit('allowException-passes', (() => { try { binding.allowException(throwTheError); return 'no-throw'; } catch (e) { return e === theError; } })());
    ...
    h.emit('returnException(no-throw)', binding.returnException(() => {}));

    // Same, but the throwing site is a constructor (napi_new_instance).
    const throwCtor = class { constructor() { throw theError; } };
    h.emit('constructReturnException===theError', binding.constructReturnException(throwCtor) === theError);
    h.emit('constructAllowException-passes', (() => { try { binding.constructAllowException(throwCtor); return 'no-throw'; } catch (e) { return e === theError; } })());
    h.emit('wasPending(after-ctor-throw)', binding.wasPending());
    // NULL callback → a constructable no-op (arrow fns aren't constructable).
    h.emit('constructReturnException(no-throw)', binding.constructReturnException(function () {}));

    // No native exception → clean state (regular, constructable no-op fns).
    h.emit('allowException(no-throw)', (() => { try { binding.allowException(function () {}); return 'ok'; } catch (e) { return 'threw:' + e.name; } })());
    h.emit('wasPending(clean)', binding.wasPending());
    h.emit('constructAllowException(no-throw)', (() => { try { binding.constructAllowException(function () {}); return 'ok'; } catch (e) { return 'threw:' + e.name; } })());
