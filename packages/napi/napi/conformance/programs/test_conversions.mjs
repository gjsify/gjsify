...
export default async function run(h) {
    ...
    for (const v of [0, 1, 1.0, 1.1, 1.9, 0.9, 999.9, -1, Number.NaN])
        h.emit('asDouble', h.fmt(v), '=>', t.asDouble(v));
    for (const [lbl, v] of [
        ['undefined', undefined],
        ['null', null],
        ['false', false],
    ...
    h.emit(
        'toNumber(sym)!',
        h.caughtName(() => t.toNumber(testSym)),
    );

    // napi_coerce_to_object — print constructor + primitive it boxes.
    const shape = (o) =>
        `${o && o.constructor ? o.constructor.name : 'null'}:${typeof o === 'object' && o !== null ? JSON.stringify(o.valueOf()) : String(o)}`;
    for (const [lbl, v] of [
        ['{}', {}],
        ['{test:1}', { test: 1 }],
        ['[]', []],
    ...
    h.emit(
        'toString(sym)!',
        h.caughtName(() => t.toString(testSym)),
    );
