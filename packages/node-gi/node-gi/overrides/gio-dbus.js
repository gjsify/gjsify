...
export function createGioDBus(ctx) {
    ...
    function _proxyInvoker(wrappedProxy, methodName, sync, inSignature, args) {
        ...
        if (sync) {
            if (typeof wrappedProxy.call_with_unix_fd_list_sync === 'function') {
                const result = wrappedProxy.call_with_unix_fd_list_sync(
                    methodName,
                    inVariant,
                    flags,
                    -1,
        ...
        if (typeof wrappedProxy.call_with_unix_fd_list === 'function') {
            ...
            return wrappedProxy.call_with_unix_fd_list(
                methodName,
                inVariant,
                flags,
                -1,
    ...
    function makeProxyWrapper(interfaceXml) {
        ...
        ProxyClass.newAsync = function newAsync(
            bus,
            name,
            object,
            cancellable = null,
    ...
    function watchNameCore(connection, name, onNameAppeared, onNameVanished) {
        ...
        const dispatch = () => {
            ...
            if (owner && owner.length > 0) {
                if (owner !== lastOwner && typeof onNameAppeared === 'function')
                    safeCallback(onNameAppeared, connection, name, owner);
                lastOwner = owner;
            } else {
                if (lastOwner !== null && typeof onNameVanished === 'function')
