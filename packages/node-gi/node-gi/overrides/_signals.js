...
function _connectFull(name, callback, after) {
    ...
    if (!connectionsByName.length) this._signalConnectionsByName[name] = connectionsByName;
...
export function _disconnect(id) {
    ...
    if (!connection) throw new Error(`No signal connection ${id} found`);

    if (connection.disconnected) throw new Error(`Signal handler id ${id} already disconnected`);
    ...
    if (indexOfId !== -1) ids.splice(indexOfId, 1);

    if (ids.length === 0) delete this._signalConnectionsByName[connection.name];
}
...
export function _emit(name, ...args) {
    ...
    if (!_callHandlers(beforeHandlers, argArray)) _callHandlers(afterHandlers, argArray);
}

function _callHandlers(handlers, argArray) {
    for (const handler of handlers) {
        if (handler.disconnected) continue;
