...
function resolveNodeGiAddon() {
    ...
    if (prefer === 'prebuild') order = [prebuild];
        ...
        order = [prefer]; // an explicit path to a node_gi.node
...
const ledgered = (program, runtime) => ledger.entries.find((e) => e.program === program && e.runtime === runtime);
...
for (const rt of runtimes) {
    if (rt === 'node') {
        ...
        if (!existsSync(shimPrebuildDir))
            problems.push(
                `@gjsify/napi shim prebuild missing: ${shimPrebuildDir} (run: gjsify run build:prebuilds in ${napiPkg})`,
            );
        if (!nodeGiAddon)
            problems.push(
                `node-gi addon not built under ${join(pkgRoot, 'build')} (run: npm install / node-gyp rebuild)`,
            );
...
function runOn(runtime, entry) {
    ...
    const env =
        runtime === 'gjs-napi'
            ? {
                  ...childEnv,
                  NODE_GI_NATIVE: nodeGiAddon,
                  GI_TYPELIB_PATH: shimPrebuildDir,
                  LD_LIBRARY_PATH: shimPrebuildDir,
              }
            : childEnv;
...
for (const { name, file } of programs) {
    ...
    for (const rt of runtimes) {
        ...
        if (pass && excuse === undefined) {
            ...
            results.set(key, '✗');
            failures.push(
                `${name} × ${rt}: PASSES but has a ledger entry — stale ledger entry, remove it (reason was: ${excuse.reason})`,
            );
        } else if (!pass && excuse !== undefined) {
