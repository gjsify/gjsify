...
test('GtkSource-5 model + a GtkSource subclass with a vfunc_query_data override run on node-gi', { skip }, () => {
  ...
  app.connect('activate', () => {
    ...
    GLib.timeout_add(GLib.PRIORITY_DEFAULT, 350, () => {
      ...
      process.stderr.write(`[gtksource-smoke] pngLen=${pngLen} queryDataFired=${global.__queryDataFired} lastLine=${lastQueriedLine}\n`);
      app.quit();
  ...
  assert.equal(subclassTypeName, 'NodeGiGutterRendererLineNumbers', 'the GtkSource subclass instantiated + reports its own GType');
  ...
  if (!global.__queryDataFired) {
    process.stderr.write('[gtksource-smoke] note: query_data not invoked by the headless snapshot (expected; see comment)\n');
  }
});
