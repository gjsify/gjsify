...
// Same reasoning as the block above, for the one SUBPATH this package imports:
// `@nativescript/core/xml` is not covered by the bare-specifier ambient module — TypeScript
// resolves a subpath specifier separately, so `blueprint-markup.spec.ts` needs its own
// declaration here or it fails `gjsify tsc` wherever the real package is absent, which is
// every gjsify workspace install (the two npm packages that pin a concrete
// `@nativescript/core` version, the showcase and the integration test, are both excluded from
// the workspace glob). `XmlParser` is the standalone SAX parser `Builder` runs under the hood
// with namespaces on; unlike `Builder` it needs no device, which is why the spec drives it
// directly instead of going through the ambient `@nativescript/core` slice above.
declare module '@nativescript/core/xml' {
    /** One parser event, as `XmlParser`'s `onEvent` callback receives it. */
    export interface ParserEvent {
        /** One of {@link ParserEventType}'s members. */
        readonly eventType: string;
        /** Set on `StartElement`/`EndElement`; absent on a text/CDATA/comment event. */
    ...
    /** The `ParserEvent.eventType` values this package switches on. */
    export class ParserEventType {
        static readonly StartElement: string;
        static readonly EndElement: string;
    }

    /** A simple non-validating SAX parser — the one `Builder.parse` runs under the hood. */
