// Reference: Node.js lib/dns.js — the callback API. `index.ts` is the barrel, so
// `promises.ts` can build on this module without importing the barrel that
// re-exports it as `dns.promises`.
...
export function resolve4(
    ...
    lookup(hostname, { family: 4, all: true }, (err, addresses) => {
        ...
        callback(
            null,
            (addresses as unknown as LookupAddress[]).map((a) => a.address),
        );
    });
}
...
export function reverse(ip: string, callback: (err: NodeJS.ErrnoException | null, hostnames: string[]) => void): void {
    ...
    resolver.lookup_by_address_async(addr, null, (_source: Gio.Resolver | null, asyncResult: Gio.AsyncResult) => {
        try {
...
export function resolve(
    hostname: string,
    rrtype: string,
    callback: (err: NodeJS.ErrnoException | null, records: unknown[]) => void,
): void;
export function resolve(
    hostname: string,
