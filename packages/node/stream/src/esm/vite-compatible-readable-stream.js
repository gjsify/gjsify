...
function ReadableState(options, stream, isDuplex) {
  ...
  if (typeof isDuplex !== 'boolean') isDuplex = stream instanceof _registry.Duplex; // object stream flag. Used to make read(n) ignore n and to
...
var _require$codes$1 = errorsBrowser.codes,
    ...
    ERR_STREAM_CANNOT_PIPE = _require$codes$1.ERR_STREAM_CANNOT_PIPE,
    ERR_STREAM_DESTROYED = _require$codes$1.ERR_STREAM_DESTROYED,
    ERR_STREAM_NULL_VALUES = _require$codes$1.ERR_STREAM_NULL_VALUES,
    ERR_STREAM_WRITE_AFTER_END = _require$codes$1.ERR_STREAM_WRITE_AFTER_END,
    ERR_UNKNOWN_ENCODING = _require$codes$1.ERR_UNKNOWN_ENCODING;
...
function WritableState(options, stream, isDuplex) {
  ...
  if (typeof isDuplex !== 'boolean') isDuplex = stream instanceof _registry.Duplex; // object stream flag to indicate whether or not this stream
  ...
  this.highWaterMark = getHighWaterMark$2(this, options, 'writableHighWaterMark', isDuplex); // if _final has been called
...
function onwriteError(stream, state, sync, er, cb) {
  ...
  if (sync) {
    ...
    cb(er);
    stream._writableState.errorEmitted = true;
    errorOrDestroy$2(stream, er); // this can emit finish, but finish must
    // always follow error
...

{
  ...
  var keys = objectKeys(_registry.Writable.prototype);

  for (var v = 0; v < keys.length; v++) {
    var method = keys[v];
    if (!Duplex.prototype[method]) Duplex.prototype[method] = _registry.Writable.prototype[method];
  }
}
...
export default readableBrowser;
var finished = readableBrowser.finished;
var pipeline$1 = readableBrowser.pipeline;
export { Duplex$1 as Duplex, PassThrough$1 as PassThrough, Readable$1 as Readable, Stream, Transform$2 as Transform, Writable$1 as Writable, readableBrowser as __moduleExports, finished, pipeline$1 as pipeline };
