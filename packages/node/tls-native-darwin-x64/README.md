# @gjsify/tls-native-darwin-x64

> **No artifact in this tarball yet.** `darwin-x64` is declared by
> `@gjsify/tls-native` and built by CI, but not committed to the repository:
...
> harmless and does nothing: `@gjsify/tls-native` finds no typelib and takes its
> no-native path, exactly as if this package were absent.


The **darwin-x64** native artifacts of [`@gjsify/tls-native`](https://www.npmjs.com/package/@gjsify/tls-native) — a shared
...
You do not install this directly. `@gjsify/tls-native` declares it as an
