// oxlint-disable typescript/no-explicit-any -- ws-API-shape spec: verifyClient callbacks (`(info, cb) => …`), the upgrade triple (`(req, socket, head)`), and ws 'connection'/'message' listener payloads are all accepted as `any` here BECAUSE that's the exact shape npm-`ws`-using code writes — the spec demonstrates drop-in consumer usage against both the real npm `ws` (Node) and `@gjsify/ws` (GJS). Narrowing per-callback would mean importing internal `UpgradeRequest`/`UpgradeSocket`/`VerifyClientInfo` types into the spec, which (a) test files in `*.spec.ts` cannot import non-bare specifiers per `@gjsify/unit`'s rules and (b) would defeat the "this is exactly what consumers write" demonstration. Same precedent as the npm-`ws`-shape tests under refs/ws/test/. The Soup.Session websocket_connect_async callback (`(_self: unknown, asyncRes: any)`) is at the GI introspection boundary (Gio.AsyncResult typed via @girs would require coupling the spec to GJS — the file is already `on('Gjs')`-wrapped, but the import would still be hoisted).
            ...
            await it('handleProtocols selection appears in client ws.protocol via handleUpgrade', async () => {
                ...
                await new Promise<void>((resolve, reject) => {
                    ...
                    server.listen(0, () => {
                        ...
                        client.on('open', () => {
                            clientProtocol = (client as any).protocol;
                        });
                        client.on('close', () => {
                            server.close();
                            wss.close();
