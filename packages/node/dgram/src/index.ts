...
export class Socket extends EventEmitter {
     ...
     */
    bind(
        port?: number | { port?: number; address?: string; exclusive?: boolean },
        address?: string | (() => void),
        callback?: () => void,
    ): this {
        ...
        try {
            const family = this.type === 'udp6' ? Gio.SocketFamily.IPV6 : Gio.SocketFamily.IPV4;
            const inetAddr =
                Gio.InetAddress.new_from_string(bindAddress) ||
    ...
    private _resolveAndSend(
        destAddress: string,
        destPort: number,
        buf: Buffer,
        cb: ((err: Error | null, bytes: number) => void) | undefined,
    ): void {
...
export function createSocket(
    type: 'udp4' | 'udp6' | SocketOptions,
    callback?: (msg: Buffer, rinfo: AddressInfo) => void,
): Socket {
