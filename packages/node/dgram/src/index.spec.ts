    ...
    await describe('dgram', async () => {
        ...
        await describe('Socket methods', async () => {
            ...
            await it('should have setRecvBufferSize and setSendBufferSize', async () => {
                const socket = createSocket('udp4');
                expect(typeof socket.setRecvBufferSize === 'function' || socket.setRecvBufferSize === undefined).toBe(
                    true,
                );
                expect(typeof socket.setSendBufferSize === 'function' || socket.setSendBufferSize === undefined).toBe(
                    true,
                );
                socket.close();
            });
