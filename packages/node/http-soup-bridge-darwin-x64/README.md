# @gjsify/http-soup-bridge-darwin-x64

> **No artifact in this tarball yet.** `darwin-x64` is declared by
> `@gjsify/http-soup-bridge` and built by CI, but not committed to the repository:
...
> harmless and does nothing: `@gjsify/http-soup-bridge` finds no typelib and takes its
> no-native path, exactly as if this package were absent.


The **darwin-x64** native artifacts of [`@gjsify/http-soup-bridge`](https://www.npmjs.com/package/@gjsify/http-soup-bridge) — a shared
...
You do not install this directly. `@gjsify/http-soup-bridge` declares it as an
