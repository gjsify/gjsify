...
const qsUnescapeTestCases: [string, string][] = [
    ...
    ['there%2Qare%0-fake%escaped values in%%%%this%9Hstring', 'there%2Qare%0-fake%escaped values in%%%%this%9Hstring'],
...
export default async () => {
    ...
    await describe('querystring.parse maxKeys', async () => {
        ...
        await it('should treat string "Infinity" as default maxKeys', async () => {
            ...
            const result = qs.parse(str, undefined, undefined, { maxKeys: 'Infinity' as unknown as number });
        ...
        await it('should treat string "NaN" as default maxKeys', async () => {
            ...
            const result = qs.parse(str, undefined, undefined, { maxKeys: 'NaN' as unknown as number });
            expect(Object.keys(result).length).toBe(1000);
        });
    });
    ...
    await describe('querystring.parse custom decode', async () => {
        await it('should use custom decodeURIComponent', async () => {
            ...
            check(qs.parse('a=a&b=b&c=c', undefined, undefined, { decodeURIComponent: demoDecode }), {
                aa: 'aa',
                bb: 'bb',
                cc: 'cc',
            });
        ...
        await it('should use custom decode with custom eq', async () => {
            check(qs.parse('a=a&b=b&c=c', undefined, '==', { decodeURIComponent: (str: string) => str }), {
                'a=a': '',
                'b=b': '',
                'c=c': '',
            });
    ...
    await describe('querystring.stringify', async () => {
        ...
        await it('should stringify BigInt with custom encode', async () => {
            expect(
                qs.stringify({ foo: 2n ** 1023n }, undefined, undefined, { encodeURIComponent: (c: string) => c }),
            ).toBe('foo=' + 2n ** 1023n);
        });
        ...
        await it('should use custom encodeURIComponent', async () => {
            function demoEncode(str: string) {
                return str[0];
            }
            const obj = { aa: 'aa', bb: 'bb', cc: 'cc' };
            expect(qs.stringify(obj, undefined, undefined, { encodeURIComponent: demoEncode })).toBe('a=a&b=b&c=c');
        });

        await it('should use custom encode for different types', async () => {
            const obj = { number: 1, bigint: 2n, true: true, false: false, object: {} };
            expect(qs.stringify(obj as any, undefined, undefined, { encodeURIComponent: (v: string) => v })).toBe(
    ...
    await describe('querystring.escape', async () => {
        ...
        await it('should escape valid surrogate pair with trailing chars', async () => {
            expect(qs.escape(`${String.fromCharCode(0xd800 + 1)}test`)).toBe('%F0%90%91%B4est');
        });
        ...
        await it('should prefer toString over valueOf for objects', async () => {
            expect(qs.escape({ test: 5, toString: () => 'test', valueOf: () => 10 } as unknown as string)).toBe('test');
        });
