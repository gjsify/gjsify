...
//   - encoding.ts      — normalizeEncoding / checkEncoding / Encoding (moved
//                        from `@gjsify/utils` in PR #265 — Phase 1 of the
//                        utils audit).
//   - base64.ts        — base64Encode / base64Decode / atobPolyfill /
//                        btoaPolyfill (moved from `@gjsify/utils` same PR).
//   - buffer-codec.ts  — encodeString / decodeString / checkOffset
//                        (per-encoding String ↔ Uint8Array switch logic,
//                        ~135 LoC). No Buffer-class coupling.
//   - buffer.ts        — this file: Buffer + SlowBuffer classes + the
//                        constants block (kMaxLength / kStringMaxLength /
//                        constants).
//
// BufferEncoding is a global type provided by @types/node.

import { normalizeEncoding, checkEncoding } from './encoding.js';
import { atobPolyfill as _atob } from './base64.js';
import { encodeString, decodeString, checkOffset } from './buffer-codec.js';
...
export class Buffer extends Uint8Array {
    ...
    static from(
        value: { viewBytes(offset: number, length: number): Uint8Array; readonly byteLength: number },
        byteOffset?: number,
        length?: number,
    ): Buffer;
    ...
    lastIndexOf(value: number | string | Uint8Array | Buffer, byteOffset?: number, encoding?: string): number {
        if (typeof value === 'number') {
            ...
            return byteOffset !== undefined
                ? super.lastIndexOf(value & 0xff, byteOffset)
                : super.lastIndexOf(value & 0xff);
        }
        const needle = typeof value === 'string' ? encodeString(value, normalizeEncoding(encoding)) : value;
        const start =
            byteOffset !== undefined ? Math.min(byteOffset, this.length - needle.length) : this.length - needle.length;
    ...
    readUInt32LE(offset = 0): number {
        checkOffset(offset, 4, this.length);
        return (
            (this[offset + 3] * 0x1000000 + ((this[offset + 2] << 16) | (this[offset + 1] << 8) | this[offset])) >>> 0
        );
    }
