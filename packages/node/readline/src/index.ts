...
import type { GjsReadableTty, GjsWritableTty, KeypressEmitter, KeypressTaggedStream } from './internal/stream-types.js';
...
export class Interface extends EventEmitter {
    ...
    close(): void {
        ...
        if (this._input) {
            ...
            if (this._boundOnKeypress)
                this._input.removeListener('keypress', this._boundOnKeypress as (...args: unknown[]) => void);
    ...
    [Symbol.asyncIterator](): AsyncIterableIterator<string> {
        ...
        return {
            next: (): Promise<IteratorResult<string, undefined>> => {
                ...
                return new Promise<IteratorResult<string, undefined>>((r) => {
                    resolve = r;
                });
            },
...
export function emitKeypressEvents(stream: Readable, iface: { escapeCodeTimeout?: number } = {}): void {
    ...
    const triggerEscape = () => (tagged[_ESCAPE_DECODER] as Generator<void, void, string>).next('');

    function onData(input: Buffer | string): void {
        if (stream.listenerCount('keypress') > 0) {
            const decoder = tagged[_KEYPRESS_DECODER] as StringDecoder;
            const str: string = decoder.write(typeof input === 'string' ? Buffer.from(input) : input);
            if (str) {
