// oxlint-disable typescript/no-explicit-any -- spec catches errors with `catch (err: any)` and uses `received: any[]` for dynamic event args; `(EventEmitter as any).EventEmitter` for backward-compat access
...
export default async () => {
    ...
    await describe('EventEmitter: this in listeners', async () => {
        await it('should bind this to the emitter in on listeners', async () => {
            const emitter = new EventEmitter();
            let self: unknown;
            emitter.on('test', function (this: unknown) {
                self = this;
            });
            emitter.emit('test');
        ...
        await it('should bind this to the emitter in once listeners', async () => {
            const emitter = new EventEmitter();
            let self: unknown;
            emitter.once('test', function (this: unknown) {
                self = this;
            });
            emitter.emit('test');
            expect(self).toBe(emitter);
