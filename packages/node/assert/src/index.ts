...
function throws(fn: () => unknown, errorOrMessage?: ErrorPredicate | string, message?: string | Error): void {
    if (typeof fn !== 'function') {
...
function doesNotThrow(fn: () => unknown, errorOrMessage?: ErrorPredicate | string, message?: string | Error): void {
    if (typeof fn !== 'function') {
        throw new TypeError('The "fn" argument must be of type function.');
    }
    ...
    if (
        expected === undefined ||
        (typeof expected === 'function' && expected.prototype !== undefined && actual instanceof expected)
    ) {
        innerFail({
            actual,
            expected,
...
function fail(
    ...
    if (arguments.length === 0 || arguments.length === 1) {
        const msg =
            arguments.length === 0 ? 'Failed' : typeof actualOrMessage === 'string' ? actualOrMessage : undefined;
...
function doesNotMatch(actual: string, expected: RegExp, message?: string | Error): void {
    ...
    if (expected.test(actual)) {
        innerFail({
            ...
            message:
                message ||
                `The input was expected to not match the regular expression ${expected}. Input:\n\n'${actual}'\n`,
