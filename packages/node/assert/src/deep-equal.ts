...
function isBoxedPrimitive(v: unknown): boolean {
    return (
        v instanceof Number || v instanceof String || v instanceof Boolean || v instanceof BigInt || v instanceof Symbol
    );
}

function isNumberObject(v: unknown): v is number {
    return v instanceof Number;
}
function isStringObject(v: unknown): v is string {
    return v instanceof String;
}
function isBooleanObject(v: unknown): v is boolean {
    return v instanceof Boolean;
}
function isBigIntObject(v: unknown): boolean {
    return (
        typeof BigInt !== 'undefined' && v instanceof Object && Object.prototype.toString.call(v) === '[object BigInt]'
    );
}
...
function isEqualBoxedPrimitive(val1: unknown, val2: unknown): boolean {
    ...
    if (isBigIntObject(val1)) {
        return (
            isBigIntObject(val2) &&
            (val1 as object & { [Symbol.toPrimitive]: (hint: string) => unknown })[Symbol.toPrimitive]('number') ===
                (val2 as object & { [Symbol.toPrimitive]: (hint: string) => unknown })[Symbol.toPrimitive]('number')
        );
    }
...
function innerDeepEqual(val1: unknown, val2: unknown, mode: number, memos: Memo | null | undefined): boolean {
    // Identical values
    if (val1 === val2) {
...
function objectComparisonStart(val1: object, val2: object, mode: number, memos: Memo | null | undefined): boolean {
    ...
    if (Array.isArray(val1)) {
        ...
        if (
            hasOwn(val1, 'cause') &&
            !innerDeepEqual(
                (val1 as Error & { cause?: unknown }).cause,
                (val2 as Error & { cause?: unknown }).cause,
                mode,
                memos,
            )
        ) {
    ...
    if (keys2.length > 0) {
        ...
        for (; i < keys2.length; i++) {
            ...
            if (
                ...
                !innerDeepEqual(descriptor.value !== undefined ? descriptor.value : aRec[key], bRec[key], mode, memos)
            ) {
                return false;
            }
        }
