# @gjsify/terminal-native-darwin-x64

> **No artifact in this tarball yet.** `darwin-x64` is declared by
> `@gjsify/terminal-native` and built by CI, but not committed to the repository:
...
> harmless and does nothing: `@gjsify/terminal-native` finds no typelib and takes its
> no-native path, exactly as if this package were absent.


The **darwin-x64** native artifacts of [`@gjsify/terminal-native`](https://www.npmjs.com/package/@gjsify/terminal-native) — a shared
...
You do not install this directly. `@gjsify/terminal-native` declares it as an
