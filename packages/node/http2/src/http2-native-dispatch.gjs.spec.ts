...
import type { Http2Server, Http2ServerRequest, Http2ServerResponse } from '@gjsify/http2';
    ...
    await on('Gjs', async () => {
        ...
        await describe('Http2NativeDispatcher — h2c round-trip via in-process SessionBridge client', async () => {
            ...
            await it('delivers PUSH_PROMISE on the wire — client sees the promised stream', async () => {
                ...
                const fx = openClient(port);
                try {
                    let pushPromise: { sid: number; promisedSid: number; headers: Array<[string, string]> } | null =
                        null;
                    const responses = new Map<
                        number,
                        { headers: Array<[string, string]> | null; body: string; done: boolean }
                    >();
                    fx.client.connect(
                        'push_promise_received',
                        (_b: unknown, sid: number, promisedSid: number, headersVar: any) => {
                            pushPromise = { sid, promisedSid, headers: headersVar.deep_unpack() };
                        },
                    );
                    fx.client.connect(
                        'headers_received',
                        (_b: unknown, sid: number, headersVar: any, _endStream: boolean) => {
                    ...
                    await waitFor(
                        () =>
                            pushPromise !== null &&
                            responses.get(1)?.done === true &&
                            responses.get(pushPromise!.promisedSid)?.done === true,
                        5000,
                    );
            ...
            await it('Phase 3 — client receives server-pushed streams via "stream" event', async () => {
                ...
                try {
                    ...
                    const pushReceived = new Promise<void>((resolve) => {
                        (session as any).on(
                            'stream',
                            async (pushed: any, headers: Record<string, string | string[]>) => {
            ...
            await it('Phase 4 — server.close() releases the listen port immediately', async () => {
                ...
                const { server: server2 } = await startServer((_req, res) => {
                    res.end('ok');
                });
