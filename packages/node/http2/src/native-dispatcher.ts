...
export class Http2NativeDispatcher {
    ...
    listen(port: number): number {
        ...
        if (port > 0) {
            ...
            const inetAddr =
                Gio.InetAddress.new_from_string('0.0.0.0') ?? Gio.InetAddress.new_any(Gio.SocketFamily.IPV4);
                ...
                if (ready & GLib.IOCondition.IN) {
                    const [n, buf] = (
                        socket as unknown as {
                            receive: (size: number, c: Gio.Cancellable | null) => [number, Uint8Array];
                        }
                    ).receive(READ_BUFFER_SIZE, null);
    ...
    private _setupNativeConnection(
        ...
        bridge.connect(
            'headers-received',
            (_b: unknown, streamId: number, headersVar: GLib.Variant, endStream: boolean) => {
    ...
    private _onHeaders(conn: NativeConnection, streamId: number, headersVar: GLib.Variant, endStream: boolean): void {
 ...
 */
function headersRecordToArrays(headers: Record<string, string | number | string[]>): {
    names: string[];
    values: string[];
} {
    const names: string[] = [];
