...
export class ClientHttp2Stream extends Duplex {
    ...
    private async _sendRequest(): Promise<void> {
        ...
        const scheme =
            (this._requestHeaders[':scheme'] as string) || (authority.startsWith('https') ? 'https' : 'http');
        ...
        try {
            ...
            try {
                ...
                while (
                    (chunk = await readBytesAsync(inputStream, 16384, GLib.PRIORITY_DEFAULT, this._cancellable)) !==
                    null
                ) {
                    if (chunk.length > 0) {
...
export class ClientHttp2Session extends Http2Session {
    ...
    private _setupNativeClient(authority: string, mode: 'auto' | 'force' | 'off'): void {
        ...
        if (!Http2NativeClientDispatcher.available()) {
            if (mode === 'force') {
                throw new Error(
                    '@gjsify/http2-native prebuild not loadable — nativeDispatcher: "force" cannot proceed',
                );
            }
        ...
        this._nativeClient = new Http2NativeClientDispatcher({
            onPushPromise: (event) => {
                ...
                (pushedStream as unknown as { _setNativeStreamId(id: number): void })._setNativeStreamId(
                    event.promisedStreamId,
                );
