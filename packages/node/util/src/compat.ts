...
export function debuglog(section: string): (...args: unknown[]) => void {
    ...
    return (...args: unknown[]) => {
        if (debug === undefined) {
            const nodeDebug =
                typeof globalThis.process?.env?.NODE_DEBUG === 'string' ? globalThis.process.env.NODE_DEBUG : '';
...
export function aborted(signal: AbortSignal, resource: object): Promise<void> {
    ...
    if (signal == null || typeof (signal as AbortSignal).aborted !== 'boolean') {
        return Promise.reject(new TypeError('The "signal" argument must be an instance of AbortSignal'));
    }
    if (resource == null || typeof resource !== 'object') {
        return Promise.reject(new TypeError('The "resource" argument must be an non-null object'));
    }
