    ...
    set src(src: string) {
        ...
        if (src.startsWith('data:')) {
            ...
            try {
                ...
                if (DEBUG)
                    console.warn(`[img] error data:: ${_error instanceof Error ? _error.message : String(_error)}`);
                this._complete = true;
                this.dispatchEvent(new Event('error'));
            }
            return;
        ...
        try {
            ...
            if (DEBUG)
                console.warn(`[img] error ${filename}: ${_error instanceof Error ? _error.message : String(_error)}`);
            this._complete = true;
            this.dispatchEvent(new Event('error'));
        }
    }
