...
export default async () => {
    ...
    await describe('renderLauncher', async () => {
        await it('execs the interpreter `gjsify.app` names, and only that one', async () => {
            ...
            expect(renderLauncher(settings(), 'app.gjs.js')).toContain('exec gjs -m "$prefix"/lib/hello/app.gjs.js');
            expect(renderLauncher(settings({ app: 'node' }), 'app.node.mjs')).toContain(
                'exec node "$prefix"/lib/hello/app.node.mjs',
            );
            // `not.toContain('gjs')` would be the wrong assertion and was: the
            // generated header line reads "Generated by `gjsify ship`". The claim
            // is about the exec line, so that is what the check names.
            expect(renderLauncher(settings({ app: 'node' }), 'app.node.mjs')).not.toContain('exec gjs');
        });

        await it('keeps the prefix derived and the user args last, on both branches', async () => {
            // A baked path would force one payload per format (ADR 0024 § 3);
            // dropping `"$@"` would silently swallow every argument the user typed.
            for (const app of ['gjs', 'node'] as const) {
                const launcher = renderLauncher(settings({ app }), 'a.mjs');
                expect(launcher).toContain('prefix=$(dirname "$(dirname "$self")")');
                expect(launcher.trimEnd().endsWith('"$@"')).toBe(true);
            }
        });
    });
