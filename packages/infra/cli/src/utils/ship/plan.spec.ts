    ...
    await describe('renderLauncher', async () => {
        ...
        await it('execs the interpreter this target resolved to, and only that one', async () => {
            ...
            expect(renderLauncher(settings(), 'app.gjs.js', LAYOUTS.linux)).toContain(
                'exec gjs -m "$prefix"/lib/hello/app.gjs.js',
            );
            expect(renderLauncher(settings({ app: 'node' }), 'app.node.mjs', LAYOUTS.linux)).toContain(
            ...
            expect(renderLauncher(settings({ app: 'node' }), 'app.node.mjs', LAYOUTS.linux)).not.toContain('exec gjs');
        });
        ...
        await it('keeps the prefix derived and the user args last, on both branches', async () => {
            ...
            for (const app of ['gjs', 'node'] as const) {
                const launcher = renderLauncher(settings({ app }), 'a.mjs', LAYOUTS.linux);
