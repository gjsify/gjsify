...
export default async () => {
    ...
    await describe('napi-node-addon: napi-rs ENTRY replacement (resolveId + load)', async () => {
        ...
        function makeEntryFixture(): { root: string; entry: string; sibling: string; siblingNode: string } {
            ...
            // The sibling is chosen BY HOST TRIPLE (that is the whole point of
            // `resolveNapiRsEntryAddon` — declaration order picked a foreign
            // binary in the lightningcss bug), so the fixture must name the
            // triple of the machine actually running the suite. Hard-coding
            // `linux-x64-gnu` made every test in this describe fail on macOS:
            // no host sibling existed, `detectNapiRsEntry` correctly returned
            // null, and that read as a plugin regression instead of a fixture
            // built for one CI runner.
            const triple = hostNapiRsTriple();
            if (!triple) {
                throw new Error(
                    'hostNapiRsTriple() returned null — napi-rs does not name this host, so the ' +
                        'entry-replacement fixture cannot be built for it.',
                );
            }
            const sibling = `@node-rs/argon2-${triple}`;
            // A deliberately FOREIGN sibling alongside it: npm installs only the
            // host's optionalDependency, but `gjsify install` materialises every
            // platform package, so selection must key on the triple rather than
            // on whichever entry resolves first.
            const foreign =
                triple === 'linux-x64-gnu' ? '@node-rs/argon2-darwin-arm64' : '@node-rs/argon2-linux-x64-gnu';
            writeFileSync(
                join(root, 'package.json'),
                JSON.stringify({
                    name: '@node-rs/argon2',
            ...
            const siblingNode = touch(join(root, 'node_modules', sibling), `argon2.${triple}.node`);
        ...
        await it('replaces the loader when the sibling lives under napi.packageName scope', async () => {
            ...
            // Host-triple driven for the same reason as `makeEntryFixture`.
            const triple = hostNapiRsTriple()!;
            const sibling = `@rolldown/binding-${triple}`;
            const foreign =
                triple === 'linux-x64-gnu' ? '@rolldown/binding-darwin-arm64' : '@rolldown/binding-linux-x64-gnu';
            writeFileSync(
                join(root, 'package.json'),
                JSON.stringify({
                    name: 'rolldown',
                    main: './dist/index.mjs',
                    napi: { binaryName: 'rolldown-binding', packageName: '@rolldown/binding' },
                    optionalDependencies: { [sibling]: '1.1.4', [foreign]: '1.1.4' },
                }),
            );
            mkdirSync(join(root, 'dist'), { recursive: true });
            const entry = join(root, 'dist', 'index.mjs');
            writeFileSync(entry, '/* auto-generated by NAPI-RS */');
            const siblingNode = touch(join(root, 'node_modules', sibling), `rolldown-binding.${triple}.node`);
