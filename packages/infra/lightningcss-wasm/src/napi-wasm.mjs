...
export const napi = {
    ...
    napi_get_typedarray_info(env_id, typedarray, type, length, data, arraybuffer, byte_offset) {
        ...
        env.setPointer(
            type,
            typedArrays.findIndex((constructor) => val instanceof constructor),
        );
    ...
    napi_is_buffer(env_id, value, result) {
        ...
        let val = env.get(value);
        env.memory[result] = (
            typeof globalThis.Buffer !== 'undefined' ? globalThis.Buffer.isBuffer(val) : val instanceof Uint8Array
        )
            ? 1
            : 0;
        return NAPI_OK;
