...
var require_pattern = __commonJS({
  "../../../node_modules/fast-glob/out/utils/pattern.js"(exports) {
    ...
    function isAffectDepthOfReadingPattern(pattern) {
      const basename2 = path6.basename(pattern);
...
var require_out = __commonJS({
  "../../../node_modules/@nodelib/fs.stat/out/index.js"(exports) {
    ...
    exports.stat = stat;
    function statSync3(path6, optionsOrSettings) {
    ...
    function readdir(directory, settings, callback) {
      settings.fs.readdir(directory, (readdirError, names) => {
        ...
        const tasks = names.map((name) => {
          const path6 = common2.joinPathSegments(directory, name, settings.pathSegmentSeparator);
          return (done) => {
...
// src/utils/scan-globals.ts
import { writeFile, mkdir } from "node:fs/promises";
import { createHash } from "node:crypto";
import { join } from "node:path";

// ../resolve-npm/lib/globals-map.mjs
var GJS_GLOBALS_GROUPS = {
  node: [
    "Buffer",
    "Blob",
    "File",
...
// src/utils/scan-globals.ts
var GLOBALS_MAP = GJS_GLOBALS_MAP;
var GLOBALS_GROUPS = GJS_GLOBALS_GROUPS;
function resolveGlobalsList(globalsArg) {
  ...
  for (const rawToken of trimmed.split(",")) {
    ...
    if (group) {
      for (const id of group) {
        const path6 = GLOBALS_MAP[id];
        if (path6) result.add(path6);
      }
    } else {
      const path6 = GLOBALS_MAP[token];
      if (path6) result.add(path6);
    }
  }
  return result;
}
async function writeRegisterInjectFile(registerPaths, cwd = process.cwd()) {
  ...
  const path6 = join(cacheDir, `auto-globals-${hash}.mjs`);
...
RegExpValidationState.prototype.at = function at(i2, forceU) {
  ...
  var c3 = s.charCodeAt(i2);
  if (!(forceU || this.switchU) || c3 <= 55295 || c3 >= 57344 || i2 + 1 >= l) {
    return c3;
  }
...
RegExpValidationState.prototype.nextIndex = function nextIndex(i2, forceU) {
  ...
  var c3 = s.charCodeAt(i2), next;
  if (!(forceU || this.switchU) || c3 <= 55295 || c3 >= 57344 || i2 + 1 >= l || (next = s.charCodeAt(i2 + 1)) < 56320 || next > 57343) {
    return i2 + 1;
  }
  return i2 + 2;
};
...
function simple(node, visitors, baseVisitor, state, override) {
  ...
  (function c3(node2, st, override2) {
    var type = override2 || node2.type;
    visitNode(baseVisitor, type, node2, st, c3);
    if (visitors[type]) {
...
function skipThrough(node, st, c3) {
  c3(node, st);
}
function ignore(_node, _st, _c) {
}
function visitNode(baseVisitor, type, node, st, c3) {
  if (baseVisitor[type] == null) {
    throw new Error("No walker function defined for node type " + type);
  }
  baseVisitor[type](node, st, c3);
}
var base = {};
base.Program = base.BlockStatement = base.StaticBlock = function(node, st, c3) {
  for (var i2 = 0, list = node.body; i2 < list.length; i2 += 1) {
    var stmt = list[i2];
    c3(stmt, st, "Statement");
  }
};
base.Statement = skipThrough;
base.EmptyStatement = ignore;
base.ExpressionStatement = base.ParenthesizedExpression = base.ChainExpression = function(node, st, c3) {
  return c3(node.expression, st, "Expression");
};
base.IfStatement = function(node, st, c3) {
  c3(node.test, st, "Expression");
  c3(node.consequent, st, "Statement");
  if (node.alternate) {
    c3(node.alternate, st, "Statement");
  }
};
base.LabeledStatement = function(node, st, c3) {
  return c3(node.body, st, "Statement");
};
base.BreakStatement = base.ContinueStatement = ignore;
base.WithStatement = function(node, st, c3) {
  c3(node.object, st, "Expression");
  c3(node.body, st, "Statement");
};
base.SwitchStatement = function(node, st, c3) {
  c3(node.discriminant, st, "Expression");
  for (var i2 = 0, list = node.cases; i2 < list.length; i2 += 1) {
    var cs = list[i2];
    c3(cs, st);
  }
};
base.SwitchCase = function(node, st, c3) {
  if (node.test) {
    c3(node.test, st, "Expression");
  }
  for (var i2 = 0, list = node.consequent; i2 < list.length; i2 += 1) {
    var cons = list[i2];
    c3(cons, st, "Statement");
  }
};
base.ReturnStatement = base.YieldExpression = base.AwaitExpression = function(node, st, c3) {
  if (node.argument) {
    c3(node.argument, st, "Expression");
  }
};
base.ThrowStatement = base.SpreadElement = function(node, st, c3) {
  return c3(node.argument, st, "Expression");
};
base.TryStatement = function(node, st, c3) {
  c3(node.block, st, "Statement");
  if (node.handler) {
    c3(node.handler, st);
  }
  if (node.finalizer) {
    c3(node.finalizer, st, "Statement");
  }
};
base.CatchClause = function(node, st, c3) {
  if (node.param) {
    c3(node.param, st, "Pattern");
  }
  c3(node.body, st, "Statement");
};
base.WhileStatement = base.DoWhileStatement = function(node, st, c3) {
  c3(node.test, st, "Expression");
  c3(node.body, st, "Statement");
};
base.ForStatement = function(node, st, c3) {
  if (node.init) {
    c3(node.init, st, "ForInit");
  }
  if (node.test) {
    c3(node.test, st, "Expression");
  }
  if (node.update) {
    c3(node.update, st, "Expression");
  }
  c3(node.body, st, "Statement");
};
base.ForInStatement = base.ForOfStatement = function(node, st, c3) {
  c3(node.left, st, "ForInit");
  c3(node.right, st, "Expression");
  c3(node.body, st, "Statement");
};
base.ForInit = function(node, st, c3) {
  if (node.type === "VariableDeclaration") {
    c3(node, st);
  } else {
    c3(node, st, "Expression");
  }
};
base.DebuggerStatement = ignore;
base.FunctionDeclaration = function(node, st, c3) {
  return c3(node, st, "Function");
};
base.VariableDeclaration = function(node, st, c3) {
  for (var i2 = 0, list = node.declarations; i2 < list.length; i2 += 1) {
    var decl = list[i2];
    c3(decl, st);
  }
};
base.VariableDeclarator = function(node, st, c3) {
  c3(node.id, st, "Pattern");
  if (node.init) {
    c3(node.init, st, "Expression");
  }
};
base.Function = function(node, st, c3) {
  if (node.id) {
    c3(node.id, st, "Pattern");
  }
  for (var i2 = 0, list = node.params; i2 < list.length; i2 += 1) {
  ...
  c3(node.body, st, node.expression ? "Expression" : "Statement");
};
base.Pattern = function(node, st, c3) {
  if (node.type === "Identifier") {
    c3(node, st, "VariablePattern");
  } else if (node.type === "MemberExpression") {
    c3(node, st, "MemberPattern");
  } else {
    c3(node, st);
  }
};
...
base.RestElement = function(node, st, c3) {
  return c3(node.argument, st, "Pattern");
};
base.ArrayPattern = function(node, st, c3) {
...
base.ObjectPattern = function(node, st, c3) {
  for (var i2 = 0, list = node.properties; i2 < list.length; i2 += 1) {
    var prop = list[i2];
    if (prop.type === "Property") {
      if (prop.computed) {
        c3(prop.key, st, "Expression");
      }
      c3(prop.value, st, "Pattern");
    } else if (prop.type === "RestElement") {
      c3(prop.argument, st, "Pattern");
    }
  }
};
base.Expression = skipThrough;
base.ThisExpression = base.Super = base.MetaProperty = ignore;
base.ArrayExpression = function(node, st, c3) {
  for (var i2 = 0, list = node.elements; i2 < list.length; i2 += 1) {
    var elt = list[i2];
    if (elt) {
      c3(elt, st, "Expression");
    }
  }
};
base.ObjectExpression = function(node, st, c3) {
  for (var i2 = 0, list = node.properties; i2 < list.length; i2 += 1) {
    var prop = list[i2];
    c3(prop, st);
  }
...
base.SequenceExpression = function(node, st, c3) {
  for (var i2 = 0, list = node.expressions; i2 < list.length; i2 += 1) {
    var expr = list[i2];
    c3(expr, st, "Expression");
  }
};
base.TemplateLiteral = function(node, st, c3) {
...
base.UnaryExpression = base.UpdateExpression = function(node, st, c3) {
  c3(node.argument, st, "Expression");
};
base.BinaryExpression = base.LogicalExpression = function(node, st, c3) {
  c3(node.left, st, "Expression");
  c3(node.right, st, "Expression");
};
base.AssignmentExpression = base.AssignmentPattern = function(node, st, c3) {
  c3(node.left, st, "Pattern");
  c3(node.right, st, "Expression");
};
base.ConditionalExpression = function(node, st, c3) {
  c3(node.test, st, "Expression");
  c3(node.consequent, st, "Expression");
  c3(node.alternate, st, "Expression");
};
base.NewExpression = base.CallExpression = function(node, st, c3) {
  c3(node.callee, st, "Expression");
  if (node.arguments) {
    for (var i2 = 0, list = node.arguments; i2 < list.length; i2 += 1) {
      var arg = list[i2];
      c3(arg, st, "Expression");
    }
  }
};
base.MemberExpression = function(node, st, c3) {
  c3(node.object, st, "Expression");
  if (node.computed) {
    c3(node.property, st, "Expression");
  }
};
base.ExportNamedDeclaration = base.ExportDefaultDeclaration = function(node, st, c3) {
  if (node.declaration) {
    c3(node.declaration, st, node.type === "ExportNamedDeclaration" || node.declaration.id ? "Statement" : "Expression");
  }
  if (node.source) {
    c3(node.source, st, "Expression");
  }
  if (node.attributes) {
...
base.ExportAllDeclaration = function(node, st, c3) {
  if (node.exported) {
    c3(node.exported, st);
  }
  c3(node.source, st, "Expression");
...
base.ImportAttribute = function(node, st, c3) {
  c3(node.value, st, "Expression");
};
base.ImportDeclaration = function(node, st, c3) {
  for (var i2 = 0, list = node.specifiers; i2 < list.length; i2 += 1) {
    var spec = list[i2];
    c3(spec, st);
  }
...
base.ImportExpression = function(node, st, c3) {
  c3(node.source, st, "Expression");
  if (node.options) {
    c3(node.options, st, "Expression");
  }
};
base.ImportSpecifier = base.ImportDefaultSpecifier = base.ImportNamespaceSpecifier = base.Identifier = base.PrivateIdentifier = base.Literal = ignore;
base.TaggedTemplateExpression = function(node, st, c3) {
  c3(node.tag, st, "Expression");
  c3(node.quasi, st, "Expression");
};
base.ClassDeclaration = base.ClassExpression = function(node, st, c3) {
  return c3(node, st, "Class");
};
base.Class = function(node, st, c3) {
  if (node.id) {
    c3(node.id, st, "Pattern");
  }
  if (node.superClass) {
    c3(node.superClass, st, "Expression");
  }
  c3(node.body, st);
};
base.ClassBody = function(node, st, c3) {
  for (var i2 = 0, list = node.body; i2 < list.length; i2 += 1) {
    var elt = list[i2];
    c3(elt, st);
  }
};
base.MethodDefinition = base.PropertyDefinition = base.Property = function(node, st, c3) {
  if (node.computed) {
    c3(node.key, st, "Expression");
  }
  if (node.value) {
    c3(node.value, st, "Expression");
  }
...
// src/utils/detect-free-globals.ts
var KNOWN_GLOBALS = new Set(Object.keys(GJS_GLOBALS_MAP));
var METHOD_MARKERS = {
  ...
  switch (node.type) {
    ...
    case "ObjectPattern":
      return node.properties.flatMap(
        (p) => p.type === "RestElement" ? extractBindingNames(p.argument) : extractBindingNames(p.value)
      );
    case "ArrayPattern":
      return node.elements.flatMap(
        (e) => e ? e.type === "RestElement" ? extractBindingNames(e.argument) : extractBindingNames(e) : []
      );
    case "AssignmentPattern":
...
function detectFreeGlobals(code) {
  ...
  const freeGlobals = /* @__PURE__ */ new Set();
  const HOST_OBJECTS = /* @__PURE__ */ new Set(["globalThis", "global", "window", "self", "globalObject"]);
  ancestor(ast, {
    ...
    Identifier(node, ancestors) {
      ...
      switch (parent.type) {
        ...
        case "MemberExpression": {
          const mem = parent;
          if (mem.property === node && !mem.computed) return;
          break;
        }
        // { key: value } — skip if this is the non-computed key
        case "Property": {
          const prop = parent;
          if (prop.key === node && !prop.computed) return;
          break;
        }
        ...
        case "PropertyDefinition": {
          const def = parent;
          if (def.key === node && !def.computed) return;
          break;
        }
        // label: — skip
        case "LabeledStatement": {
          const labeled = parent;
          if (labeled.label === node) return;
          break;
        }
        // export { X as Y } — skip the exported name
        case "ExportSpecifier": {
          const spec = parent;
          if (spec.exported === node) return;
          break;
        }
        ...
        case "ClassExpression": {
          const decl = parent;
          if (decl.id === node) return;
          break;
        }
        case "VariableDeclarator": {
          const vd = parent;
          if (vd.id === node) return;
          break;
        }
...
var aliasPlugin = (aliasObj) => {
  ...
  const plugin = {
    name: "alias",
    setup(build2) {
      build2.onResolve({ filter: re }, async (args) => {
        ...
        if (resolvedAliasPath.startsWith("http://")) {
          ...
          let resolvedAlias = await build2.resolve(resolvedAliasPath, {
          ...
          if (resolvedAlias.errors.length > 0 && build2.initialOptions.absWorkingDir) {
            resolvedAlias = await build2.resolve(resolvedAliasPath, {
              kind: args.kind,
              namespace,
              resolveDir: build2.initialOptions.absWorkingDir
            });
          }
...
var ALIASES_NODE_FOR_GJS = {
  ...
  "vm": "@gjsify/vm",
  "wasi": "@gjsify/wasi",
  // stub — low priority (WebAssembly System Interface)
...
var ALIASES_GJS_FOR_NODE = {};
var ALIASES_WEB_FOR_NODE = {
...
var getAliasesGeneralForGjs = (options) => ALIASES_GENERAL_FOR_GJS;
var getAliasesNodeForGjs = (options) => setNodeAliasPrefix(ALIASES_NODE_FOR_GJS);
var getAliasesWebForGjs = (options) => ALIASES_WEB_FOR_GJS;
var getAliasesGeneralForNode = (options) => ALIASES_GENERAL_FOR_NODE;
var getAliasesGjsForNode = (options) => ALIASES_GJS_FOR_NODE;
var getAliasesWebForNode = (options) => ALIASES_WEB_FOR_NODE;
var getAliasesForGjs = (options) => {
...
// src/utils/entry-points.ts
var import_fast_glob = __toESM(require_out4(), 1);
var globToEntryPoints = async (_entryPoints, ignore2 = []) => {
  if (Array.isArray(_entryPoints) && typeof _entryPoints[0] === "string") {
    return await (0, import_fast_glob.default)(_entryPoints, { ignore: ignore2 });
  } else if (Array.isArray(_entryPoints) && typeof _entryPoints[0] === "object") {
    const entryPoints2 = [];
    for (const entryPoint of _entryPoints) {
      const inputs = await (0, import_fast_glob.default)(entryPoint.in, { ignore: ignore2 });
  ...
  for (const input in _entryPoints) {
    ...
    const inputs = await (0, import_fast_glob.default)(input, { ignore: ignore2 });
    for (const input2 of inputs) {
      entryPoints[input2] = output;
    }
  }
  return entryPoints;
};
...
import { dirname as dirname2, join as join3, relative as relative2, resolve as resolve2 } from "node:path";
import { readFile } from "node:fs/promises";

// src/utils/inline-static-reads.ts
import { dirname, join as join2, resolve, basename, relative, extname } from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import { existsSync as existsSync2, readFileSync, readdirSync, statSync } from "node:fs";
function inlineStaticReads(src, sourceFilePath) {
  ...
  const outermost = [];
  edits.sort((a2, b) => a2.start - b.start || b.end - a2.end);
  for (const e of edits) {
    const last = outermost[outermost.length - 1];
    if (last && e.start >= last.start && e.end <= last.end) continue;
    outermost.push(e);
  }
  outermost.sort((a2, b) => b.start - a2.start);
  let out = src;
...
function tryInlineCall(node, ctx, src) {
  ...
  if (callee.type === "MemberExpression" && !callee.computed && callee.object.type === "Identifier" && callee.object.name === "JSON" && callee.property.type === "Identifier" && callee.property.name === "parse" && node.arguments.length >= 1 && node.arguments[0].type === "CallExpression") {
    const inner = node.arguments[0];
    const innerEdit = tryInlineReadFile(
      inner,
      ctx,
      /*forceTextEncoding*/
      true
  ...
  if (calleeName === "readFileSync") {
    const replacement = tryInlineReadFile(
      node,
      ctx,
      /*forceTextEncoding*/
      false
  ...
  if (calleeName === "readdirSync") {
    ...
    if (path6 && existsSyncSafe(path6) && isDirectorySafe(path6)) {
      try {
        const names = readdirSync(path6);
        return {
          start: node.start,
          end: node.end,
  ...
  if (calleeName === "existsSync") {
    ...
    if (path6 !== void 0) {
      return {
        ...
        replacement: existsSyncSafe(path6) ? "true" : "false"
      };
    }
  }
...
function tryInlineReadFile(node, ctx, forceTextEncoding) {
  ...
  if (!existsSyncSafe(path6) || isDirectorySafe(path6)) return void 0;
  let encoding;
  if (forceTextEncoding) {
    encoding = "utf8";
  } else if (node.arguments.length >= 2) {
    encoding = evalEncodingExpr(node.arguments[1]);
    if (encoding === void 0) return void 0;
  }
  try {
    if (encoding) {
      const text = readFileSync(path6, encoding);
      return jsStringLiteral(text);
    } else {
      const bytes = readFileSync(path6);
...
function evalExpr(node, ctx) {
  ...
  switch (node.type) {
    ...
    case "TemplateLiteral": {
      const tl = node;
      if (tl.expressions.length > 0) return void 0;
      return tl.quasis.map((q) => q.value.cooked ?? "").join("");
    }
    case "BinaryExpression": {
      const be = node;
      if (be.operator !== "+") return void 0;
      const l = evalExpr(be.left, ctx);
      const r = evalExpr(be.right, ctx);
      if (typeof l !== "string" || typeof r !== "string") return void 0;
      return l + r;
    }
    case "Identifier": {
      const id = node;
    ...
    case "MemberExpression": {
      const me = node;
      if (me.object.type === "MetaProperty" && me.object.meta.name === "import" && me.object.property.name === "meta" && me.property.type === "Identifier" && me.property.name === "url") {
    ...
    case "NewExpression": {
      ...
      if (calleeName === "URL") {
        if (ne.arguments.length === 0) return void 0;
        ...
        const base2 = evalExpr(ne.arguments[1], ctx);
        const baseStr = base2 instanceof URL ? base2.href : typeof base2 === "string" ? base2 : void 0;
        if (!baseStr) return void 0;
        try {
    ...
    case "CallExpression": {
      ...
      if (name === "fileURLToPath") {
        ...
        const url = arg instanceof URL ? arg.href : typeof arg === "string" ? arg : void 0;
      ...
      if (name === "pathToFileURL") {
        const arg = evalExpr(ce.arguments[0], ctx);
        if (typeof arg !== "string") return void 0;
        try {
      ...
      if (name === "join" || name === "resolve") {
        const args = [];
        for (const a2 of ce.arguments) {
          const v = evalExpr(a2, ctx);
          if (typeof v !== "string") return void 0;
          args.push(v);
        }
        return name === "join" ? join2(...args) : resolve(...args);
      }
      if (name === "dirname" || name === "basename" || name === "extname") {
        ...
        if (name === "basename") {
          const ext = ce.arguments.length >= 2 ? evalExpr(ce.arguments[1], ctx) : void 0;
          return basename(v, typeof ext === "string" ? ext : void 0);
        }
        if (name === "extname") return extname(v);
      }
      if (name === "relative") {
        const a2 = evalExpr(ce.arguments[0], ctx);
        const b = evalExpr(ce.arguments[1], ctx);
        if (typeof a2 !== "string" || typeof b !== "string") return void 0;
        return relative(a2, b);
      }
      return void 0;
    }
  }
...
function identifierName(node) {
  ...
  if (node.type === "MemberExpression" && !node.computed) {
    const me = node;
    if (me.property.type === "Identifier") return me.property.name;
  }
  return void 0;
}
...
function isDirectorySafe(path6) {
  try {
    return statSync(path6).isDirectory();
  } catch {
    return false;
  }
}
...
async function loadAndRewrite(args, build2) {
  ...
  return rewriteContents(args, src, getBundleDir(build2));
}
function registerNodeModulesPathRewrite(build2) {
  build2.onLoad({ filter: REWRITE_FILTER }, (args) => loadAndRewrite(args, build2));
}
...
var setupLib = async (build2, pluginOptions) => {
  ...
  const esbuildOptions = {
    ...
    minify: false,
    sourcemap: false,
    splitting: format2 === "esm" ? true : false,
    // Works only on esm
    loader: {
      ".ts": "ts",
      ".mts": "ts",
    ...
    target: ["esnext"],
    platform: "neutral",
    mainFields: format2 === "esm" ? ["module", "main"] : ["main"],
  ...
  await aliasPlugin(pluginOptions.aliases).setup(build2);
  await transformExtPlugin({ outExtension: getJsExtensions(pluginOptions.jsExtension) }).setup(build2);
};
...
function blueprintPlugin() {
  return {
    name: "blueprint",
    setup(build2) {
      build2.onResolve({ filter: /\.blp$/ }, (args) => {
        return {
          path: args.path,
          namespace: "blueprint",
      ...
      build2.onLoad({ filter: /\.blp$/, namespace: "blueprint" }, async (args) => {
        const fullPath = resolve3(args.pluginData.resolveDir, args.path);
...
// ../esbuild-plugin-css/dist/esm/index.mjs
import { build as esbuildBuild } from "esbuild";
var CSS_ASSET_LOADERS = {
  ".ttf": "dataurl",
  ".woff": "dataurl",
  ".woff2": "dataurl",
  ".otf": "dataurl",
...
function cssPlugin(options = {}) {
  return {
    name: "gjsify-css",
    setup(build2) {
      const minify = options.minify ?? build2.initialOptions.minify ?? false;
      const target = options.target ?? build2.initialOptions.target;
      build2.onLoad({ filter: /\.css$/ }, async (args) => {
        const subBuildOpts = {
          ...
          conditions: build2.initialOptions.conditions,
          resolveExtensions: build2.initialOptions.resolveExtensions,
          mainFields: build2.initialOptions.mainFields,
          preserveSymlinks: build2.initialOptions.preserveSymlinks,
          logLevel: "silent"
        };
        if (target) {
...
var setupForBrowser = async (build2, pluginOptions) => {
  ...
  await deepkitPlugin.deepkitPlugin({ reflection: pluginOptions.reflection }).setup(build2);
};
...
function registerToCommonJSPatch(build2) {
  build2.onEnd((result) => {
    if (result.errors.length > 0) return;
    const outfile = build2.initialOptions.outfile;
    if (!outfile) return;
    try {
  ...
  return ns;
        ...
        writeFileSync2(outfile, content);
      }
    } catch {
    }
  });
...
var setupForGjs = async (build2, pluginOptions) => {
  ...
  if (pluginOptions.consoleShim !== false) {
    esbuildOptions.inject = [resolve4(_shimDir, "../shims/console-gjs.js"), ...userInject];
  ...
  registerNodeModulesPathRewrite(build2);
  merge(build2.initialOptions, esbuildOptions);
  build2.initialOptions.entryPoints = await globToEntryPoints(build2.initialOptions.entryPoints, pluginOptions.exclude);
  ...
  await deepkitPlugin3.deepkitPlugin({ reflection: pluginOptions.reflection }).setup(build2);
  registerToCommonJSPatch(build2);
};
...
var setupForNode = async (build2, pluginOptions) => {
  ...
  await aliasPlugin(aliases).setup(build2);
  await cssPlugin(pluginOptions.css ?? {}).setup(build2);
  await deepkitPlugin5.deepkitPlugin({ reflection: pluginOptions.reflection }).setup(build2);
  registerToCommonJSPatch(build2);
};

// src/plugin.ts
var gjsifyPlugin = (pluginOptions = {}) => {
  const plugin = {
    name: "gjsify",
    async setup(build2) {
      if (pluginOptions.library) {
        switch (pluginOptions.library) {
          case "esm":
          case "cjs":
            return setupLib(build2, pluginOptions);
          default:
      ...
      switch (pluginOptions.app) {
        case "gjs":
          return await setupForGjs(build2, pluginOptions);
        case "node":
          return await setupForNode(build2, pluginOptions);
        case "browser":
          return await setupForBrowser(build2, pluginOptions);
        default:
...
// src/utils/auto-globals.ts
var GLOBALS_MAP2 = GJS_GLOBALS_MAP;
...
function detectedToRegisterPaths(detected) {
  ...
  for (const name of detected) {
    const path6 = GLOBALS_MAP2[name];
    if (path6) paths.add(path6);
  }
  return paths;
}
async function detectAutoGlobals(esbuildUserOptions, pluginOptions, verbose, options = {}) {
  ...
  for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
    ...
    if (registerPaths.size === 0) {
      return { detected, injectPath: void 0 };
    }
    currentInject = await writeRegisterInjectFile(registerPaths) ?? void 0;
    if (verbose) {
      const sorted = [...detected].sort();
      console.debug(
