...
export type TransformHookHandler = (
    this: NativePluginContext,
    code: string,
    id: string,
    moduleType: string,
...
export type ResolveIdHookHandler = (
    this: NativePluginContext,
    specifier: string,
...
export type RenderChunkHookHandler = (
    this: NativePluginContext,
    code: string,
    chunk: { fileName: string; name: string; isEntry: boolean },
) => string | { code: string } | null | undefined | Promise<string | { code: string } | null | undefined>;

export type AddonHookHandler = (
    this: NativePluginContext,
    chunk: { fileName: string; name: string; isEntry: boolean },
) => string | { text: string } | null | undefined | Promise<string | { text: string } | null | undefined>;
...
interface LoadArgs extends BaseEnvelope {
    hook: 'load';
    id: string;
}
interface ResolveIdArgs extends BaseEnvelope {
    hook: 'resolveId';
    specifier: string;
    importer?: string;
    isEntry: boolean;
...
interface TransformArgs extends BaseEnvelope {
    hook: 'transform';
    id: string;
    moduleType: string;
    payloadKind?: 'code';
}
interface RenderChunkArgs extends BaseEnvelope {
    hook: 'renderChunk';
    code: string;
    fileName: string;
    name: string;
...
interface AddonArgs extends BaseEnvelope {
    hook: 'banner' | 'footer' | 'intro' | 'outro';
    fileName: string;
    name: string;
    isEntry: boolean;
}
interface LifecycleArgs extends BaseEnvelope {
    hook: 'buildStart' | 'buildEnd' | 'generateBundle' | 'writeBundle' | 'closeBundle';
    error?: string;
}
...
export function bundleWithPlugins(options: BundleOptions, plugins: NativePlugin[]): Promise<BundleResult> {
            ...
            resolve(specifier, importer, opts) {
                return new Promise((resolve, reject) => {
                    ...
                    if (childId === 0) {
                        reject(
                            new Error(
                                `@gjsify/rolldown-native: ctx.resolve('${specifier}') failed — parent req_id ${reqId} unknown`,
                            ),
                        );
                        return;
                    }
    ...
    session.connect(
        'hook_requested',
        (_self: SessionInstance, hookName: string, reqId: number, pluginIndex: number, argsBytes: GLib.Bytes) => {
    ...
    function respondOk(reqId: number, value: unknown): void {
        const payload =
            value === undefined || value === null ? { kind: 'skip' as const } : { kind: 'ok' as const, value };
    ...
    function respondTransform(reqId: number, value: unknown): void {
        ...
        session.respond(
            ...
            enc(
                JSON.stringify({
                    kind: 'ok',
                    value: moduleType !== undefined ? { hasCodeBytes: true, moduleType } : { hasCodeBytes: true },
                }),
            ),
        );
    }
...
async function runHook(p: NativePlugin, hookName: string, args: HookArgs, ctx: NativePluginContext): Promise<unknown> {
    switch (hookName) {
        ...
        case 'resolveId': {
            ...
            return normalizeResolveIdResult(
                await p.resolveId.call(ctx, a.specifier, a.importer, { isEntry: a.isEntry }),
            );
        }
        case 'renderChunk': {
            if (!p.renderChunk) return null;
            const a = args as RenderChunkArgs;
            return normalizeRenderChunkResult(
                await p.renderChunk.call(ctx, a.code, { fileName: a.fileName, name: a.name, isEntry: a.isEntry }),
            );
        }
        case 'banner':
        case 'footer':
        ...
        case 'outro': {
            ...
            return normalizeAddonResult(
                await handler.call(ctx, { fileName: a.fileName, name: a.name, isEntry: a.isEntry }),
            );
        }
        case 'buildStart':
        case 'buildEnd':
