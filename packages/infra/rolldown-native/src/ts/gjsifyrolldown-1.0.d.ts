        ...
        class BundlerSession extends GObject.Object {
            ...
            connect(
                ...
                cb: (
                    ...
                    req_id: number,
                    plugin_index: number,
                    args_json: GLib.Bytes,
                ) => void,
            ): number;
            connect(signal: 'completed', cb: (self: BundlerSession, output_json: GLib.Bytes) => void): number;
            connect(signal: 'error_occurred', cb: (self: BundlerSession, message: string) => void): number;
            connect(
                signal: 'context_response',
                cb: (self: BundlerSession, child_id: number, response_json: GLib.Bytes) => void,
            ): number;
