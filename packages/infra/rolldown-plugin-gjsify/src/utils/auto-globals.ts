...
/**
 * Expand a detected-identifier set with the PRECOMPUTED transitive closure of
 * the register modules those identifiers inject (the committed
 * `register-globals-closure` map, generated by
 * `packages/infra/cli/scripts/generate-register-closure.mjs`).
 *
 * The iterative loop's passes 2..N exist only to discover which ADDITIONAL
 * globals the injected register modules' own code references — static,
 * per-register information. Seeding pass 2 with the full closure makes the
 * loop converge at the very next pass (a verification bundle), collapsing
 * ~3-5 analysis bundles to 2 for every `--globals auto` build. A stale or
 * missing closure entry is SAFE: the verification pass detects unexpected
 * growth and the loop simply continues iterating (slower + a hint to
 * regenerate).
 */
...
export async function detectAutoGlobals(
    ...
    for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
        ...
        // Fixpoint check: detection is monotonic — injecting registers can
        // only ADD references. The injected set (`detected`) may be a
        // SUPERSET of what the bundle actually references (the closure-map
        // expansion below deliberately over-approximates), so convergence is
        // "this pass found nothing OUTSIDE the injected set", not equality.
        ...
        // identifier's register module — the static answer to what passes
        // 2..N of the plain loop would re-discover one bundle at a time.
        ...
        if (iteration > 1 && verbose) {
            // Reaching a growth pass after the closure-seeded pass means the
            // committed closure map no longer covers a register module's
            // references — correctness is preserved by iterating, but builds
            // pay extra passes until the map is regenerated.
            console.debug(
