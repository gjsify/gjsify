...
//
// Two correctness constraints shape the implementation:
//
...
//     `import.meta.url` here at all and read the anchor from
//     `globalThis.__gjsifyBundleUrl`, captured by a one-line banner at byte 0 of
//     the bundle — the single point where `import.meta.url` is unambiguously the
//     bundle's own URL.
//
...
// `createRequire` anchored at the bundle URL — built lazily so the banner that
// sets `__gjsifyBundleUrl` is guaranteed to have run, and so a build that never
// hits a rewritten read pays nothing.
...
function bundleResolve(): RequireResolve {
    ...
    _resolve = createRequire(anchor).resolve;
    return _resolve;
}
...
function resolvePackageRoot(pkg: string): string {
    ...
    try {
        ...
        // Unusual layout (no node_modules segment, e.g. a linked package):
        // the package root is the nearest ancestor holding a package.json.
        // dirname(main) is a safe lower bound; callers join a subpath onto it.
        return dirname(main);
    }
}
...
function resolveSpec(spec: string): string {
    ...
    const abs = subpath ? join(root, subpath) : root;
    _cache.set(spec, abs);
    return abs;
}
