...
Controllers are paired by USB vendor:product (read from each backend's GUID), then by connection order. A difference of up to 0.02 is scaling, not disagreement: libmanette normalises the evdev range, SDL rescales to int16 first. One difference is expected and is not a control: the two name the same pad differently (`Gamepad.id`), because SDL takes the name from its mapping database and libmanette from the kernel.

Found so far (ADR 0075, Amendment 1's measurements): on an 8BitDo N30 Pro 2 SDL is the correct one of the two. libmanette 0.2 reports no Home button and no right-stick Y for it, and fires the left trigger when the right stick is pushed down: its evdev backend matches a mapping's `aN` against the kernel axis CODE rather than the N-th axis the device has, which only coincides on pads whose axis codes have no gaps.
