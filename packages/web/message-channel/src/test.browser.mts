// W3C MessageChannel + MessagePort — browser conformance tests.
//
// Validates that Firefox's native MessageChannel/MessagePort behaves the way
// our @gjsify/message-channel implementation claims. Uses browser globals only
// — no @gjsify/* imports — per AGENTS.md § Browser tests.

import { run, describe, it, expect } from '@gjsify/unit';

run({
    async MessageChannelTest() {
        ...
        await describe('MessagePort — onmessage IDL attribute', async () => {
            await it('onmessage setter wires up the message handler', async () => {
                const ch = new MessageChannel();
                const received = await new Promise<unknown>((resolve) => {
                    ch.port2.onmessage = (e) => resolve((e as MessageEvent).data);
            ...
            await it('onmessage assignment implicitly starts the port', async () => {
                const ch = new MessageChannel();
        ...
        await describe('MessagePort — async dispatch', async () => {
