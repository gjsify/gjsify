...
| adwaita-web | — | Browser Adwaita components (Custom Elements + SCSS mirroring `refs/adwaita-web/scss/`), built to `dist/adwaita-web.css` via sass, light/dark. No GJS deps. Adaptive by itself since `addBreakpoints` (ResizeObserver) — `breakpoint="max-width: 720px"` on either split view drives `collapsed`. Coverage matrix is DERIVED into STATUS.md; the roadmap section holds only the upstream partials with no counterpart |
