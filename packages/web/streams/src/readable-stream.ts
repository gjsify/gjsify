...
class ReadableStream {
    ...
    values(options: ReadableStreamIteratorOptions = {}): AsyncIterableIterator<unknown> {
        ...
        async function returnSteps(value: unknown): Promise<IteratorResult<unknown>> {
            if (state.done) return { done: true, value };
            state.done = true;
        ...
        return Object.setPrototypeOf(
            {
                next() {
                    ...
                    state.current =
                        state.current !== undefined ? state.current.then(nextSteps, nextSteps) : nextSteps();
...
class DefaultReadRequest {
    [kState]: {
        promise: Promise<ReadableStreamReadResult<unknown>>;
        resolve: ((v: ReadableStreamReadResult<unknown>) => void) | undefined;
...
function readableStreamPipeTo(
    ...
    function shutdownWithAnAction(action: () => Promise<unknown>, rejected?: boolean, originalError?: unknown) {
...
class BYOBReadIntoRequest {
    [kState]: {
        promise: Promise<{ value: ArrayBufferView | undefined; done: boolean }>;
        resolve: ((v: { value: ArrayBufferView | undefined; done: boolean }) => void) | undefined;
        reject: ((e: unknown) => void) | undefined;
    };
...
class ReadableStreamBYOBReader {
    ...
    read(
        ...
        options: { min?: number } = {},
    ): Promise<{ value: ArrayBufferView | undefined; done: boolean }> {
        if (!isReadableStreamBYOBReader(this)) return Promise.reject(new TypeError('Invalid this'));
    ...
    releaseLock(): void {
        if (!isReadableStreamBYOBReader(this)) throw new TypeError('Invalid this');
        if (this[kState].stream === undefined) return;
...
function readableByteStreamControllerRespondWithNewView(controller: ReadableByteStreamController, view: ArrayBufferView): void {
    ...
    if (state === 'closed') {
        ...
        if (byteLength === 0) {
            throw new TypeError(
                "view's length must be greater than 0 when calling respondWithNewView() on a readable stream",
            );
        }
    }
