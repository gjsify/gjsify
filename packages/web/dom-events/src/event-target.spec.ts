...
export const EventTargetTest = async () => {
    ...
    await describe('EventTarget.dispatchEvent method', async () => {
        ...
        await it('should not throw even if empty object had been added', async () => {
            const f = {};
            // Deliberately register an empty object — the spec tolerates it (the object can grow
            // a `handleEvent` later). Cast through the listener-object shape since `{}` is not
            // structurally assignable to `EventListenerObject` until `handleEvent` is added.
            target.addEventListener('foo', f as unknown as (evt: Event) => void);
            const retv = target.dispatchEvent(new Event('foo'));
            assert.strictEqual(retv, true);
        });
        ...
        await it('should call obj.handleEvent method even if added later', async () => {
            ...
            assert.strictEqual(f.handleEvent.calls.length, 1, 'handleEvent should be called');
        ...
        await it('should call a registered listener.', async () => {
            ...
            assert.strictEqual(f1.calls[0].arguments.length, 1, 'the argument of callback should be one');
            assert.strictEqual(
        ...
        await it('should handle once in nested dispatches', async () => {
            const f1 = spy(() => {
                target.dispatchEvent(new Event('foo'));
                assert.strictEqual(f2.calls.length, 1, 'f2 should be called only once');
            });
            const f2 = spy();
            target.addEventListener('foo', f1, { once: true });
            target.addEventListener('foo', f2, { once: true });
            ...
            assert.strictEqual(f1.calls.length, 1, 'f1 should be called only once');
            assert.strictEqual(f2.calls.length, 1, 'f2 should be called only once');
        });
