 ...
 */
const FACES = [
    { const: 'ADWAITA_SANS_400_CSS', file: 'files/adwaita-sans-400.ttf', style: 'normal', weight: 400 },
    { const: 'ADWAITA_SANS_400_ITALIC_CSS', file: 'files/adwaita-sans-400-italic.ttf', style: 'italic', weight: 400 },
];
...
const declarations = rules
    .map(
        (r) =>
            `/** \`@font-face\` for \`${r.file}\` (${r.style} ${r.weight}), TTF inlined as a \`data:\` URI. */\nexport const ${r.const} = ${JSON.stringify(r.css)};`,
    )
    .join('\n\n');
...
const dts = `${header}
${rules.map((r) => `/** \`@font-face\` for \`${r.file}\` (${r.style} ${r.weight}), TTF inlined as a \`data:\` URI. */\nexport declare const ${r.const}: string;`).join('\n\n')}
