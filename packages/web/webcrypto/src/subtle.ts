...
export class SubtleCrypto {
        ...
        switch (name) {
            ...
            case 'HMAC': {
                ...
                return new CryptoKey(
                    'secret',
                    extractable,
                    { name: 'HMAC', hash: { name: hashAlg.name }, length },
                    keyUsages,
                    new Uint8Array(keyData),
                );
            }
            case 'ECDH': {
                ...
                const privKey = new CryptoKey('private', extractable, { name: 'ECDH', namedCurve }, keyUsages, {
                    pub: pubBytes,
                    priv: privBytes,
                } satisfies EcKeyPairHandle);
            ...
            case 'ECDSA': {
                ...
                const privKey = new CryptoKey('private', extractable, { name: 'ECDSA', namedCurve }, ['sign'], {
                    pub: pubBytes,
                    priv: privBytes,
                } satisfies EcKeyPairHandle);
                return { publicKey: pubKey, privateKey: privKey } as CryptoKeyPair;
            ...
            case 'HMAC': {
                ...
                if (format === 'raw') {
                    ...
                    return new CryptoKey(
                        'secret',
                        extractable,
                        { name: 'HMAC', hash: { name: hashAlg.name }, length: bytes.length * 8 },
                        keyUsages,
                        new Uint8Array(bytes),
                    );
                }
            ...
            case 'ECDSA': {
                ...
                if (format === 'jwk') {
                    ...
                    if (jwk.d) {
                        ...
                        return new CryptoKey('private', extractable, { name: alg.name, namedCurve }, keyUsages, {
                            pub: pubBytes,
                            priv: privBytes,
                        } satisfies EcKeyPairHandle);
                    } else {
    ...
    async exportKey(format: 'raw' | 'jwk' | 'pkcs8' | 'spki', key: CryptoKey): Promise<ArrayBuffer | JsonWebKey> {
        if (!key.extractable) {
    ...
    private async _deriveBitsInternal(
        ...
        switch (name) {
            ...
            case 'ECDH': {
                ...
                const pubBytes =
                    publicKey._handle instanceof Uint8Array
                        ? publicKey._handle
                        : (publicKey._handle as EcKeyPairHandle).pub;
                ...
                return secretBytes.buffer.slice(
