...
// copies of the initials + colour derivation, one had silently drifted to
// two-letter initials for single-word names, and BOTH hashed UTF-16 code units
// where GLib hashes UTF-8 bytes — so every accented name got the wrong colour in
// both, and nothing in the build was in a position to notice.
//
// EVERY VECTOR CARRIES A `rule`: the C line, selector or edge case that row pins
// down, and why it is a row at all. It is also the test title, so it is what a
// failure names.
//
// WRITING A VECTOR: CITE THE SELECTOR THAT WINS THE CASCADE. A vector derived
// from the same reading of the source as the implementation cannot catch a
// MISREADING — it pins the misreading down and ships it under the word
// "conformance". So a `rule` citing CSS must name the most SPECIFIC selector that
// applies to the widget under test, and say when a less specific one is beaten.
// The incident: `SPLIT_BUTTON_DIRECTION_VECTORS` pinned `direction="none"` to
// `open-menu-symbolic`, citing `_buttons.scss:451-453` — the PLAIN `menubutton
// arrow` block. 167 lines further down, inside `splitbutton { … }`,
// `> menubutton > button > arrow.none` re-declares the glyph as
// `pan-down-symbolic` (:621-623): four element selectors plus a class against two
// plus a class, so the split button never drew the hamburger in GTK. Both
// renderers drew it anyway and the vector asserted they were right. Grep the
// whole stylesheet for the node you are citing, not just the block you found it
// in first.
//
