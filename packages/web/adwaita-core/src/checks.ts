...
//
// A checkbox is one boolean the host already holds, so per ADR 0004 it gets no core
// class. Two things about the pair are not trivial and live here:
...
//
// `GtkCheckButton` group semantics (`gtk_check_button_set_group`, whether a fresh group
// starts with a member active) and the exact meaning of `GtkCheckButton:inconsistent`
// are NOT modelled: they are GTK, `refs/gtk` is empty here and libadwaita vendors no
// `adw-checkbox.c`. A fresh group reports "nothing selected"; indeterminate is whatever
// the caller sets.
//
// The stylesheet IS vendored, and {@link resolveCheckState}'s precedence is read off
// it: `check:checked` draws `check.svg`, `radio:checked` draws `bullet.svg`, then
// `check, radio { &:indeterminate { … dash.svg } }` re-declares the glyph at EQUAL
// specificity further down the file — so checked+indeterminate draws the dash.
//
 ...
 * Indeterminate wins over checked — _checks.scss re-declares the glyph
 ...
 *
 * One registry over many groups keyed by name, because a radio knows its own group NAME
 * and not its siblings, exactly as `<input type="radio" name="…">` does. As global as
 * its host — one instance per document in `@gjsify/adwaita-web`.
 *
 * {@link select} notifies once per move carrying both halves, so a renderer repaints the
 * loser too. Re-selecting the held value is a no-op: clicking a checked radio twice must
 * not fire twice, and must never leave the group empty (HTML radios cannot be unchecked
 * by clicking).
 */
export class RadioGroupState {
    ...
    /**
     * Select `value` in group `name`, deselecting whatever was selected there. Returns
     * whether the selection actually moved; a no-op notifies nothing.
     */
