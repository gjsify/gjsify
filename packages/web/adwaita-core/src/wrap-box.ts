...
//
// The line-BREAKING engine stays out: neither renderer can be fed that decision,
// because CSS flexbox breaks lines itself and NativeScript's `FlexboxLayout`
// breaks them in native code. What IS portable is everything the engine decides
// BEFORE it measures — the property normalisers, the justify/align/last-line
// decision table, and the child-ORDER arithmetic. Both renderers map onto the
// same flex primitives (`flexDirection`, `flexWrap`, `justifyContent`,
// `alignItems`, `alignContent`, per-child grow/shrink), so both consume all three.
//
 ...
 * `g_param_spec_int (…, 0, G_MAXINT, 0, …)` (adw-wrap-box.c:285-287, :393-395). */
export const ADW_WRAP_BOX_DEFAULT_SPACING = 0;

/** `Adw.WrapBox:align` default — `g_param_spec_float (…, 0, 1, 0, …)`, so GObject
 * clamps back into `[0, 1]` on set (adw-wrap-box.c:335-337). */
 ...
 * (adw-wrap-box.c:438-441). Not "zero length": the box asks for whatever its
 ...
 * (adw-wrap-box.c:300-305, :408-413, :454-458). DIFFERS from the split views' `sp`
 * (`DEFAULT_SIDEBAR_WIDTH_UNIT`); sharing `normalizeLengthUnit` is only safe
 * because the default is the caller's argument, not the helper's. */
...
/** `adw_wrap_box_set_child_spacing` / `set_line_spacing` (adw-wrap-box.c:587-588,
 * :927-928) — a negative spacing is clamped to 0 before it can reach the layout.
...
/** `Adw.WrapBox:natural-line-length` — an `int` in `[-1, G_MAXINT]`. Anything
 * below -1 is out of range and lands on the sentinel rather than on 0: a GObject
 * setter would have refused it, and "unset" is where the property started. */
 ...
 * measure and allocate paths both make (adw-wrap-layout.c:554-565, :798-804). The
 * UNSET natural line length passes through untouched, mirroring the C's own
 * `if (self->natural_line_length >= 0)` gate (:562): converting `-1` would turn a
 * sentinel into a length, and at dpi ≠ 96 would not even leave it -1. */
...
export interface WrapBoxLineLayout {
     ...
     * `available_size`, adw-wrap-layout.c:336-341). CSS: `flex-grow` on the items. */
    growChildren: boolean;
    /** Whether the GAPS absorb it instead — children keep `minimum_size` and are
     * spread by `size_delta` (adw-wrap-layout.c:338-339, :752-757). CSS:
     ...
     * (adw-wrap-layout.c:717-725), 0 = line start, 1 = line end. Reported as 0 once
     * the line is justified, because C computes `length_delta` only inside
     * `if (!justify_line)`. */
    align: number;
}
 ...
 * lone child is STRETCHED instead (adw-wrap-box.c:349-352).
 *
 ...
 * HYPOTHETICAL main sizes and `flex-shrink` only distributes negative free space
 * WITHIN an already-broken line, so no value of it packs more children per line —
 * which is what `minimum` is for. It does faithfully decide whether an
 * overflowing line squeezes its children or lets them spill.
 *
 * Deriving it is still worth it for the DEFAULT: CSS defaults `flex-shrink` to 1
 * (= `minimum`) where libadwaita defaults `wrap-policy` to `natural`, so a
 * renderer with no policy silently draws the one Adwaita does not choose.
 */
 ...
 * (adw-wrap-box.c:1283-1300, :1315-1332), whose rule is counter-intuitive: a NULL
 * sibling inserts at the FIRST position, not the last. Reading it as "append" is
 * silent — the child lands somewhere plausible.
 *
 * MODIFICATION: the C's `g_return_if_fail`s become a refusal (`null`) rather than
 * a throw, because a renderer has no `g_critical` to fall through to; the caller
 * then leaves its view tree untouched, as C does. Insert demands an UNPARENTED
 * child and a sibling of this box, reorder demands the child already be one, and
 * `child === sibling` returns early in both.
 */
