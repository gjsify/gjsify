...
//
// `Adw.Sidebar` is a list of `AdwSidebarSection`s of `AdwSidebarItem`s with zero or one
// selected item. Almost nothing about that is rendering: the flat item index space, the
// out-of-range rule, how a selection survives (or does not survive) an insert/remove, which
// section headers actually get drawn, and the selection-vs-activation split are all pure
// derivations off the C source, lifted here so every renderer answers them the same way.
//
// The rule most likely to be got wrong: setting `selected` to 5 on a 3-item sidebar leaves
// NO SELECTION — it neither clamps to the last row nor ignores the write.
// `conformance/sidebar.ts` holds the vectors.
//
// PLATFORM-NEUTRAL: renders nothing, reads no global, starts no timer.
// {@link SidebarState} exposes the same per-instance subscribe/emit seam as
// `ComboState`/`ToggleGroupState` in `rows.ts` and tags every change with `interactive` —
// true only for {@link SidebarState.activate} (a row click) — so a renderer knows when to
// re-emit `notify::selected` and when to only repaint.
//
 ...
 * (`adw_sidebar_init`).
 *
 * libadwaita spells it `G_MAXUINT` because `AdwSidebar:selected` is a `guint`; `-1` is the
 * idiomatic TS sentinel and, being `G_MAXUINT` mod 2^32, it makes
 * {@link adjustSidebarSelection}'s arithmetic agree with the C bit for bit.
 */
export const ADW_SIDEBAR_NO_SELECTION = -1;

/**
 * How a sidebar looks and behaves — `AdwSidebarMode`. `'sidebar'` is a flat navigation list
 * with a persistent, VISIBLE selection; `'page'` is a page of boxed lists where "the
 * selection is invisible and only tracked to determine the initially selected item once
 * switched back". The tracked index is identical in both.
 */
 ...
 * `'title'` one (`update_header_page_cb`).
 */
...
export interface SidebarFlatItem {
    /**
     * The flat index `adw_sidebar_item_get_index` returns — the section's first index plus the
     * local one. It stays in the UNFILTERED space even when the item came out of a filtered
     * walk, because libadwaita keeps the selection on `items_model` and the row list on
     * `filtered_items`.
     */
    index: number;
    ...
    /** `string_is_not_empty(title)` — the title label's visibility. */
...
export interface SidebarHeaderSpec {
    ...
    sectionIndex: number;
    /** Which page of the header stack shows — `'title'` or `'separator'`. */
    kind: 'title' | 'separator';
    /** The section title (`''` for a separator header). */
    title: string;
    /** True only for the section owning the FIRST rendered row — the `.first` class. */
    first: boolean;
}
...
export interface SidebarActivation {
    ...
    index: number;
    /**
     * Whether a row was actually activated. False when `index` addresses no rendered, enabled
     * row — libadwaita's callbacks run off an existing, sensitive `GtkListBoxRow`.
     */
    activated: boolean;
...
/** Per-instance subscriber, same shape as `ComboStateListener` (rows.ts). */
export type SidebarStateListener = (change: SidebarSelectionChange) => void;

/**
 * The TS stand-in for `GtkFilter` on `AdwSidebar:filter`.
 *
...
/**
 * The out-of-range rule: anything that is not a valid position becomes NO SELECTION — never
 * the nearest in-range index. `adw_sidebar_set_selected` is literally
 * `if (selected >= self->n_items) selected = GTK_INVALID_LIST_POSITION;`, and a negative
 * input agrees with that for free: as a `guint`, `-7` is `4294967289`, which is
 * `>= n_items` for any real sidebar.
 *
 * Non-integers and `NaN` are rejected rather than truncated. C cannot express them, so
 * there is no ground truth to copy; rejecting matches "not a valid position is no
 * position", and it stops `sidebar.selected = 1.5` from highlighting no row while still
 ...
 *   1. `0 → n` auto-selects index 0 and notifies;
 *   2. a splice covering the selected index clears it;
 *   3. a splice at or before it shifts by `added - removed`;
 *   4. a splice after it changes nothing.
 *
 * The comparisons run in the UNSIGNED domain (`>>> 0`) because that is where C runs them:
 * with no selection, `selected` is `G_MAXUINT`, so case 2 can never fire and case 3 wraps.
 * That wraparound is REPRODUCED, not repaired — inserting two items above a selection-less
 * sidebar genuinely selects index 1 in GTK.
 */
...
/**
 * Flatten the sections into the item model, the way `items_changed_cb`'s section walk does:
 * `set_first_index (section, current); current += get_n_items (section)`.
 *
 * `sectionFirstIndex[i]` is `adw_sidebar_section_get_first_index` for section `i` — a
 * zero-item section advances nothing, so it shares its successor's index. With a `filter`,
 * `items` is the RENDERED subset while every `index` (and `sectionFirstIndex`) stays in the
 * unfiltered space.
 */
 ...
 *
 * In sidebar mode a header is a property OF A ROW (`gtk_list_box_row_set_header`), so a
 * section with no rows contributes none; in page mode the group's `visible` is bound to
 * `filtered n-items > 0`, which comes to the same thing. Either way it is the sections that
 * RENDER something that produce headers, NOT the declared section list — deriving from the
 * declared list draws a stray separator for an empty leading section.
 *
 * The section owning the first rendered row gets `first: true`; when that section has no
 * title its header is bound to `string_is_not_empty(title)` and therefore invisible, so it
 * is omitted entirely.
 */
 ...
 *
 * Programmatic changes ({@link setSelected}, {@link setSections}, {@link insertSection},
 * {@link removeSectionAt}, {@link removeAllSections}) notify with `interactive: false`;
 * {@link activate} — the row click — notifies with `interactive: true` and applies the
 * selection BEFORE reporting the activation, matching GtkListBox's
 * `row-selected`-then-`row-activated` order and page mode's `set_selected`-then-emit.
 */
export class SidebarState {
    ...
    /** Store an already-derived index; notify only on a real change. */
    ...
    /** The current look/behaviour. Defaults to `'sidebar'`. */
     ...
     * on the current value. Never touches the selection.
     */
    ...
    /**
     * Whether the selection is PAINTED. Page mode tracks the selection but shows no selected
     * row: its rows are plain `AdwActionRow`s in a boxed list (`create_boxed_row`) and the
     * mode is documented as "the selection is invisible, but still tracked".
     */
     ...
     *
     * Equivalent to `adw_sidebar_remove_all()` followed by appending each section, i.e. two
     * splices: everything removed (which clears the selection) and everything added (which
     * re-runs the `0 → n` auto-select of index 0). Coalesced into ONE notification, because
     * one property write should produce one change.
     */
    ...
    /**
     * Insert a section, returning the index it landed at. `adw_sidebar_insert` appends when
     * `position` is negative OR at/past the end — so on a two-section sidebar, positions `-1`,
    ...
    /** `adw_sidebar_remove_all` — one splice removing every item. */
     ...
     *
     * libadwaita needs no such call: title, subtitle and icon are live
     * `g_object_bind_property` bindings straight onto the labels and `notify::icon-name` goes
     * to `notify_icon_cb` — none of them runs through `items_changed_cb`, so the selection
     * cannot move. Same contract here for renderers that hand `SidebarState` mutable specs:
     * the derived flags are recomputed, the selection is untouched, nothing is emitted.
     */
    refresh(): void {
        this._invalidate();
    }

    /** `adw_sidebar_get_section` — `undefined` past the end. */
    ...
    /** Number of items in the UNFILTERED model — the selection index space. */
    get count(): number {
    ...
    /** `adw_sidebar_get_item` — `undefined` for `index >= n_items`. */
    ...
    /**
     * Set the item filter — `adw_sidebar_set_filter`. It only swaps the filter on the
     * `GtkFilterListModel`; the selection is never re-derived, so a selected item that filters
     * out stays selected.
     */
    ...
    /**
     * The `.empty` / placeholder state — `update_placeholder` counts the FILTERED model, so a
     * fully filtered-out sidebar is empty even though its selection index space is not.
     */
    get isEmpty(): boolean {
    ...
    /**
     * A row click. Selects (notifying `interactive: true` if that moved) and then reports the
     * activation, which libadwaita emits on EVERY click including a re-click of the
     * already-selected row (`row_activated_cb`). That unconditional emit is how re-tapping the
     * current row reveals the content pane of a split view.
     *
     * A row that is filtered out, `visible: false`, or `enabled: false` is not activatable: it
     * either has no `GtkListBoxRow` or has an insensitive one.
     */
