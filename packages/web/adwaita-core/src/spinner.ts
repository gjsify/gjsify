...
// fits a whole number of turns. Do not copy `_spinner.scss` instead: its fixed 90°
// `border-top-color` chase at 0.8s is a different animation with a different period.
//
// The DRAWN arc spans 2.7° to 102.8°, not to `MAX_ARC_LENGTH`'s 162°: both ends lerp
// towards it and then subtract the same drift term, which is what advances the
// figure. `SPINNER_ARC_ENVELOPE` in the conformance table is the measured answer —
// reading the constant as the drawn length is the obvious mistake.
//
// Every number here is a `#define` in the C, exported rather than inlined so that
// the renderer's per-frame geometry and the vectors that judge it read one copy.
//
...
/** `SPIN_DURATION_MS` — one full turn. */
export const ADW_SPINNER_SPIN_DURATION_MS = 1200;

/**
 * `START_ANGLE` — a fixed phase offset added to both arc ends, so the figure does not
 * start at 3 o'clock. Omitting it is a constant rotation error rather than a wrong
...
/**
 * `CIRCLE_OPACITY` — the track is the WIDGET's colour at 15%, not a hardcoded grey,
 * so a spinner on a coloured background keeps its track relationship.
 */
export const ADW_SPINNER_TRACK_OPACITY = 0.15;

/** `MIN_ARC_LENGTH` — GSK refuses to draw an arc shorter than this. */
export const ADW_SPINNER_MIN_ARC_LENGTH = Math.PI * 0.015;

/** `MAX_ARC_LENGTH` — the LERP TARGET, 162°. The drawn arc peaks at 102.8°. */
...
/**
 * `N_CYCLES` — how many full TURNS the animation runs before repeating.
 *
 * The constraint the C states: the cycle length `IDLE + EXTEND + CONTRACT - OVERLAP`
 * must divide `N_CYCLES * 2pi` exactly, or the arc jumps when the animation loops. It
 * does — 53 turns is 40 arc cycles — and {@link ADW_SPINNER_CYCLES_PER_LOOP} is that
 * quotient, asserted rather than assumed.
 */
 ...
 * OVERLAP_DISTANCE / 2`, i.e. `pi * 0.75`. Reached whenever there is no animation
 * object: before the paintable has a widget, and in a still frame.
 */
export const ADW_SPINNER_STILL_PROGRESS = ADW_SPINNER_EXTEND_DISTANCE - ADW_SPINNER_OVERLAP_DISTANCE / 2;

/**
 * `normalize_angle` — fold into `[0, 2pi]`. The C loops rather than taking a modulo,
 * and the boundary follows: it stops at `angle > G_PI * 2`, so exactly `2pi` is left
 * alone where a modulo would give 0.
 */
...
/**
 * `get_arc_start` — the trailing end of the arc, relative to the base angle. The
 * `- angle * MAX_ARC_LENGTH / l` term is the drift that makes the arc advance around
 * the circle as it breathes; without it the figure would pulse in place.
 */
...
/**
 * `get_arc_end` — the leading end. Its phase is offset by
 * `EXTEND_DISTANCE - OVERLAP_DISTANCE`: the end only starts moving once the start has
 * extended past the overlap, which is what makes the arc grow before it slides.
 */
...
export interface SpinnerArc {
    ...
    /**
     * Arc length along the circle, `start - end` unwrapped into `(0, 2pi]`. The segment
     * runs from `end` to `start` (`gsk_path_builder_add_segment (builder, circle,
     * &end_point, &start_point)`) — the naming is the C's and it is the opposite way
     * round from what "start" suggests.
     */
    length: number;
}
 ...
 *
 * `progress` is the animation's value: 0 at the start of a loop, rising linearly to
 * `N_CYCLES * 2pi`. A renderer with no animation passes
 ...
 * N_CYCLES * 2pi, SPIN_DURATION_MS * N_CYCLES, target)` with `ADW_LINEAR` easing and
 * `repeat_count = 0`, i.e. forever. A linear ramp over the whole loop reduces to one
 * turn per {@link ADW_SPINNER_SPIN_DURATION_MS}; the loop length only decides where it
