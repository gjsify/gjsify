# @gjsify/gamepad-native-linux-arm64

> **No artifact in this tarball yet.** `linux-arm64` is declared by
> `@gjsify/gamepad-native` and built by CI, but not committed to the repository:
> prebuilds.yml's `build-prebuilds` job (linux-arm64 leg) builds and tests it; `commit-prebuilds` lands the first artifact on main after this package merges
>
...
The **linux-arm64** native artifacts of [`@gjsify/gamepad-native`](https://www.npmjs.com/package/@gjsify/gamepad-native) — a shared
...
You do not install this directly. `@gjsify/gamepad-native` declares it as an
