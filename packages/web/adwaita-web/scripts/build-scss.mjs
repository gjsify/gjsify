...
// is generated from, so supply a matching 3-column glyph in the same symbolic
// style (used by the Toggle Group story's view-mode toggles).
...
//
// This is a build INTERMEDIATE (input to the TypeScript-compiling consumer +
// build:types), NOT build output: it is regenerated from the compiled CSS on
// every build and kept in `src/` (gitignored) so `src/index.ts` can import it.
// It emits ONLY the `.ts` — an earlier `.js` sibling written here leaked build
// output into `src/` and, combined with the old `--library` build that also
// emitted alongside sources, produced the duplicate `styles.generated{,2}.js`
// (and the `adw-*{,2}.js` element duplicates) the bundler saw once both the
// `.ts` source and a co-located `.js` existed.
