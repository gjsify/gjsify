// <adw-checkbox> and <adw-radio> — the two form-control primitives, in ONE
// module, for the same reason `scss/_checks.scss` is one partial: upstream
// merges them (`refs/libadwaita/src/stylesheet/widgets/_checks.scss` has no
// `_radio.scss` beside it), and everything but the corner radius, the glyph and
// the group is shared. `refs/adwaita-web` split them into two files and the two
// promptly drifted apart.
//
// WHAT LIVES IN `@gjsify/adwaita-core`, AND WHAT DOES NOT
//
// The checkbox has no core class: it is one boolean plus a third state, which
// ADR 0004 calls out as too trivial to model. What IS core is the pair of
// pieces a second renderer would otherwise re-invent:
//
...
//     unchecks the sibling INPUT and leaves the sibling `<adw-radio>` host's
//     `checked` attribute — the published state, and the selector
//     `_checks.scss` paints from — stale, so a group left to the browser draws
//     two selected radios. The shared state is what repaints the loser.
//   - {@link resolveCheckState} — indeterminate outranks checked, which is read
//     off the cascade in `_checks.scss:79-81` and is what `aria-checked` needs.
//
// WHAT IS NOT VERIFIABLE IN THIS TREE. `GtkCheckButton` is a GTK widget:
// `refs/gtk` is EMPTY here and libadwaita vendors no `adw-checkbox.c`, so its
// group semantics (`gtk_check_button_set_group`, whether a fresh group starts
// with a member active) and the exact meaning of `GtkCheckButton:inconsistent`
// are NOT verifiable and are not guessed at. Where a rule was needed the HTML
// one is used and cited as such — clearing `indeterminate` on activation is the
// HTML pre-click activation step, not a reading of GTK.
//
 ...
 * `<input type="radio" name="x">` outside a form is — two independent groups
 * therefore need two names.
 */
...
abstract class AdwCheckBase extends HTMLElement {
    ...
    set checked(value: boolean) {
        // `toggleAttribute` with a force flag is a no-op when the attribute
        // already holds that state, so re-setting the current value mutates
        // nothing and the notify never runs.
    ...
    /** The label text beside the indicator. */
    get label(): string {
    ...
    connectedCallback() {
        ...
        // Captured BEFORE the element takes over its own children: a
        // server-rendered `<adw-checkbox>Enable</adw-checkbox>` keeps its text.
        ...
        // The indicator and the label cover the control; the input is 0x0, so
        // every pointer click lands on one of them. A click that DID reach the
        // input is already handled above — activating again would undo it.
    ...
    /** Extra wiring for a subclass on connect. */
    protected onConnected(): void {}

    /** Extra teardown for a subclass on disconnect. */
    ...
    protected render(): void {
        ...
        // Explicit, because HTML has a native mixed CHECKBOX (the property
        // above) but no mixed radio — this is the one spelling that announces
        // the third state for both.
...
export class AdwCheckbox extends AdwCheckBase {
    ...
    protected activate(): void {
        // HTML's pre-click activation steps clear `indeterminate` before the
        // toggle, so a user can always reach a definite state. That rule is the
        // HTML spec's, not a reading of GtkCheckButton:inconsistent, whose
        // behaviour is not verifiable in this tree.
...
export class AdwRadio extends AdwCheckBase {
    ...
    /**
     * This member's value. Falls back to the label text so a group authored
     * without values still has members the group state can tell apart.
     */
    get value(): string {
    ...
    protected activate(): void {
        this.indeterminate = false;
        // An UNNAMED radio is not in a group — HTML forms a radio button group
        // only from a non-empty `name`, so a nameless one checks itself and
        // never unchecks anything. Without this guard every nameless radio in
        // the document would join one `''` group and fight over it.
        if (this.name === '') {
            this.checked = true;
            return;
        }
        // The pick goes through the shared state, never straight onto this
        // element: the member that LOSES the selection is a different element
        // and only the state knows which one it is.
