...
function renderRow(row, columnWidths, columnRightAlign?: boolean[]) {
...
function cliTable(head, columns) {
  ...
  for (let i = 0; i < head.length; i++) {
    ...
    for (let j = 0; j < longestColumn; j++) {
      ...
      columnRightAlign[i] &= canRightAlign(value) as any;
    }
  }
...
export function quoteString(string: string) {
  ...
  string = StringPrototypeReplace(string, escapePattern, "\\" as any);
      ...
      ArrayPrototypeMap(
        ...
        (error) =>
          StringPrototypeReplace(
            inspectArgs([error]),
            /^(?!\s*$)/gm,
            StringPrototypeRepeat(" ", 4) as any,
          ),
      ),
      "\n",
    );
...
export function cssToAnsi(css, prevCss = null) {
...
export function inspectArgs(args, inspectOptions = {}) {
  circular = undefined;
