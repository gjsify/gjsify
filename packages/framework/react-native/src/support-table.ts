...
export const EXPO_FONT_TABLE: Readonly<Record<string, SupportEntry>> = {
    useFonts: {
        ...
        limits: [
            ...
            'The map’s VALUES are ignored: a `require("./Inter.ttf")` id is an index into React Native’s asset registry, which ADR 0032 § 12 leaves to the consumer’s build chain, and this layer refuses one for `Image.source` for the same reason. Ship the face with the application and name the FAMILY in your styles: `gjsify.ship.fonts` stages it into `share/fonts/<appId>/`, and each OS reaches it differently (ADR 0037) — fontconfig scans that directory on Linux (measured), `ATSApplicationFontsPath` in the bundle’s Info.plist is documented to activate it on macOS (Apple’s key reference; unverified on hardware here), and on Windows nothing reaches it declaratively at all, so the launcher exports GJSIFY_FONT_DIR and the app registers the faces itself with `PangoCairo.FontMap.get_default().add_font_file()`.',
