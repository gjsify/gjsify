...
export const SUPPORT_TABLE: Readonly<Record<string, SupportEntry>> = {
    ...
    Platform: { status: 'planned', tier: 'P1', reason: 'OS is "linux" | "macos" | "windows"; select() picks the default branch.' },
    Modal: { status: 'planned', tier: 'P1', gtk: 'Adw.Dialog', reason: 'A dialog rather than a full-screen overlay, which is what a desktop expects.' },
    useColorScheme: { status: 'planned', tier: 'P1', gtk: 'Adw.StyleManager.dark', reason: 'Follows the Adwaita colour scheme.' },
    Share: {
        status: 'planned',
        tier: 'P1',
    ...
    SectionList: { status: 'planned', tier: 'P2', gtk: 'Gtk.ListView + a section model', reason: 'Sections map onto GTK section models.' },
    VirtualizedList: {
        status: 'planned',
        tier: 'P2',
        gtk: 'Gtk.ListView',
    ...
    VirtualizedSectionList: { status: 'planned', tier: 'P2', gtk: 'Gtk.ListView', reason: 'The section-shaped sibling of VirtualizedList.' },
    Image: { status: 'planned', tier: 'P2', gtk: 'Gtk.Picture / Gdk.Texture', reason: 'resizeMode becomes content-fit.' },
    ImageBackground: { status: 'planned', tier: 'P2', gtk: 'Gtk.Picture in a Gtk.Overlay', reason: 'A picture with children over it.' },
    TouchableOpacity: { status: 'planned', tier: 'P2', gtk: 'Gtk.Button (flat)', reason: 'The same machinery as Pressable, and nearly free once it exists.' },
    TouchableHighlight: { status: 'planned', tier: 'P2', gtk: 'Gtk.Button (flat)', reason: 'As TouchableOpacity, with a different pressed style.' },
    TouchableWithoutFeedback: { status: 'planned', tier: 'P2', gtk: 'Gtk.GestureClick', reason: 'A gesture controller on the child, with no button chrome.' },
    Button: { status: 'planned', tier: 'P2', gtk: 'Gtk.Button', reason: 'The one component whose React Native styling story is "you cannot", which GTK agrees with.' },
    Dimensions: { status: 'planned', tier: 'P2', gtk: 'Gdk.Surface', reason: 'Window size, not screen size — a desktop app is not full-screen.' },
    useWindowDimensions: { status: 'planned', tier: 'P2', gtk: 'Gdk.Surface', reason: 'The hook form of Dimensions, re-rendering on resize.' },
    Alert: { status: 'planned', tier: 'P2', gtk: 'Adw.AlertDialog', reason: 'Direct counterpart.' },
    Appearance: { status: 'planned', tier: 'P2', gtk: 'Adw.StyleManager', reason: 'The imperative sibling of useColorScheme.' },
    SafeAreaView: {
    ...
    KeyboardAvoidingView: { status: 'no-desktop-meaning', tier: 'P2', reason: 'No on-screen keyboard eats a desktop window layout.' },
    Keyboard: { status: 'no-desktop-meaning', tier: 'P2', reason: 'Its events are on-screen-keyboard events, which do not occur.' },
    RefreshControl: {
        status: 'refused',
        reason: 'GTK has no pull-to-refresh idiom and should not grow one. Give the desktop build a refresh action instead.',
    },
    AppState: { status: 'planned', tier: 'P3', gtk: 'Gtk.Application / Gdk.Surface state', reason: 'active/background from window focus and visibility.' },
    PixelRatio: { status: 'planned', tier: 'P3', gtk: 'Gdk.Surface.scale-factor', reason: 'The scale factor of the surface the widget is on.' },
    PlatformColor: {
        status: 'planned',
        tier: 'P3',
    ...
    LayoutAnimation: { status: 'planned', tier: 'P3', reason: 'Needs an animated layout pass, which is the same subsystem as Animated.' },
    InteractionManager: { status: 'planned', tier: 'P3', reason: 'Deferring work until interactions settle; a main-loop idle source is the counterpart.' },
    ...
    usePressability: { status: 'planned', tier: 'P3', reason: 'The hook behind the Touchable family; it lands with the gesture work.' },
    DrawerLayoutAndroid: {
        status: 'planned',
        tier: 'P3',
    ...
    I18nManager: { status: 'planned', tier: 'P3', gtk: 'Gtk.Widget.direction', reason: 'RTL is a widget-direction question on GTK.' },
    LogBox: { status: 'no-desktop-meaning', tier: 'P3', reason: 'A development overlay for a phone; the console is the desktop equivalent.' },
    ...
    ToastAndroid: { status: 'planned', tier: 'P3', gtk: 'Adw.Toast', reason: 'Android-only by name, and Adwaita has the exact widget.' },
    InputAccessoryView: { status: 'no-desktop-meaning', reason: 'An iOS keyboard accessory bar; there is no keyboard to accessorise.' },
    TouchableNativeFeedback: { status: 'planned', tier: 'P3', gtk: 'Gtk.Button', reason: 'Android-only ripple; shimmed to its portable sibling.' },
    ActionSheetIOS: { status: 'planned', tier: 'P3', gtk: 'Adw.AlertDialog with responses', reason: 'iOS-only by name; the pattern is a dialog with choices.' },
    PushNotificationIOS: { status: 'refused', reason: 'Platform notification plumbing. A desktop app uses Gio.Notification directly.' },
    PermissionsAndroid: { status: 'refused', reason: 'Android runtime permissions have no desktop counterpart; portals are asked for at use time.' },
    Settings: { status: 'refused', reason: 'An iOS user-defaults bridge. GSettings is the desktop answer and is not this API.' },
    Clipboard: { status: 'planned', tier: 'P3', gtk: 'Gdk.Clipboard', reason: 'Deprecated upstream in favour of a community package, but trivial here.' },
    BackHandler: { status: 'planned', tier: 'P3', reason: 'An Android hardware back button. Maps to the navigation view’s own back, so it lands with routing.' },
    DynamicColorIOS: { status: 'planned', tier: 'P3', reason: 'A light/dark colour pair; the Adwaita scheme already provides the switch.' },
    processColor: { status: 'planned', tier: 'P3', reason: 'Colour string to a platform value; a Gdk.RGBA on this side.' },
    UTFSequence: { status: 'planned', tier: 'P3', reason: 'A table of unicode constants. Pure data, no platform in it.' },
    experimental_LayoutConformance: { status: 'refused', reason: 'An experimental switch between two React Native layout implementations. Neither is used here.' },
    ...
    ReactNativeVersion: { status: 'planned', tier: 'P3', reason: 'The version this layer targets, reported honestly rather than spoofed.' },
    ...
    NativeModules: { status: 'refused', reason: 'The native-module bridge. There is no bridge here; this layer renders in-process onto GTK.' },
    TurboModuleRegistry: { status: 'refused', reason: 'The TurboModule lookup. Same reason as NativeModules.' },
    NativeComponentRegistry: { status: 'refused', reason: 'Fabric component registration. The host owns widget creation.' },
    requireNativeComponent: { status: 'refused', reason: 'Looks up a native view manager. Write a GTK widget descriptor instead.' },
    ...
    findNodeHandle: { status: 'refused', reason: 'Returns a native view tag. A ref here is the Gtk.Widget itself, which is more useful.' },
    registerCallableModule: { status: 'refused', reason: 'Registers a module callable from the native side. There is no native side.' },
    Networking: { status: 'refused', reason: 'React Native’s XHR internals. Use fetch, which gjsify provides.' },
    NativeDialogManagerAndroid: { status: 'refused', reason: 'An Android dialog native module. Alert is the portable spelling.' },
    ProgressBarAndroid: { status: 'planned', tier: 'P3', gtk: 'Gtk.ProgressBar', reason: 'Android-only by name; GTK has the widget.' },
    ...
    RootTagContext: { status: 'refused', reason: 'A React Native surface identifier. This layer has one root per Adw window.' },
