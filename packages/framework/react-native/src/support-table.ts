...
export const ROUTER_SUPPORT_TABLE: Readonly<Record<string, SupportEntry>> = {
    ...
    Tabs: {
        ...
        limits: [
            'A tab answers title and iconName. tabBarIcon stays refused: in React Native it is a COMPONENT, and Adwaita paints a switcher button from Adw.ViewStackPage:icon-name — an icon-theme NAME, on the page object the add returns. iconName is that name. It is not decoration: the switcher reserves the icon either way (MEASURED, identical widths with and without), so a page without one draws the missing-image glyph.',
            ...
            'The switcher sits in the header bar with policy WIDE (MEASURED: the default is NARROW, the phone-shaped one) and moves to an Adw.ViewSwitcherBar at the bottom when the window is too narrow to show it — Adwaita’s own adaptive answer, and the tab bar on a Linux phone. The threshold is not a written-in number: it is the header bar’s own natural width while it holds the switcher, measured, because the width where labels start being ellipsised is the application’s labels plus that user’s window controls.',
