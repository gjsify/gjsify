...
const VIEW_WIDGETS: readonly { readonly gtype: string; readonly ctor: { list_properties(): GObject.ParamSpec[] } }[] = [
    { gtype: 'GtkBox', ctor: Gtk.Box as never },
    { gtype: 'GtkOverlay', ctor: Gtk.Overlay as never },
];
        ...
        await gated('the animated property table, against the installed typelib', async () => {
            await it('names a property every widget a View can become really installs', async () => {
                ...
                for (const [key, spec] of Object.entries(ANIMATED_PROPERTIES)) {
                    for (const widget of VIEW_WIDGETS) {
                        const found = widget.ctor.list_properties().find((p) => p.get_name() === spec.property);
                        if (found === undefined) {
                ...
                // Not vacuous: an empty table satisfies an empty problem list.
