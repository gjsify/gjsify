...
function reconstructFromPlaceholder(p: BinaryPlaceholder): ArrayBuffer | ArrayBufferView {
    ...
    switch (p.type) {
        ...
        case 'BigUint64Array':
            return new BigUint64Array(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength));
        case 'BigInt64Array':
            return new BigInt64Array(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength));
        case 'Float32Array':
