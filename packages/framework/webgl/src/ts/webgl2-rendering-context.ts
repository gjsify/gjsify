...
import type { WebGLBuffer as OurWebGLBuffer } from './webgl-buffer.js';
...
export class WebGL2RenderingContext extends WebGLContextBase implements WebGL2RenderingContext {
    ...
    override bindBuffer(target: GLenum, buffer: WebGLBuffer | null): void {
        ...
        super.bindBuffer(target, buffer as unknown as OurWebGLBuffer);
    }
    ...
    override bufferData(target: GLenum, dataOrSize: GLsizeiptr | BufferSource | null, usage: GLenum): void {
        ...
        if (isWebGL2Target) {
            ...
            if (typeof dataOrSize === 'number') {
                ...
                const u8Data = arrayToUint8Array(dataOrSize as Uint8Array | DataView | ArrayBuffer);
                this._gl.bufferData(target, Uint8ArrayToVariant(u8Data), remappedUsage);
            }
            return;
        }

        // Both overloads of `super.bufferData` exist on the prototype; we know
        // either branch (size:number / data:BufferSource) is valid, so dispatch through
        // the size overload — TS demands one signature.
        super.bufferData(target, dataOrSize as BufferSource, remappedUsage);
    }
    ...
    override bindTexture(target: GLenum, texture: WebGLTexture | null): void {
        if (target === 0x806f /* TEXTURE_3D */ || target === 0x8c1a /* TEXTURE_2D_ARRAY */) {
            ...
            if (ourTex) ourTex._binding = target;
            return;
        }
        super.bindTexture(target, texture as unknown as WebGLTexture | null);
    }
...
import { installTexImage3DMethods } from './webgl2-context/tex-image-3d.js';
import './webgl2-context/uniform.js';
import { installUniformMethods } from './webgl2-context/uniform.js';
import './webgl2-context/clear-buffer.js';
import { installClearBufferMethods } from './webgl2-context/clear-buffer.js';
import './webgl2-context/instanced-and-framebuffer-ops.js';
import { installInstancedAndFramebufferOpsMethods } from './webgl2-context/instanced-and-framebuffer-ops.js';
installObjectLifecycleMethods(WebGL2RenderingContext.prototype);
installTexImage3DMethods(WebGL2RenderingContext.prototype);
installUniformMethods(WebGL2RenderingContext.prototype);
installClearBufferMethods(WebGL2RenderingContext.prototype);
installInstancedAndFramebufferOpsMethods(WebGL2RenderingContext.prototype);
