...
// `system-accent.spec.ts` is here because it imports `gi://Gio` and `gi://GLib`,
// which the node bundle cannot resolve either. None needs a DISPLAY though — see
// those files' headers — so all run wherever gjs and gtk4 are installed, CI included.
...
run({ aboutDialogSuite, iconThemeSuite, systemAccentSuite });
