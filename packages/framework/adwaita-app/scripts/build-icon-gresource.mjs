...
// EMITTED ALREADY FORMATTED, one name per line in single quotes, because this artifact is
// COMMITTED and `Whole-tree checks` runs `oxfmt --check` over every committed source. A
// `JSON.stringify` of the name list is one long double-quoted line, which is what turned that
// gate red the first time. The alternative — shelling out to oxfmt here — would put a second
// binary on a build step that has to run on the Windows leg of `gtk-os-suites.yml`, which is
// the dependency `glib-compile-resources` already taught us not to add. So the emitter matches
// the formatter's shape instead, and `oxfmt --check` in CI is the backstop if that shape moves.
