...
//
// THE `Gtk.Adjustment` IS NOT BUILT HERE ANY MORE. ADR 0047 § 7 built the edge in this file
// because it was the one file that could import `gi://` without a new dependency; that edge
// is `@gjsify/gtk-host`'s `coerce` now (ADR 0047 § Amendment), so this half hands the
// portable value through and imports no `gi://` at all — as `combo-row.gtk.tsx` does for its
// model, and for the same reason.
//
// THE ADJUSTMENT IS MEMOISED, and it is the same measurement `combo-row.gtk.tsx` carries
// about its model: gtk-host writes a property only when the prop changes by identity, and
// the seam builds a fresh `Gtk.Adjustment` for every object it is handed, so an unmemoised
// one would be re-set on every parent re-render and take the value back to whatever was
// authored. The key is the six numbers, not the object identity — and they are six because
// the authored value arrives as ONE portable `AdwAdjustment` (ADR 0047), normalised by the
// core rather than defaulted here.
//
...
// element. Handing the seam ONE object with the value inside it closes the window instead of
// relying on an order. `preferences.gtk.spec.tsx` asserts a value authored BELOW the range
// comes back at the bound, which is the assertion that would fail if the range arrived late.
...
export function AdwSpinRow({
    ...
    // The core fills the authored subset out to six numbers, so this file picks no default
    // per field. The value is clamped HERE rather than left to the seam: the seam normalises
    // the object it is handed, and a value authored below the range is clamped by the CORE,
    // which both halves share, so the two suites can assert the same number.
    const range = normalizeAdjustment(adjustment);
    const current = clampAdjustmentValue(range, value ?? range.value);

    const adjusted = useMemo<AdwAdjustment>(
        () => ({ ...range, value: current }),
