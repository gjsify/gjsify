// `AdwClamp` — the base module. See `../refuse.ts` for who reaches this and why it throws
 ...
 * Both are libadwaita's easing curve, so the suites assert the same child size and the
 * same centring offset on both sides rather than each side's own idea of "clamped".
 */
