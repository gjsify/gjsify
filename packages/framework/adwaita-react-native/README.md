...
| none honoured (`main`/`module`) | `lib/esm/index.js` — refuses, by name |
...
| claim | held by |
|---|---|
| both halves satisfy one declared surface | `tsc`, through `parity.spec.ts` — a platform module that renames a prop or changes an arity is a compile error |
...
| both halves agree on the number | a 1000-point frame with `maximumSize` 400 gives width 400 at offset 300 — asserted as a GTK allocation on one side and a style object on the other |
