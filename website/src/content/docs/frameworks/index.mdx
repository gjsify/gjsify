...
(`add_response`, `add`), so a child inside one of them is refused by name rather than silently
replacing the widget's template — put your content in an `<adw-dialog>` and use the subclasses
through a `ref`.
