...
**One application per process.** `runApplication` builds the `GApplication`, and a process has
one, so a second call while the first is still running is refused by name rather than quietly
taking the first one's window. Await the call, or render both trees under one root component:

```tsx
async function main(): Promise<void> {
    const code = await AppRegistry.runApplication('Counter', { applicationId: 'com.example.Counter' });
...
`return process.exit(code)`, not a bare `process.exit()`: a bare call leaves the GTK main loop
...
When the window first appears, the layer checks its own chrome and warns if the composition left
you no way to close the window, or two close buttons where one is dead. You will see it on
stderr, and you can assert the same sentences in a test:

```ts
import { lastWindowChromeProblems } from 'react-native';

lastWindowChromeProblems(); // [] once the window has been checked and is well formed
```
...
| `Modal` | `Adw.Dialog`, presented against the parent rather than placed inside it |
...
const items = [
    { id: 'a', title: 'Quarterly report' },
    { id: 'b', title: 'Travel expenses' },
];
...
function Notes() {
    ...
    return (
        <View className="flex-1 p-m">
            <Pressable onPress={() => setOpen(true)}>
                <Text>Rename…</Text>
            </Pressable>

            <Modal visible={open} onRequestClose={() => setOpen(false)}>
                <View className="p-l gap-m">
                    <Text className="text-lg">Rename this note</Text>
...
**`onRequestClose` is required in practice**, exactly as React Native documents it for Android
and tvOS, and here for a reason you can see: the dialog never closes itself. Escape, the close
control and a click on the backdrop all arrive at `onRequestClose`, and setting `visible` to
...
// Whatever handed you the entry point: a URI handler, a `Gio.SimpleAction`,
// or your own restore logic.
const entryHref = '/settings';

router.navigate(entryHref);
```
...
A prop can be answered and still refuse some of its VALUES. `accessibilityRole` is the one that
does: most of React Native's role names map onto a GTK accessible role, and a handful name an
Android class or an iOS trait that has no counterpart at all. Ask about the value:

```ts
import { acceptsPropValue, explainPropValue, propRefusedValues } from '@gjsify/react-native/prop-table';

acceptsPropValue('View', 'accessibilityRole', 'button'); // true
acceptsPropValue('View', 'accessibilityRole', 'keyboardkey'); // false
explainPropValue('View', 'accessibilityRole', 'keyboardkey'); // what to write instead
propRefusedValues('View', 'accessibilityRole'); // every value refused by name
```

`explainProp` and `explainPropValue` return the render's own message rather than a paraphrase of
it, and [`PROPS.md`](https://github.com/gjsify/gjsify/blob/main/packages/framework/react-native/PROPS.md)
is the same data as a document: one section per primitive, with the refused values and their
reasons under the row they belong to.
