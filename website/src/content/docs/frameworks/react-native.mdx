...
- **The container needs a curated child policy.** gtk-host will not guess how a widget adopts a
  ...
  warning at exit 0. 26 of the 164 tags carry such a rule; toolbar view, header bar, status page,
  navigation view, preferences page and action row are among them. Clamp, wrap box and action bar
  are not, and refuse a child by name until someone writes their policy.

The [Layout page](/gjsify/adwaita/layout/) of the Adwaita gallery carries this as a **React
Native on GTK** tab beside the GJS, Blueprint, web and NativeScript ones — on the header bar and
the toolbar view, and on neither of the two whose policy is missing.
...
function Notes() {
    const input = useRef<Gtk.TextView>(null);

    useEffect(() => {
        // multiline text lives in the buffer, not in `value`
        input.current?.get_buffer().set_text('hello', -1);
    }, []);
