---
title: React Native
description: Run React Native components on GTK 4. Primitives, lists, routing, styling and the one build flag that turns it on.
---
...
`@react-navigation/core`, which is a peer rather than a bundled copy.
...
export function Counter({ count, onPress }: { count: number; onPress: () => void }) {
    return (
        <View className="flex-1 items-center justify-center gap-m">
            <Text className="text-heading">{count}</Text>
...
`@gjsify/react-native/router` is the `expo-router` surface, and the build aliases `expo-router`
to it. `router`, `usePathname`, `useLocalSearchParams`, `Stack` and `Tabs` are implemented.
