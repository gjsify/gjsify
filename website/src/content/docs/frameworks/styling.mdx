...
### The accent is not a property you can set

**Measured on libadwaita 1.9.3: `AdwStyleManager:accent-color` is read-only.** It
reports what the user chose; assigning to it throws *Property
AdwStyleManager.accent-color is not writable*, and the ParamSpec carries no
`WRITABLE` flag. So "set the accent from the application's tokens" is not a property
write at all — it is a CSS custom property, `--accent-bg-color` and its 47 siblings,
redefined on `:root`, which is what Adwaita's own rules resolve against.
