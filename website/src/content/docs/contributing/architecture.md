...
- **macOS** runs `@gjsify/node-gi`'s own CI (build, conformance, a real GTK/Adwaita window, the full Adwaita storybook), `@gjsify/napi`'s build and gates, and the native-bridge prebuild job. Two jobs run the `@gjsify/*` suites themselves: `main.yml`'s `macos` job executes a curated subset of `--app gjs` bundles on arm64 under Homebrew `gjs` 1.88, and `macos-suites.yml` runs the Node-pillar suites on both `test:node` and `test:gjs`, on darwin-arm64 and darwin-x64, on every pull request as well as on `main` and the nightly. macOS is the only one of the three where both legs can run at all, which is the point of running them: `test:node` says the spec is right, `test:gjs` says the port is. A third, `gtk-os-suites.yml`, runs the GTK-bearing suites (`@gjsify/gtk-host`, `@gjsify/react-native`) against the PUBLISHED runtime bundle on both darwin arches — on Node over node-gi, installing no `gjs` of its own, because that is the runtime a shipped `.app` carries. `@gjsify/node-gi`'s own macOS legs are gated by its `scope` job on node-gi's sources, so they do not run on every PR; the two suite workflows above are unfiltered and do.
- **Windows** has no GJS host at all, so every job named here runs on **Node**, and `test:gjs` is absent rather than failing. `windows-suites.yml` runs the Node-pillar `@gjsify/*` suites on every pull request as well as on `main` and the nightly, under **cmd.exe with the git-bash utilities stripped from `PATH`** — Git for Windows supplies a real `rm`, `cp` and `sh` that npm's `%COMSPEC%` scripts do not have, and testing from git-bash reports false greens. `node-gi.yml` adds the bridge's own CI, a real GTK window and the storybook against a bundled GTK runtime rather than a system install; those legs are gated by its `scope` job on node-gi's own sources, so they do not run on every PR and the nightly is what bounds how stale the newest Windows result is. `gtk-os-suites.yml` adds the GTK-bearing suites (`@gjsify/gtk-host`, `@gjsify/react-native`) against the PUBLISHED runtime bundle — also on Node, over node-gi — and there both suite steps are `continue-on-error` probes, so that job's colour answers "did the shipped closure stage, verify and load", not "does the layer pass".
