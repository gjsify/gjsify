...
Some GJSify packages need native code. For example, `@gjsify/webgl` ships a Vala-built shared library + GIR typelib to bridge WebGL calls to OpenGL ES through `libepoxy`. These packages publish their binaries under `prebuilds/<os>-<arch>/`, declare the directory via `"gjsify": { "prebuilds": "prebuilds" }` and the targets they promise via `"gjsify": { "platforms": ["linux-x86_64", "darwin-arm64", …] }` in their `package.json`.

### How a prebuild directory is located

Two `<os>-<arch>` spellings are in use, and both resolve:

- **uname-style** — `prebuilds/linux-x86_64/`, `prebuilds/linux-aarch64/`. What every
  Vala/meson bridge stages, because its CI job names the directory after `uname -m`.
- **node-style** — <code>prebuilds/&#36;&#123;process.platform&#125;-&#36;&#123;process.arch&#125;/</code>, i.e. `linux-x64`,
  `darwin-arm64`, `win32-x64`. What `@gjsify/node-gi` and `@gjsify/napi` stage.

The CLI probes the package's own declared spelling from `gjsify.platforms` first, then both
generic spellings for the running host. A package that ships nothing for your platform is
...
`linux-x86_64` and `linux-aarch64`; most also ship `linux-ppc64`, `linux-s390x` and
