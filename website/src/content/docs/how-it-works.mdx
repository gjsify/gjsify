...
<CommandTabs title="Build the bundle">
  ...
  <Fragment slot="npm">
    ```bash
    npx @gjsify/cli@latest build src/index.ts --outfile dist/index.js
    # (--globals auto is the default — no flag needed)
    ```
  </Fragment>
  <Fragment slot="bun">
    ```bash
    bunx @gjsify/cli@latest build src/index.ts --outfile dist/index.js
    # (--globals auto is the default — no flag needed)
    ```
  </Fragment>
  <Fragment slot="deno">
    ```bash
    deno run -A --reload --min-dep-age 0 npm:@gjsify/cli@latest build src/index.ts --outfile dist/index.js
    # (--globals auto is the default — no flag needed)
    ```
  </Fragment>
</CommandTabs>
...
<CommandTabs title="Auto detection, per iteration">
  ...
  <Fragment slot="npm">
    ```bash
    npx @gjsify/cli@latest build src/index.ts -o dist/index.js --verbose
    # [gjsify] --globals auto: iteration 1, 7 global(s): Buffer, fetch, process, …
    # [gjsify] --globals auto: iteration 2, 11 global(s): + AbortSignal, Headers, …
    # [gjsify] --globals auto: converged after 2 iteration(s), 11 global(s)
    ```
  ...
  <Fragment slot="bun">
    ```bash
    bunx @gjsify/cli@latest build src/index.ts -o dist/index.js --verbose
    # [gjsify] --globals auto: iteration 1, 7 global(s): Buffer, fetch, process, …
    # [gjsify] --globals auto: iteration 2, 11 global(s): + AbortSignal, Headers, …
    # [gjsify] --globals auto: converged after 2 iteration(s), 11 global(s)
    ```
  ...
  <Fragment slot="deno">
    ```bash
    deno run -A --reload --min-dep-age 0 npm:@gjsify/cli@latest build src/index.ts -o dist/index.js --verbose
    # [gjsify] --globals auto: iteration 1, 7 global(s): Buffer, fetch, process, …
    # [gjsify] --globals auto: iteration 2, 11 global(s): + AbortSignal, Headers, …
    # [gjsify] --globals auto: converged after 2 iteration(s), 11 global(s)
    ```
...
<CommandTabs title="Auto plus an explicit safety net">
  ...
  <Fragment slot="npm">
    ```bash
    # Auto + the entire DOM group
    npx @gjsify/cli@latest build src/gjs/gjs.ts -o dist/gjs.js --globals auto,dom

    # Auto + a single identifier we know auto can't see
    npx @gjsify/cli@latest build src/index.ts -o dist/index.js --globals auto,matchMedia,FontFace
    ```
  </Fragment>
  <Fragment slot="bun">
    ```bash
    # Auto + the entire DOM group
    bunx @gjsify/cli@latest build src/gjs/gjs.ts -o dist/gjs.js --globals auto,dom

    # Auto + a single identifier we know auto can't see
    bunx @gjsify/cli@latest build src/index.ts -o dist/index.js --globals auto,matchMedia,FontFace
    ```
  </Fragment>
  <Fragment slot="deno">
    ```bash
    # Auto + the entire DOM group
    deno run -A --reload --min-dep-age 0 npm:@gjsify/cli@latest build src/gjs/gjs.ts -o dist/gjs.js --globals auto,dom

    # Auto + a single identifier we know auto can't see
    deno run -A --reload --min-dep-age 0 npm:@gjsify/cli@latest build src/index.ts -o dist/index.js --globals auto,matchMedia,FontFace
    ```
  </Fragment>
</CommandTabs>
...
### Projects scaffolded via `npx @gjsify/cli@latest create`
...
Some GJSify packages need native code. For example, `@gjsify/webgl` ships a Vala-built shared library + GIR typelib to bridge WebGL calls to OpenGL ES through `libepoxy`. These packages publish their binaries under `prebuilds/<os>-<arch>/`, declare the directory via `"gjsify": { "prebuilds": "prebuilds" }` and the targets they promise via `"gjsify": { "platforms": ["linux-x64", "darwin-arm64", …] }` in their `package.json`.

### How a prebuild directory is located

There is one `<os>-<arch>` spelling: <code>&#36;&#123;process.platform&#125;-&#36;&#123;process.arch&#125;</code> — `linux-x64`,
`linux-arm64`, `darwin-arm64`, `win32-x64`. (`linux-ppc64`, `linux-s390x` and `linux-riscv64`
are spelled the same in every vocabulary.) It is what a running process can compute about
itself, so locating the directory needs no translation step.

The CLI probes the package's own declared spelling from `gjsify.platforms` first, then that
canonical name, then — for backward compatibility only — the retired uname spelling
(`prebuilds/linux-x86_64/`) that the Vala/meson bridges staged before the rename, so a
tarball published back then still loads. A package that ships nothing for your platform is
skipped silently — every native bridge is optional at runtime.

### The environment it sets

`gjsify run` scans your `node_modules` for these packages and prepends the directories it
finds to `GI_TYPELIB_PATH` plus **the library-search variable your operating system's loader
actually reads**:
...
:::caution[Which prebuilds exist is a separate question]
Resolution is cross-platform; the *artifacts* are not yet. Every native bridge ships
`linux-x64` and `linux-arm64`; most also ship `linux-ppc64`, `linux-s390x` and
`linux-riscv64`. `darwin-arm64` is built for `lightningcss-native`, `oxfmt-native`,
`terminal-native`, `tls-native`, `http-soup-bridge` and `http2-native`.

This matters most for the **Node-free toolchain**: `gjsify install -g` lays down
`@gjsify/rolldown-native`, `@gjsify/lightningcss-native` and `@gjsify/oxfmt-native`, and
`@gjsify/rolldown-native` is the *only* bundler engine `gjsify build` has under GJS. It no
longer has a Rust-level macOS blocker (its wakeup channel is a portable pipe, not
`eventfd(2)`), but its darwin prebuild has not been built or load-tested yet — so on macOS
GJSify still needs Node present for the build step, falling back to the npm `rolldown` /
`lightningcss` / `oxfmt` packages.

The per-package matrix lives in [Platform Support](/gjsify/platform-support/).
:::
