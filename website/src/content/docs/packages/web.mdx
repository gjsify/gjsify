...
| Package | Notes |
|---|---|
| `@gjsify/adwaita-web` | Web Components (AdwWindow, AdwHeaderBar, AdwPreferencesGroup, AdwCard, AdwSwitchRow, AdwComboRow, AdwSpinRow, AdwToastOverlay, AdwOverlaySplitView) + SCSS partials. Renders the DOM; the behaviour comes from `@gjsify/adwaita-core` |
| `@gjsify/adwaita-core` | Headless Adwaita behaviour — breakpoint conditions, color-scheme observable, toast queue (one at a time + FIFO + auto-dismiss), row state machines (expander / combo / spin / toggle group), alert-dialog response model. Pure TS, no platform imports; shared by the Web and NativeScript renderers |
