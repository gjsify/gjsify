---
title: Overview
description: Which npm packages, Node.js modules and Web APIs you can use on GJS, and exactly how you import them.
---
...
That file builds and runs on GJS. Nothing in it mentions `@gjsify`. When you run `gjsify build`, `node:fs` is rewritten to `@gjsify/fs` (backed by `Gio.File`), and every `node:*` import inside `express` and its dependencies goes the same way. The details are in [How It Works](/gjsify/how-it-works/).

## Install a package from npm

Install it like any other dependency. `gjsify install` is the Node-free path; npm, Bun and Deno work too.
...
`fetch`, `WebSocket`, `crypto.subtle`, `ReadableStream` and friends are globals, not imports. The build detects which ones your bundle actually reaches and registers only those:
...
| Framework | app shell plus tooling | [`@gjsify/adwaita-app`](/gjsify/guides/native-adwaita-app/), [storybook](/gjsify/guides/storybook/), [devtools](/gjsify/guides/devtools/) |
