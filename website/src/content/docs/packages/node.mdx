...
41 Node.js API modules (plus `@gjsify/node-polyfills` meta and 5 Vala native bridges) implemented for GJS using native GNOME libraries. Many of the pure-TypeScript modules (`path`, `buffer`, `events`, `querystring`, `string_decoder`, …) also run unchanged in the browser — each package declares which [runtimes](/gjsify/runtimes/) it supports.
...
| `child_process` | Gio | Full | exec/execSync, execFile/execFileSync, spawn/spawnSync via Gio.Subprocess. Shell (`/bin/sh` · `%ComSpec%` + `/d /s /c`), `detached` (`setsid(1)`), `argv0` and signal numbers resolve per platform; `spawnSync` timeouts run on an in-process GLib timer (no `timeout(1)`). See the [package README](https://github.com/gjsify/gjsify/tree/main/packages/node/child_process#platform-support) for the per-platform contract |
...
| `v8` | GLib | Partial | Wire-format v15 serialize/deserialize; `getHeapStatistics` approximated from OS process memory (`/proc/self/status` on Linux, `ps` on macOS, all-zero where no reader exists); no heap snapshot or CPU profile |
