...
| [`three-loader-ldraw`](./three-loader-ldraw/) | DOM + `@gjsify/webgl` + `@gjsify/fetch` | Three.js LDraw viewer — models streamed from disk over `fetch`, with building steps |
...
### Pick the runtime — `--runtime`

Add `--runtime` to run a showcase on Node.js, Bun or Deno instead of GJS, through the [`@gjsify/node-gi`](/gjsify/projects/node-gi/) reverse bridge — same GTK4 window, same source, a different JS engine underneath:
