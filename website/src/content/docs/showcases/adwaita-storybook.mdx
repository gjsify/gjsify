...
```bash
gjsify showcase adwaita-storybook --runtime node   # or --runtime bun / --runtime deno
```

The same native window, with GTK reached through [`@gjsify/node-gi`](/gjsify/projects/node-gi/) instead of GJS. One `--app node` bundle serves all three — Node-API is their common native ABI — so there is no per-runtime build and no third copy of a story.
