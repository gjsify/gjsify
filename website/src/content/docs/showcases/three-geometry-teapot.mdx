...
A GTK 4 window opens as an `Adw.OverlaySplitView`: the sidebar holds the controls, the canvas fills the rest. Drag to orbit the camera, scroll to zoom.
...
Same window and same scene, with GTK reached through the [node-gi](/gjsify/projects/node-gi/) bridge.
