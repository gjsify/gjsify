...
description: Excalibur.js 0.32 platformer with Tiled tilemaps — runs on GJS through @gjsify/webgl + @gjsify/xmlhttprequest + @gjsify/domparser without an Excalibur.js fork.
---
...
This showcase also declares `node` in `package.json#gjsify.example.runtimes` (`["gjs", "node"]`), so the full Excalibur.js game — WebGL rendering, tilemap loading, input, WebAudio — renders through the [`@gjsify/node-gi`](/gjsify/projects/node-gi/) reverse bridge, no Excalibur fork needed there either:

```bash
gjsify showcase excalibur-jelly-jumper --runtime node
```

Bun and Deno are not declared for this showcase yet — [`canvas2d-fireworks`](../canvas2d-fireworks/) is the one verified on all four runtimes.
