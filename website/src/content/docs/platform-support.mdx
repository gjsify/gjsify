...
## What the Node-free toolchain on macOS still owes

`gjsify build` under GJS needs a bundler, and `@gjsify/rolldown-native` is the only
bundler engine that runs there.

**The engines ship.** All three — `@gjsify/rolldown-native`,
`@gjsify/lightningcss-native`, `@gjsify/oxfmt-native` — publish `darwin-arm64` and
`darwin-x64` prebuilds, and the matrix below marks each of those cells `✓`: declared, a
CI job targets it, artifact committed. The Rust-level blocker that once stood in the way
is long gone — the wakeup channel driving the plugin bridge used to be three
`eventfd(2)` descriptors, a Linux-only syscall the `libc` crate does not expose on Apple
targets; it is now three anonymous pipes, one portable implementation everywhere.

**What is missing is a leg that exercises the Node-free path end to end.** The two
darwin suites install the workspace, bootstrap the toolchain and build it by invoking
the CLI *under Node* (`node …/@gjsify/cli/lib/index.js`), which is the right shape for
testing the Node pillar and says nothing about `gjs -m install.mjs` → `gjsify build`
with no Node on the box. So the prerequisite is met and the claim is untested: a macOS
user may well find the Node-free toolchain works, and nothing here promises it does.

That gap is a CI leg, not an engineering unknown, and it is tracked in
...
The table above is generated from these same functions on every website build.
