...
> Linux's `LD_LIBRARY_PATH`). All three publish `linux-x64`, `linux-arm64`,
> `darwin-arm64` and `darwin-x64`; none publishes a `win32-x64` prebuild, and
> Windows has no GJS host to run one under in any case, so a Node-free toolchain
> there is not a gap but a consequence. On macOS the engines are in place while no
> CI leg yet drives the Node-free path end to end — if it does not work for you,
> Node plus the npm `rolldown` / `lightningcss` / `oxfmt` packages is the fallback.
> See
...
The Deno tab carries a second flag because deno adds a second rule:
...
- **gjs ≥ 1.86** — bundled with Fedora 43+, Arch, and Debian forky/sid.
  **Not Debian 13 (trixie)**, which ships 1.82.3: Debian went straight to 1.88.1 in
  forky, skipping 1.84 and 1.86, so no Debian *stable* satisfies the floor yet.
