...
exec), a CRLF `.cmd` on Windows — and the interpreter it execs follows the OS
rather than your project: `gjs` on Linux, `node` on macOS and Windows, because
that is the runtime each of those platforms' CI actually proves.
...
anywhere, and what you get on a Mac is a Linux package built on a Mac. Flatpak
...
```bash
gjsify ship --stage --target flatpak       # here, on any OS, offline
