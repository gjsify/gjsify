...
out:** the bundle carries no interpreter, so it runs where `node` is already on
`PATH` and not on a stranger's Mac. macOS has GJS but no *relocatable* GJS, and
Windows has no GJS host at all — which is why the macOS formats accept
`gjsify.app: "node"` only, and why a `--app gjs` project can stage the layout and
not pack it. A self-contained artifact on either OS needs a bundled runtime,
...
What is still missing is the `.dmg`, a Windows installer, the bundled runtime
that would make a `.app` self-contained, and signing — a bundle assembled on
Linux is unsigned by construction, and Gatekeeper will quarantine it. A `.dmg` is
