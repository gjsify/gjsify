---
title: Ship your app
description: Turn a built GJS app into something other people can install. One command gives you a .deb and an .rpm, and `--target flatpak` adds a Flatpak bundle; this page also points at the one-line installer, self-executing bundles and dlx.
---
...
**Installable artifacts: Linux.** `.deb`, `.rpm` and a Flatpak bundle, from apps
built for the `gjs` target. The build target is enforced: on Linux, a project
declaring anything other than `gjsify.app: "gjs"` is refused with a message
saying so, instead of producing a package that installs and then fails to start.
...
gjsify ship windows --stage      # a program directory + a .cmd launcher
```

Those two are `--stage` only: the tree is assembled, and nothing wraps it yet.
The launcher each one carries is that OS's — `/bin/sh` without `readlink -f` and
without a `DYLD_*` inside a `.app` (macOS strips inherited ones at the `sh`
exec), a CRLF `.cmd` on Windows — and all three exec `gjs -m` in front of your
bundle, because a `--app gjs` bundle is the only thing `ship` packages today.

That last part is the honest limit of these two layouts, and `gjsify ship`
prints it rather than leaving you to find out: macOS has GJS but no
*relocatable* GJS, so a `.app` a stranger downloads cannot assume the one on
your machine, and Windows has no GJS host at all. A self-contained artifact on
either needs a bundled Node, which is the next milestone. It also tells you
which staged files are only correct on Linux — a `.gschema.xml` is compiled by
the `.deb`'s install step and by nothing else — because the container that
decides what they become does not exist yet either.
...
Mac. (Name the layout there: a bare `gjsify ship` on a Mac assembles the macOS
one, which no format wraps yet, and says so.) Flatpak
...
What is still missing on those two is the CONTAINER, not the tree: the zip
around a `.app`, the `.dmg`, and a Windows installer. A `.dmg` is an image over a
real HFS+/APFS volume and will have to be packed on macOS, which is the same
split one platform over. See
