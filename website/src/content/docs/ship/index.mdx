...
`appimage` is the one row that also needs a NETWORK: `appimagetool` fetches the
AppImage runtime from GitHub on every pack. Every other format here is offline.
