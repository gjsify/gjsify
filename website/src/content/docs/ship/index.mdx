---
title: Ship your app
description: Turn a built GJS app into something other people can install. One command gives you a .deb and an .rpm, `gjsify ship darwin` gives you a macOS .app and a zip, and `--target flatpak` adds a Flatpak bundle; this page also points at the one-line installer, self-executing bundles and dlx.
---
...
Windows is `--stage` only: the tree is assembled, and nothing wraps it yet. The
launcher each layout carries is that OS's — `/bin/sh` without `readlink -f` and
without a `DYLD_*` inside a `.app`, a CRLF `.cmd` on Windows — and each execs the
interpreter your bundle was BUILT for, `gjs -m` or `node`.
...
**The macOS bundle carries its own runtime, and you say so in `package.json`.**
An unzipped `.app` runs on a Mac with no Node and no Homebrew GTK — but only if
the four packages below are installed where you PACKAGE it. They are resolved by
...
looks for the `darwin-x64` packages and nothing else. `GJSIFY_NODE_RUNTIME` and
...
all — which is why the macOS formats accept `gjsify.app: "node"` only, why a
`--app gjs` project can stage the layout and not pack it, and why Windows is
`--stage` only for now.
...
Mac. The macOS `.app` and its zip are the same: `ship` writes the zip itself, so
they pack anywhere too — which is also what keeps `zipinfo` an INDEPENDENT reader
...
What is still missing is the `.dmg`, a Windows installer, a bundled runtime for
Windows, and signing — a bundle assembled on Linux is unsigned by construction,
and Gatekeeper will quarantine it. A `.dmg` is
