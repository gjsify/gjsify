...
Linux and Node where no GJS exists.
...
directory carry them inside, as described below.
...
## What macOS and Windows carry

Neither operating system ships GTK, and neither has a GJS you could put in a
download, so the darwin and windows layouts carry a Node interpreter and a GTK
closure and run `node` only. Set that for those targets alone, and the Linux
package of the same project stays on GJS:

```jsonc
{
  "gjsify": {
    "app": "gjs",                                          // Linux keeps it
...
including the Linux `.deb` from `Depends: gjs` to `Depends: nodejs`.

`gjsify ship` resolves the interpreter and the closure by name out of your
`node_modules`, so they are installed on the machine that packages the app, not
on the one that runs it. Each operating system has its own pair, listed on its
page. `@gjsify/node-gi` is a dependency rather than a devDependency, because a
`--app node` bundle keeps `@gjsify/node-gi/*` external and requires it at run
time:

```jsonc
{
  "devDependencies": {
    "@gjsify/node-runtime-<os>-<arch>": "^0.44.0",  // Node and its LICENSE
    "@gjsify/gtk-runtime-<os>-<arch>": "^0.44.0"    // GTK and GObject-Introspection
  },
  "dependencies": { "@gjsify/node-gi": "^0.44.0" }
}
```

When one is missing, ship names the package to install and still produces an
artifact. That artifact starts only where Node and GTK are already installed,
so use it as an intermediate and do not hand it to a user. For a patched or
unpublished build, `GJSIFY_NODE_RUNTIME` names a directory holding the `node`
binary (`node.exe` on Windows) and its `LICENSE`, and `GJSIFY_GTK_RUNTIME` one
holding the GTK closure.

The `.desktop` entry and the AppStream component travel along and are never
read, because neither macOS nor Windows reads them. Ship lists every such file on
each run. The zips carry the operating system in their filename, `macos` or
`windows`, because `.zip` is the one suffix two layouts share.
...
Ten formats wrap three layouts. Seven of them pack on any host, offline, with
no tool outside this CLI. Three do not, and ship refuses those on the wrong host
...
| `appimage` | linux | no | Linux | `appimagetool`, which no distribution packages |
...
`appimage` is the one row that embeds something this project did not write: the
AppImage runtime the finished file starts with. `gjsify ship` passes a pinned one
when your machine has it at
`/usr/local/share/gjsify/appimage-runtime/runtime-<arch>` (or wherever
`GJSIFY_APPIMAGE_RUNTIME_DIR` points), and says so on every pack. Without one,
`appimagetool` downloads a runtime, so that pack needs a network, and the pack
log says that too.
...
| One portable file for someone else's distro, with no install and no root | `gjsify ship linux --target appimage` | [Linux packages](/gjsify/ship/linux-packages/#the-appimage) |
...
The AppImage and the Flatpak bundle are both one file with no repository behind
it, and they answer different questions. An AppImage needs no installation and
no root at all, and takes GJS and GTK from the machine it runs on. `gjsify
ship` prints that list, and the file repeats it if the runtime is missing. A
Flatpak carries its own runtime and needs flatpak installed to put it anywhere.
