...
Linux workstation, and signed on a macOS host when you pass an identity. The build
target is enforced rather than assumed:
...
What is still missing is the `.dmg`, the Windows `.msi`, and notarisation. The two
operating systems also differ in what an unsigned artifact costs you: Gatekeeper
BLOCKS an unsigned `.app`, while SmartScreen only warns until a download builds
reputation, so a Windows program directory is usable unsigned in a way the macOS
bundle is not. A `.dmg` is an image over a real HFS+/APFS volume and will have to
be packed on macOS, which is the same split one platform over. See
