...
- **Windows, x64.** node-gi builds with MSVC and gvsbuild, and passes the display-free conformance suite on Node. Beyond that, the GTK GUI renders and the full Libadwaita Storybook renders, both measured in CI on `windows-latest` and both on Node. Those Windows legs are scope-gated: CI runs them when node-gi's own sources change, and otherwise on the nightly, so the newest Windows result is normally a day old rather than this commit's. [`@gjsify/gtk-runtime-win32-x64`](https://www.npmjs.com/package/@gjsify/gtk-runtime-win32-x64) ships the prebuilt GTK 4 and Adwaita closure, so no gvsbuild is needed to consume it.
