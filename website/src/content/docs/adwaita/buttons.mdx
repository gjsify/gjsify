...
<AdwWidget title="Adw.ButtonContent" blueprint="adwaita/button-content.blp">
  <Fragment slot="gjs">
    ```ts
    import Adw from 'gi://Adw?version=1';
    import Gtk from 'gi://Gtk?version=4.0';
    import ui from './button-content.blp';

    Adw.init();
    const builder = Gtk.Builder.new_from_string(ui, -1);
    const button = builder.get_object('download_button') as Gtk.Button;
    const content = builder.get_object('content') as Adw.ButtonContent;
  ...
  <Fragment slot="webloader">
    ...
    import tree from './button-content.blp?shared-tree';

    const { root: button } = mountSharedTree(tree);
    const content = button.querySelector<Adw.ButtonContent>('#content')!;
    button.addEventListener('click', () => {
        content.setAttribute('label', 'Downloading…');
    });
    ```
  </Fragment>
  <Fragment slot="nativescriptloader">
    ...
    const button = build(tree) as Gtk.Button;
    const content = button.getViewById<Adw.ButtonContent>('content')!;
...
<AdwWidget title="Adw.SplitButton" blueprint="adwaita/split-button.blp">
  <Fragment slot="gjs">
    ```ts
    import Adw from 'gi://Adw?version=1';
    import Gio from 'gi://Gio?version=2.0';
    import Gtk from 'gi://Gtk?version=4.0';
    import ui from './split-button.blp';

    Adw.init();
    const builder = Gtk.Builder.new_from_string(ui, -1);
    const button = builder.get_object('split_button') as Adw.SplitButton;
  ...
  <Fragment slot="webloader">
    ```ts
    import { mountSharedTree, type Adw } from '@gjsify/adwaita-web';
    import tree from './split-button.blp?shared-tree';

    const { root } = mountSharedTree(tree);
    const button = root as Adw.SplitButton;
    // button.classList.add('flat'); // the header-bar / toolbar variant
  ...
  <Fragment slot="nativescriptloader">
    ...
    const button = build(tree) as Adw.SplitButton;
