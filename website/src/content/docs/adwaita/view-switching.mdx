...
description: "Adwaita view-switching widgets: view switcher, tab view, inline view switcher and carousel. Live preview plus the code for GJS, Blueprint, the web and NativeScript."
---
...
a **view stack** and give the user a **switcher** to pick between them. Four
switchers are on this page: a segmented bar, a strip of tabs, a compact toolbar
control and a swipeable pager. All of them adapt to the window, so a wide desktop
...
(swipe-to-page, drag-to-reorder tabs, the tab bar's auto-hide, cross-fade
transitions) can be simplified here. Click a switcher button, a tab or a carousel
dot to move between pages.
:::
...
Two knobs are worth knowing. The **scroll wheel** pages the carousel by default;
set `allow-scroll-wheel="false"` to leave wheel events to the page instead.
**Long swipes** are off by default; turn them on to let one flick skip several
pages.
