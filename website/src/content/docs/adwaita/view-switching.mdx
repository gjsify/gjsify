...
<AdwWidget title="Adw.ViewSwitcher" padding="flush" stage="height: 300px; width: 100%; display: flex; flex-direction: column;">
  ...
  <Fragment slot="nativescript">
    ...
    const switcher = new Adw.ViewSwitcher();
    // There is no Adw.ViewStack page API behind the switcher here: each view carries its
    // title, icon and content together.
    switcher.setViews([
        { title: 'Inbox', icon: 'mail-unread-symbolic', content: page('mail-unread-symbolic', 'Inbox', 'You have three unread conversations.') },
        { title: 'Starred', icon: 'starred-symbolic', content: page('starred-symbolic', 'Starred', 'Messages you have marked as important.') },
        { title: 'Archive', icon: 'folder-symbolic', content: page('folder-symbolic', 'Archive', 'Older conversations kept for reference.') },
...
<AdwWidget title="Adw.InlineViewSwitcher" padding="flush" stage="height: 300px; width: 100%; display: flex; flex-direction: column;">
  ...
  <Fragment slot="nativescript">
    ...
    const switcher = new Adw.InlineViewSwitcher();
    // There is no displayMode enum: pass '' as the title for icons-only, omit the icon
    // for labels-only.
    switcher.setViews([
        { title: 'Overview', icon: 'view-grid-symbolic', content: page('view-grid-symbolic', 'Overview', 'A quick summary of your project.') },
        { title: 'Activity', icon: 'document-edit-symbolic', content: page('document-edit-symbolic', 'Activity', 'Recent edits and changes.') },
        { title: 'Settings', icon: 'emblem-system-symbolic', content: page('emblem-system-symbolic', 'Settings', 'Configure how things behave.') },
    ] satisfies AdwViewPage[]);
    ```
  </Fragment>
</AdwWidget>
...
<AdwWidget title="Adw.Carousel" padding="flush" stage="height: 220px; width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px;">
  ...
  <Fragment slot="nativescript">
    ...
    for (const [title, style] of [
        ...
        const card = new Gtk.Box({ orientation: 'vertical', height: 260, verticalAlignment: 'middle' });
        card.add_css_class('card');
        card.add_css_class(style);
        const label = new Gtk.Label({ label: title, horizontalAlignment: 'center', verticalAlignment: 'middle' });
        label.add_css_class('title-1');
        card.append(label);
        carousel.append(card);
    }

    // Adw.Carousel draws its own dot row; there is no Adw.CarouselIndicatorDots to bind.
    ```
  </Fragment>
</AdwWidget>
