...
through `node-gi`. Adwaita has to be initialised before the builder meets `Adw.Clamp`,
which an `Adw.Application` does on startup; a script without one calls `Adw.init()`
first.
