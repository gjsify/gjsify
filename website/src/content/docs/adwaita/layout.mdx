...
The header bar and the toolbar view carry a fifth, **React Native on GTK**:
the same widget written with [`@gjsify/react-native`](/gjsify/frameworks/react-native/)
primitives inside it. Clamp and wrap box do not, because
[`@gjsify/gtk-host`](https://github.com/gjsify/gjsify/tree/main/packages/framework/gtk-host)
has no child policy for `AdwClamp` or `AdwWrapBox` yet, and a renderer that
guessed one would call `add`, `append` or `set_child` and be wrong at exit 0.
That tab renders a GTK window, not a phone screen — the [React Native
