...
<AdwWidget title="Adw.ActionRow">
  ...
  <Fragment slot="nativescript">
    ...
    // Gtk.Button is text-only here, so the trailing chevron is a Gtk.Image rather than
    // a flat button — and with no button there is no activatable widget to point at.
...
surfaces without being rewritten. On GJS the property is a real `Gtk.StringList`,
as the code windows above show.
...
<AdwWidget title="Adw.ExpanderRow">
  ...
  <Fragment slot="nativescript">
    ...
    row.add(new Adw.EntryRow({ title: 'Host', text: 'proxy.example.com' }));
    row.add(new Adw.SwitchRow({ title: 'Use authentication', active: false }));
    ```
  </Fragment>
</AdwWidget>
