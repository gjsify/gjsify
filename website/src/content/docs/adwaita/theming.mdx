...
like `#9141ac` has no representation in it. Whether it is writable at all *varies* —
measured read-only on one host and writable on a later libadwaita — so it is not
something to branch on either.
