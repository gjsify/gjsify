---
title: Controls
description: "GTK's standalone controls, the text entry and the drop down. A live preview with its markup, plus the TypeScript, Blueprint and NativeScript code."
---

import AdwWidget from '../../../components/AdwWidget.astro';

Two controls on this page stand on their own rather than inside a boxed list. A
single-line **entry** takes typed text, and a **drop down** picks one value out
of a list. Both belong to GTK. They are `Gtk.Entry` and `Gtk.DropDown`,
libadwaita ships neither, and the Adwaita stylesheet gives them their GNOME look.

Each has a boxed-list twin, `Adw.EntryRow` and `Adw.ComboRow`, documented under
[Boxed Lists](/gjsify/adwaita/boxed-lists/). Which one you want is a question
about the surrounding layout rather than about the control. A preferences page
uses the rows, a toolbar or a dialog uses these.

Every block below is three windows. The first one runs the widget. It is the live
browser port, so it follows the light and dark toggle like a real window.

**GJS** holds the `@girs` program and the Blueprint declaration that goes with it:
two files of one program, against the real libadwaita widget. **UI frameworks** holds
the same widget reached through an element model. Its HTML Web Components tab is the
markup that paints the preview, and one `import '@gjsify/adwaita-web'` registers every
element in it. Solid, Vue and React go through `@gjsify/gtk-host`. NativeScript is the
XML template that runs on a phone. Where a widget is built in code rather than
declared, a recorded reason takes the place of the snippet.

GTK 4 is the reference, typed by
[`@girs/gtk-4.0`](https://www.npmjs.com/package/@girs/gtk-4.0). The browser port
spells both controls the way the GIR does, `<gtk-entry>` and `<gtk-drop-down>`, the same tags
`@gjsify/gtk-host` uses. A widget is named after the library that owns it, and the
Adwaita part of these two is the stylesheet. The NativeScript port names them by the
same rule, `Gtk.Entry` and `Gtk.DropDown`. Where a port differs, the window that shows it
says so. Both answer `gi://Adw` and `gi://Gtk`. On NativeScript that resolves once you
build with `gjsify build --app nativescript --gi-renderer`.

:::note
The previews run the browser port, so behaviour the toolkits drive natively can
be simplified here. Two divergences are worth knowing: `<gtk-entry>` has no
`editable`, so a field you may read but not change is spelled `disabled`, which
also greys it; and the NativeScript drop down opens the platform action sheet,
which has no search field.
:::

## Entry

A single-line text field. Give it a **placeholder** to say what belongs in it
while it is empty, a **value** for the text itself, and `disabled` when it should
be read-only. For a field that belongs in a preferences group, use
[Entry Row](/gjsify/adwaita/boxed-lists/#entry-row) instead. It is the same
control in row presentation, and the label doubles as the row title.

<AdwWidget title="Gtk.Entry">
  <Fragment slot="preview">
    ```html
    <div style="display:flex;flex-wrap:wrap;gap:12px;justify-content:center;align-items:center;">
      <gtk-entry placeholder="Search files…" style="min-width: 280px;"></gtk-entry>

      <!-- `<gtk-entry>` has no `editable`: the browser spelling of a field you may
           read but not change is `disabled`, which also greys it. -->
      <gtk-entry value="notes.md" disabled></gtk-entry>
    </div>
    ```
  </Fragment>
  <Fragment slot="gjs">
    ```ts
    import Gtk from 'gi://Gtk?version=4.0';

    const entry = new Gtk.Entry({
        placeholderText: 'Search files…',
        widthRequest: 280,
        halign: Gtk.Align.CENTER,
    });

    entry.text = 'notes.md';
    entry.editable = false; // read but do not change
    ```
  </Fragment>
  <Fragment slot="blueprint">
    ```blueprint
    using Gtk 4.0;

    Gtk.Entry {
      placeholder-text: _("Search files…");
      width-request: 280;
      halign: center;
    }
    ```
  </Fragment>
  <Fragment slot="nativescript">
    ```ts
    import Gtk from 'gi://Gtk?version=4.0';

    const entry = new Gtk.Entry({
        placeholderText: 'Search files…',
    });

    entry.text = 'notes.md';
    entry.editable = false; // read but do not change
    ```
  </Fragment>
</AdwWidget>

## Drop Down

Pick one value from a list. The button shows the current selection and the list
opens in a popover; **selected** is the zero-based index of the chosen option,
and `enable-search` adds a filter field for a long list. For the same choice
presented as a boxed-list row, use
[Combo Row](/gjsify/adwaita/boxed-lists/#combo-row).

The list is the **model** property — an array of plain strings, or of
`{ value, label }` objects — and it is the same array the combo row takes, so a
model written for one works on the other unchanged. Assign a new one at any time:
the popover follows, and the rows that did not change keep their place.

<AdwWidget title="Gtk.DropDown">
  <Fragment slot="preview">
    ```html
    <!-- `model` is a JSON array of strings, or of { label, value } objects. -->
    <!-- selected: The position of the selected item. -->
    <gtk-drop-down model='["Automatic","Always","Never","When busy"]' selected="0"></gtk-drop-down>
    ```
  </Fragment>
  <Fragment slot="gjs">
    ```ts
    import Gtk from 'gi://Gtk?version=4.0';

    const dropDown = new Gtk.DropDown({
        model: new Gtk.StringList({ strings: ['Automatic', 'Always', 'Never', 'When busy'] }),
        selected: 0,
    });
    // Without an expression the search field has nothing to match against.
    dropDown.expression = Gtk.PropertyExpression.new(Gtk.StringObject.$gtype, null, 'string');
    dropDown.enableSearch = false;
    dropDown.halign = Gtk.Align.CENTER;
    ```
  </Fragment>
  <Fragment slot="blueprint">
    ```blueprint
    using Gtk 4.0;

    Gtk.DropDown {
      selected: 0;
      halign: center;

      model: Gtk.StringList {
        strings [_("Automatic"), _("Always"), _("Never"), _("When busy")]
      };
    }
    ```
  </Fragment>
  <Fragment slot="nativescript">
    ```ts
    import Gtk from 'gi://Gtk?version=4.0';

    const dropDown = new Gtk.DropDown({
        model: new Gtk.StringList({ strings: ['Automatic', 'Always', 'Never', 'When busy'] }),
        selected: 0,
    });
    // The list opens as the platform action sheet, which has no search field —
    // so `enableSearch` has no NativeScript equivalent.
    ```
  </Fragment>
</AdwWidget>

## Related

- [Buttons](../buttons/): the plain button, its style classes and the menu
  button.
- [Boxed Lists](/gjsify/adwaita/boxed-lists/): the row twins of both controls,
  entry row, password entry row, combo row and spin row.
- [Adwaita Storybook](/gjsify/showcases/adwaita-storybook/): the same widgets in
  a live component browser you can poke at.
