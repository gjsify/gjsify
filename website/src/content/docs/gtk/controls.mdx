...
that owns it and the Adwaita part of these two is the stylesheet. NativeScript still
says `AdwEntry` and `AdwDropDown`; that port ships one Adwaita-styled widget set
without dividing it by owning library, and converging it is tracked rather than done.
Where a port behaves differently, its own window says so.
