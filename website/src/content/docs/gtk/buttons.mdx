...
spells both buttons the way the GIR does, `<gtk-button>` and `<gtk-menu-button>`, the same tags
...
same rule, `Gtk.Button` and `Gtk.MenuButton`. Where a port differs, the window that shows it
