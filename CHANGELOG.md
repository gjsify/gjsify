...
* **globals:** Stage 3 of the globals tree-shaking refactor — hybrid auto-injection in the esbuild plugin. User code is scanned for references to known global identifiers (`fetch`, `Buffer`, `process`, `ReadableStream`, `AbortController`, `crypto`, `URL`, …) and the matching `/register` subpaths are auto-injected via esbuild's `inject` option. No more manual `import '@gjsify/*-globals/register'` needed in typical user code. New CLI flags `--auto-globals` (default `true`) and `--globals <list>` support absolute whitelists (`fetch,crypto`) and modifier syntax (`+crypto,-fetch`). The create-gjsify template no longer ships `@gjsify/node-globals`/`@gjsify/node-polyfills` as dependencies. Minimal GTK hello-world bundles stay under 6 KB because no register modules are injected unless user code references globals.
* **globals:** Stage 4 of the globals tree-shaking refactor — experimental AST-based scanner via the hidden `--ast-scan` flag. Uses acorn + acorn-walk to detect global references with full scope analysis, correctly handling shadowed identifiers (`const fetch = ...`) and property accesses (`api.Buffer`). On the 3 reference showcases (express-webserver, canvas2d-fireworks, three-geometry-teapot) both scanners produce identical output and the AST scanner adds only ~15ms overhead (+3%). On synthetic shadowing tests the AST scanner correctly filters out 2 false positives that the regex scanner triggers. Verdict: **both scanners kept as a hybrid**. Regex stays the default (established behavior, negligible real-world difference); `--ast-scan` is available for power users who need strict precision.

### Features

* **cli/build:** add `--auto-globals` and `--globals` flags to `gjsify build` — control which `/register` modules get auto-injected.
* **cli/build:** add hidden `--ast-scan` flag to `gjsify build` — experimental acorn-based auto-globals scanner.
