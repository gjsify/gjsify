...
* **globals:** Stage 4 of the globals tree-shaking refactor — AST-based scanner for `--auto-globals` using acorn + acorn-walk. Scope-aware: correctly handles shadowed identifiers (`const fetch = ...`) and property accesses (`api.Buffer`). On the 3 reference showcases both scanners produce identical output; on synthetic shadowing tests AST correctly filters out 2 false positives that regex triggers. **Stage 5 (same PR #9):** the AST scanner was promoted to the default — the regex implementation is kept only as a silent fallback when acorn cannot parse a file (unusual TypeScript syntax). The `--ast-scan` experimental flag has been removed; the scanner is simply how `--auto-globals` works now. Dependencies: `acorn`, `acorn-walk` added.
