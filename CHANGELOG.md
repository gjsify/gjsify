...
**Known limitation (tracked in STATUS.md):** libsoup stops polling the input stream while a server message is paused, so `'disconnected'` never fires for long-poll/SSE clients that hang up. `SoupMessageLifecycle` opts GET requests *out* of the GC guard so SpiderMonkey GC can eventually collect them, but this isn't a true fix — the inspector-CLI sequential-call test stays capped at 3 iterations because past that the libsoup-side state accumulation crashes the GJS process. Real fixes need either a libsoup `idle_timeout` API or a GJS bindings patch making `g_socket_receive_message` introspectable.
