...
Every package **builds** `--app node`. 26 of 33 pass on node, 22 on bun, 21 on deno; ~57k tests pass per runtime (the bulk is `zlib`'s exhaustive matrix).
...
**Green on node, runtime-specific residue elsewhere (7):** `child_process` (Gio.Subprocess), `dgram` (UDP via Gio.Socket), `dns` (Gio.Resolver), `fs` (Gio.File + Gio.FileMonitor), `worker_threads` (Gio.Subprocess workers + sab-native), `zlib` (Gio.ZlibCompressor), `tty`.
...
**Affects:** `node-globals` (node: hang; bun/deno: early exit), and `fs` + `worker_threads` on bun/deno (same `ran-no-summary` shape). For `fs` the shared cause is now more than a guess: its node leg passes 657/657 since the gap-7 harness fix, so the bun/deno `ran-no-summary` cannot be about missing assets and is the same lifetime shape as the other two.
