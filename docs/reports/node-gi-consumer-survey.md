...
`iframe` left that list. It is wired as its own committed `test:gjs-on-node` leg in `node-gi.yml`'s `gtk-host-node` job, whose container is the one that DOES carry the GTK/WebKit stack — the sibling job follow-up 7 names. That leg is what [ADR 0022](../adr/0022-webkit-on-darwin.md) § *Amendment — the `node` slot* rests on; it re-measured green on GJS 1.88.1 / Node 24.19.0 with `DISPLAY` and `WAYLAND_DISPLAY` both unset, at the same test count as the gjs leg.

It is also where the harness's convenience and a committed leg part company. The harness forces a sibling-polyfill closure with `--alias`; the first draft of this leg copied that device for `@gjsify/message-channel` (`node: "native"`, so routing hands the node target Node's own `MessageChannel`) and went green with it. The alias was covering a defect in the SHIPPING node build rather than an artefact of the leg: the WebKit bridge needs the polyfill's transport hook, and with the host port in its place `_registerTransferredPort()` threw and no transferred port was ever substituted. Fixed at the source — `@gjsify/message-channel` now exports `./core`, which slot routing does not rewrite — and the leg carries no flags. **A committed leg is not the harness**: the harness's aliases serve a survey that deliberately puts polyfills under Node builtins, while a package's own leg must build what its consumers build, so an alias there is a finding rather than a setting.
