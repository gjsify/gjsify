...
- **Whether the AAC gap ever closes on darwin.** `faad` is GPL and `avdec_aac` brings the
  libav closure ADR 0037 refuses, so it is a redistribution decision belonging to whoever
  ships the product. It stays a declared gap there, with no `plugin` — nothing was ever
  going to be copied, so no file's arrival can retire it. win32 closed its own copy of this
  gap a different way: `mediafoundation` registers `mfaacdec`, the OS's own decoder, beside
  the `mfmp3dec` that already closed MP3 there (ADR 0056 § 7, amended 2026-09-25) — a route
