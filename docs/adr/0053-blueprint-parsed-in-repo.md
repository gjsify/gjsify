...
### What the 30 `.blp` files actually use

Measured over the 30 real `.blp` in this tree by
...
|---|---|---|---|
| `using Gtk 4.0;` and `using Adw 1;` — EVERY import line | 58 | carried by the class name | yes — namespace and version |
| `Adw.HeaderBar { }` — an object as its own statement | 140 | `tag: 'AdwHeaderBar'` | yes — the GIR type |
| `title: "…"` — every property whose value is not an anonymous object | 316 | `props: { title: '…' }` | yes — a ParamSpec |
...
| `styles ["flat"]` | 28 | `cssClasses: ['flat']` | yes — ADR 0049 decided style classes are a list |
| `template $Foo: Adw.Bin` | 13 | — | **no** — a GtkBuilder composite-template declaration |
| `Gtk.Box canvasContainer { }` | 72 of those 140 | — | **no** — a GtkBuilder object id |
| `_("Back")` | 57 | — | **no** — a `translatable` attribute on the emitted XML |
...
EVERY import line — 28 `using Adw 1;` and 30 `using Gtk 4.0;`, since not every file
imports both — where reading it as the Adw one alone gives 28. The object row is anchored at
...
table: 316 + 28 = 344, every property assignment in the tree. The 28 are the ones whose
value is an ANONYMOUS object. The other 316 are not one thing: 307 have no object value at
all, and nine have an object value that carries a GtkBuilder id — `content: Gtk.Box
...
against their own total: 316 + 28 was sitting beside the 344 that was already known. Two orphan
