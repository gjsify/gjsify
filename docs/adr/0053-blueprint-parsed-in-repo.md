...
### What the 16 `.blp` files actually use

Measured over the 16 real `.blp` in this tree by
...
|---|---|---|---|
| `using Gtk 4.0;` and `using Adw 1;` — EVERY import line | 31 | carried by the class name | yes — namespace and version |
| `Adw.HeaderBar { }` — an object as its own statement | 105 | `tag: 'AdwHeaderBar'` | yes — the GIR type |
| `title: "…"` — every property whose value is not an anonymous object | 265 | `props: { title: '…' }` | yes — a ParamSpec |
| `[start]`, `[end]`, `[top]`, `[bottom]`, `[center]`, `[breakpoint]` | 29 | `slot: 'start'` | yes — ADR 0029 § 4 derives slot candidates from GIR |
| `content: Adw.ToolbarView { }` — an ANONYMOUS object as a property value | 28 | a child carrying `slot: 'content'` | yes — a ParamSpec, read as a slot |
| `styles ["flat"]` | 20 | `cssClasses: ['flat']` | yes — ADR 0049 decided style classes are a list |
| `template $Foo: Adw.Bin` | 13 | — | **no** — a GtkBuilder composite-template declaration |
| `Gtk.Box canvasContainer { }` | 51 of those 105 | — | **no** — a GtkBuilder object id |
| `_("Back")` | 34 | — | **no** — a `translatable` attribute on the emitted XML |
...
EVERY import line — 15 `using Adw 1;` and 16 `using Gtk 4.0;`, since not every file
imports both — where reading it as the Adw one alone gives 15. The object row is anchored at
...
table: 265 + 28 = 293, every property assignment in the tree. The 28 are the ones whose
value is an ANONYMOUS object. The other 265 are not one thing: 257 have no object value at
all, and eight have an object value that carries a GtkBuilder id — `content: Gtk.Box
...
against their own total: 265 + 28 was sitting beside the 293 that was already known. Two orphan
