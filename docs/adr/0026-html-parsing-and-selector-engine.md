...
against ~24 KB for the complete set.
...
That is the failure class this whole ADR is about. 24 KB is the honest price of "decodes
...
- **Bundles that reference `DOMParser` get materially larger.** The measured comparison
  point is 107,071 bytes for a minified `htmlparser2 + css-select + domutils + entities`
  bundle, of which the entity table is ~23 %. Only bundles that actually name the global
  pay it (`--globals auto` injects on a detected identifier), and the leaf subpaths mean a
  consumer wanting only selectors or only the tokenizer imports only that. The real number
  for `excalibur-jelly-jumper` is measured before/after and replaces this estimate.
