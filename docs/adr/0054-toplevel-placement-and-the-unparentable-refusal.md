...
- A fourth placement kind — `Gtk.Popover`, placed by `set_parent()` and shown by
  `popup()`, is the obvious candidate — is now four `never` arms away instead of two. It
  is not added speculatively: nothing needs it, and ADR 0028's rule holds here as
  everywhere.
